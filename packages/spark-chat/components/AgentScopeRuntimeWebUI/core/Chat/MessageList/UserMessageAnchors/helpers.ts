...
export function getUserMessageAnchor(
  message: IAgentScopeRuntimeWebUIMessage,
): UserMessageAnchor | undefined {
  if (message.role !== 'user' || !message.id) return undefined;

  const request = message.cards?.find(
    (card) => card.code === 'AgentScopeRuntimeRequestCard',
  )?.data as IAgentScopeRuntimeRequest | undefined;
  const contentList =
    request?.input?.flatMap((item) => item.content || []) || [];
  const text = normalizePreviewText(
    contentList.map(getContentText).filter(Boolean).join(' '),
  );
  const attachments = contentList
    .map(getAttachmentPreview)
...
export function getUserMessageAnchorBadgeText(
  count: number,
  badgeMaxCount?: number,
) {
  const normalizedBadgeMaxCount =
    getUserMessageAnchorBadgeMaxCount(badgeMaxCount);
  return count > normalizedBadgeMaxCount
    ? `${normalizedBadgeMaxCount}+`
    : String(count);
}
...
export function areAnchorPositionsEqual(
...
export function getMessageElementInScrollContainer(
  scrollEl: HTMLElement,
  messageId: string,
) {
  const target =
    typeof document === 'undefined' ? null : document.getElementById(messageId);
  if (
...
function getScrollBoundaryAnchorId(
  scrollEl: HTMLElement,
  ...
  if (isDescOrder) {
    ...
    if (maxScrollDistance - scrollDistance <= SCROLL_BOUNDARY_OFFSET)
      return firstAnchor.id;
    return undefined;
  }

  if (scrollEl.scrollTop <= SCROLL_BOUNDARY_OFFSET) return firstAnchor.id;
  if (maxScrollDistance - scrollEl.scrollTop <= SCROLL_BOUNDARY_OFFSET)
...
export function getScrollVisibleTopOffset(scrollEl: HTMLElement) {
  ...
  const layoutRight = scrollEl.closest(
    '[class*="chat-anywhere-layout-right"]',
  ) as HTMLElement | null;
  const header = layoutRight?.querySelector(
    '[class*="chat-anywhere-layout-right-header"]',
  ) as HTMLElement | null;
  const headerRect = header?.getBoundingClientRect();
  const headerOverlap = headerRect
    ? Math.max(0, headerRect.bottom - scrollRect.top)
    : 0;
  const scrollPaddingTop =
    Number.parseFloat(window.getComputedStyle(scrollEl).scrollPaddingTop) || 0;

  return Math.max(
    scrollPaddingTop,
    headerOverlap + (headerOverlap > 0 ? SCROLL_VISIBLE_TOP_GAP : 0),
  );
}
...
export function getActiveAnchorId(
  ...
  anchors: UserMessageAnchor[],
  topOffset = 0,
  targetMap = getMessageElementMapInScrollContainer(scrollEl),
  ...
  renderedAnchors.forEach((anchor) => {
    ...
    const targetRect =
      targetRects?.get(anchor.id) || target.getBoundingClientRect();
...
export function getTargetTopOffset(
  scrollEl: HTMLElement,
  target: HTMLElement,
  topOffset = 0,
) {
