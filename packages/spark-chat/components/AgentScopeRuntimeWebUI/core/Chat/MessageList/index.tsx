...
type MessageWithHistory = IAgentScopeRuntimeWebUIMessage & {
  history?: boolean;
};
...
function getAnchorJumpHistoryRange(
  historyIndex: number,
...
function useSimulatedMessagePagination(
  ...
  const setHistoryRange = useCallback((nextRange: HistoryRange) => {
    ...
    setHistoryRangeState((prev) =>
      areHistoryRangesEqual(prev, nextRange) ? prev : nextRange,
    );
  }, []);

  const updateHistoryRange = useCallback(
    (updater: (prev: HistoryRange) => HistoryRange) => {
  ...
  const visibleHistory = historyMessages.slice(
    historyRange.start,
    historyRange.end,
  );
  ...
  const ensureMessageVisible = useCallback(
    async (messageId: string) => {
      ...
      if (historyIndex >= 0) {
        ...
        if (!targetVisible) {
          const nextRange = getAnchorJumpHistoryRange(
            historyIndex,
