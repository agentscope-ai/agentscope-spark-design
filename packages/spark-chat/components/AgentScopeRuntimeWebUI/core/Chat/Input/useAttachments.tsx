...
import type { IAgentScopeRuntimeWebUISenderAttachmentsOptions } from '../../types';

type UploadCustomRequest = NonNullable<
  IAgentScopeRuntimeWebUISenderAttachmentsOptions['customRequest']
>;
...
export default function useAttachments(
  ...
  const setMergedFileList = useCallback(
    (nextFileList: React.SetStateAction<UploadFile[]>) => {
      const resolvedFileList =
        typeof nextFileList === 'function'
          ? (nextFileList as (prev: UploadFile[]) => UploadFile[])(
              fileListRef.current,
            )
          : nextFileList;
  ...
  const { accept, beforeUpload, customRequest, maxCount, onChange } = rest;
  const disabled = !!options?.disabled;
  const uidCounter = useRef(0);

  const uploadFile = useCallback(
    (fileToUpload: File | Blob, uploadOptions: UploadFileOptions = {}) => {
      return new Promise<UploadFile>((resolve, reject) => {
        ...
        const sourceName =
          ...
          ('name' in fileToUpload ? fileToUpload.name : '');
        ...
        const uploadFileItem: UploadFile = {
          ...
          name:
            sourceName ||
            `file-${timestamp}.${getFileExtension(sourceName, uploadType)}`,
    ...
    (file: File) => {
      ...
      try {
        ...
        if (
          ...
          beforeUploadResult === Upload.LIST_IGNORE
        ) {
          return;
        }
        ...
        if (
          beforeUploadResult &&
          typeof (beforeUploadResult as Promise<unknown>).then === 'function'
        ) {
          (beforeUploadResult as Promise<File | Blob | boolean | string>)
            .then((processedFile) => {
              if (
                processedFile === false ||
                processedFile === Upload.LIST_IGNORE
              ) {
                return;
              }
  ...
  if (customRequest) {
    const uploadIconButton = (
      ...
      >
        {trigger ? (
          React.createElement(trigger, { disabled })
        ) : (
          <IconButton
            disabled={disabled}
