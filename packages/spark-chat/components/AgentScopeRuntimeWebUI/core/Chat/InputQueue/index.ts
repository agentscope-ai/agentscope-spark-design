...
export const INPUT_QUEUE_STORAGE_SCHEMA_VERSION = 2;
export const INPUT_QUEUE_CONTEXT_MISMATCH_ERROR =
  'Queued input request context no longer matches its queue session.';

export function isInputQueueEnabled(options?: boolean | { enable?: boolean }) {
  return (
    options === true ||
    (!!options && typeof options === 'object' && options.enable !== false)
  );
}
