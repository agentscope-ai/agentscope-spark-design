...
test('input queue is opt-in and disabled when sender.queue is omitted', () => {
  assert.equal(isInputQueueEnabled(), false);
  assert.equal(isInputQueueEnabled(false), false);
  assert.equal(isInputQueueEnabled(true), true);
  assert.equal(isInputQueueEnabled({}), true);
  assert.equal(isInputQueueEnabled({ enable: false }), false);
});
...
test('queued input item preserves full message body aliases', () => {
  ...
  assert.equal(item.data.session_id, 'session-b');
  assert.deepEqual(item.data.context, { workspaceId: 'workspace-b' });
});
