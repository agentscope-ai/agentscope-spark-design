"use strict";(self.webpackChunk_agentscope_ai_chat=self.webpackChunk_agentscope_ai_chat||[]).push([[4229],{10270:function(h,n,e){e.r(n),e.d(n,{default:function(){return f}});var P=e(5574),a=e.n(P),v=e(37668),c=e(44116),o=e(64057),i=e(45286),s=e(31912),t=e(21403),u=e(67294),l=e(56290),g=e(85893),b="agent-scope-runtime-webui-sessions";function f(p){var r=(0,l.useTranslation)(),d=r.t,x=(0,u.useState)(!1),y=a()(x,2),m=y[0],k=y[1],E=(0,u.useState)(""),C=a()(E,2),D=C[0],M=C[1];(0,u.useEffect)(function(){if(m){var T=localStorage.getItem(b)||"";M(T)}},[m]);var S=function(){try{localStorage.setItem(b,D),c.Z.success((d==null?void 0:d("common.saveSuccess"))||"\u4FDD\u5B58\u6210\u529F"),location.reload()}catch(I){c.Z.error((d==null?void 0:d("common.saveFailed"))||"\u4FDD\u5B58\u5931\u8D25")}};return(0,g.jsxs)(g.Fragment,{children:[(0,g.jsx)(o.Z,{onClick:function(){return k(!0)},icon:(0,g.jsx)(v.Z,{}),bordered:!1}),(0,g.jsx)(i.Z,{destroyOnHidden:!0,open:m,onClose:function(){return k(!1)},title:(d==null?void 0:d("messageImport.title"))||"Sessions \u6570\u636E\u5BFC\u5165",styles:{body:{padding:16},header:{padding:8}},children:(0,g.jsxs)("div",{style:{display:"flex",flexDirection:"column",gap:12},children:[(0,g.jsx)(s.Z.TextArea,{value:D,onChange:function(I){return M(I.target.value)},placeholder:(d==null?void 0:d("messageImport.placeholder"))||"\u8F93\u5165 JSON \u6570\u636E\u4EE5\u8986\u76D6\u5F53\u524D sessions",rows:10,style:{fontFamily:"monospace"}}),(0,g.jsx)(t.Z,{type:"primary",onClick:S,children:(d==null?void 0:d("messageImport.saveToLocalStorage"))||"\u4FDD\u5B58\u5230 LocalStorage"})]})})]})}},22896:function(h,n,e){e.r(n),e.d(n,{default:function(){return o}});var P=e(21054),a=e(9053),v=e(85893),c=(0,a.kc)(function(i){var s=i.token,t=i.css;return{label:{marginBottom:6,fontSize:12,color:s.colorTextSecondary}}});function o(i){var s=c(),t=s.styles,u=i.isList?(0,v.jsx)(P.Z.List,{name:i.name,children:i.children}):(0,v.jsx)(P.Z.Item,{name:i.name,normalize:i.normalize,children:i.children});return(0,v.jsxs)("div",{children:[i.label&&(0,v.jsx)("div",{className:t.label,children:i.label}),u]})}},24546:function(h,n,e){e.r(n);var P=e(5574),a=e.n(P),v=e(67294),c=e(21054),o=e(96074),i=e(34041),s=e(51663),t=e(48045),u=e(13457),l=e(86250),g=e(9053),b=e(31108),f=e(64057),p=e(21403),r=e(12624),d=e(50287),x=e(22896),y=e(74503),m=e(85893),k=(0,g.kc)(function(C){var D=C.token,M=C.css;return{container:{height:"100%",display:"flex",flexDirection:"column"},form:{height:0,flex:1,padding:"8px 16px 16px 16px",overflow:"auto"},actions:{padding:16,display:"flex",borderTop:"1px solid ".concat(D.colorBorderSecondary),justifyContent:"flex-end",gap:16}}}),E=function(D){var M=D.value,S=D.onChange,T=k(),I=T.styles,O=c.Z.useForm(),L=a()(O,1),w=L[0],B=function(){w.validateFields().then(function(W){S(W)})},ne=function(){w.setFieldsValue(y.default)};return(0,m.jsxs)("div",{className:I.container,children:[(0,m.jsxs)(c.Z,{className:I.form,form:w,layout:"vertical",initialValues:M,children:[(0,m.jsx)(o.Z,{orientation:"left",children:"General"}),(0,m.jsx)(x.default,{name:["theme","locale"],label:"Language",children:(0,m.jsx)(i.default,{options:[{label:"\u4E2D\u6587",value:"cn"},{label:"English",value:"en"}]})}),(0,m.jsx)(o.Z,{orientation:"left",children:"Theme"}),(0,m.jsx)(x.default,{name:["theme","colorPrimary"],label:"colorPrimary",normalize:function(W){return W.toHexString()},children:(0,m.jsx)(s.Z,{})}),(0,m.jsx)(x.default,{name:["theme","colorBgBase"],label:"colorBgBase",normalize:function(W){return W.toHexString()},children:(0,m.jsx)(s.Z,{})}),(0,m.jsx)(x.default,{name:["theme","colorTextBase"],label:"colorTextBase",normalize:function(W){return W.toHexString()},children:(0,m.jsx)(s.Z,{})}),(0,m.jsx)(x.default,{name:["theme","darkMode"],label:"darkMode",children:(0,m.jsx)(b.Z,{})}),(0,m.jsx)(x.default,{name:["theme","leftHeader","logo"],label:"leftHeader.logo",children:(0,m.jsx)(t.Z,{})}),(0,m.jsx)(x.default,{name:["theme","leftHeader","title"],label:"leftHeader.title",children:(0,m.jsx)(t.Z,{})}),(0,m.jsx)(o.Z,{orientation:"left",children:"Sender"}),(0,m.jsx)(x.default,{name:["sender","disclaimer"],label:"disclaimer",children:(0,m.jsx)(t.Z,{})}),(0,m.jsx)(x.default,{name:["sender","attachments"],label:"attachments",children:(0,m.jsx)(b.Z,{})}),(0,m.jsx)(x.default,{name:["sender","maxLength"],label:"maxLength",children:(0,m.jsx)(u.Z,{min:1e3})}),(0,m.jsx)(o.Z,{orientation:"left",children:"Welcome"}),(0,m.jsx)(x.default,{name:["welcome","greeting"],label:"greeting",children:(0,m.jsx)(t.Z,{})}),(0,m.jsx)(x.default,{name:["welcome","description"],label:"description",children:(0,m.jsx)(t.Z,{})}),(0,m.jsx)(x.default,{name:["welcome","avatar"],label:"avatar",children:(0,m.jsx)(t.Z,{})}),(0,m.jsx)(x.default,{name:["welcome","prompts"],isList:!0,label:"prompts",children:function(W,J){var q=J.add,te=J.remove;return(0,m.jsx)("div",{children:W.map(function(K){return(0,m.jsxs)(l.Z,{gap:6,children:[(0,m.jsx)(c.Z.Item,{style:{flex:1},name:[K.name,"value"],children:(0,m.jsx)(t.Z,{})},K.key),(0,m.jsx)(f.Z,{icon:(0,m.jsx)(r.Z,{}),onClick:function(){return q({})}}),(0,m.jsx)(f.Z,{icon:(0,m.jsx)(d.Z,{}),onClick:function(){return te(K.name)}})]},K.key)})})}}),(0,m.jsx)(o.Z,{orientation:"left",children:"API"}),(0,m.jsx)(x.default,{name:["api","baseURL"],label:"baseURL",children:(0,m.jsx)(t.Z,{})}),(0,m.jsx)(x.default,{name:["api","token"],label:"token",children:(0,m.jsx)(t.Z,{})})]}),(0,m.jsxs)("div",{className:I.actions,children:[(0,m.jsx)(p.Z,{onClick:ne,children:"Reset"}),(0,m.jsx)(p.Z,{type:"primary",onClick:B,children:"Save & Copy"})]})]})};n.default=E},74503:function(h,n,e){e.r(n),n.default=function(){return{theme:{locale:"en",colorPrimary:"#615CED",darkMode:!0,prefix:"agentscope-runtime-webui",leftHeader:{logo:"https://img.alicdn.com/imgextra/i2/O1CN01lmoGYn1kjoXATy4PX_!!6000000004720-2-tps-200-200.png",title:"Runtime WebUI"}},sender:{attachments:!0,maxLength:1e4,disclaimer:"AI can also make mistakes, so please check carefully and use it with caution",suggestions:[{label:"Draw a picture",value:"draw"},{label:"Check some knowledge",value:"knowledge"}]},welcome:{greeting:"Hello, how can I help you today?",nick:"Assistant",description:"I am a helpful assistant that can help you with your questions.",avatar:"https://img.alicdn.com/imgextra/i2/O1CN01lmoGYn1kjoXATy4PX_!!6000000004720-2-tps-200-200.png",prompts:[{value:"Hello"},{value:"How are you?"},{value:"What can you do?"}]},api:{baseURL:"",token:""}}}},21122:function(h,n,e){e.r(n),e.d(n,{default:function(){return u}});var P=e(5574),a=e.n(P),v=e(87697),c=e(64057),o=e(45286),i=e(67294),s=e(24546),t=e(85893);function u(l){var g=(0,i.useState)(!1),b=a()(g,2),f=b[0],p=b[1];return(0,t.jsxs)(t.Fragment,{children:[(0,t.jsx)(c.Z,{onClick:function(){return p(!0)},icon:(0,t.jsx)(v.Z,{}),bordered:!1}),(0,t.jsx)(o.Z,{destroyOnHidden:!0,open:f,onClose:function(){return p(!1)},styles:{body:{padding:0},header:{padding:8}},children:(0,t.jsx)(s.default,{value:l.value,onChange:function(d){p(!1),l.onChange(d)}})})]})}},46989:function(h,n,e){e.r(n),e.d(n,{default:function(){return p}});var P=e(97857),a=e.n(P),v=e(5574),c=e.n(v),o=e(66672),i=e(13224),s=e(21122),t=e(67294),u=e(74503),l=e(51283),g=e(86250),b=e(10270),f=e(85893);function p(){var r=(0,t.useRef)(null);window.chatRef=r;var d=(0,l.Z)("agent-scope-runtime-webui-options",{defaultValue:(0,u.default)(),listenStorageChange:!0}),x=c()(d,2),y=x[0],m=x[1],k=(0,t.useMemo)(function(){var E=(0,f.jsxs)(g.Z,{gap:16,children:[(0,f.jsx)(s.default,{value:y,onChange:function(D){m(function(M){return a()(a()({},M),D)})}}),(0,f.jsx)(b.default,{})]});return a()(a()({},y),{},{customToolRenderConfig:{},session:{multiple:!0},sender:a()(a()({},y.sender),{},{beforeUI:(0,f.jsx)(o.Z.BeforeUIContainer,{children:(0,f.jsx)(g.Z,{gap:6,children:y.welcome.prompts.map(function(C){return(0,f.jsx)("a",{onClick:function(){var M;(M=r.current)===null||M===void 0||M.input.submit({query:C.value})},children:C.value},C.value)})})}),attachments:y.sender.attachments?{customRequest:function(D){D.onProgress({percent:100}),D.onSuccess({url:URL.createObjectURL(D.file)})}}:void 0}),theme:a()(a()({},y.theme),{},{rightHeader:E}),api:a()(a()({},y.api),{},{cancel:function(D){console.log("cancel",D)}})})},[y]);return(0,f.jsx)("div",{style:{height:"100dvh"},children:(0,f.jsx)(i.Z,{ref:r,options:k})})}},29772:function(h,n,e){e.r(n);var P=e(97857),a=e.n(P),v=e(19632),c=e.n(v),o=e(68400),i=e.n(o),s=e(70833),t=e(9053),u=e(55682),l=e(85893),g,b=(0,t.vJ)(g||(g=i()([`
  .user-flow-node {
    width: calc(100% - 32px);
    height: calc(100% - 32px);
    background-color: var(--sps-color-fill);
    border-radius: 8px;
    padding: 10px 16px 24px;
    box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.12), 0 2px 4px 0 rgba(0, 0, 0, 0.1);
    box-sizing: content-box;
  }

  .user-flow-node-name {
    font-size: 16px;
    font-weight: bold;
    color: var(--sps-color-text);
    margin-bottom: 4px;
  }

  .user-flow-node-metric {
    font-size: 12px;
    color: var(--sps-color-text);
    display: flex;
    justify-content: space-between;
    line-height: 1.5;
  }

  .user-flow-node-metric--value {
    font-weight: bold;
  }
`]))),f={nodes:[{id:"node-0",name:"\u9875\u9762-0",layerName:"\u5C42\u7EA70",measure:{name:"DAU",value:175e8,formattedValue:175,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:1e6,formattedValue:100,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#F5A623"}},{id:"node-1",name:"\u9875\u97621",layerName:"\u5C42\u7EA70",measure:{name:"DAU",value:55e8,formattedValue:55,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:5e5,formattedValue:50,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#F5A623"}},{id:"node-2",name:"\u9875\u97622",layerName:"\u5C42\u7EA70",measure:{name:"DAU",value:1e9,formattedValue:10,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:1e5,formattedValue:10,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#F5A623"}},{id:"node-3",name:"\u9875\u97623",layerName:"\u5C42\u7EA70",measure:{name:"DAU",value:9e8,formattedValue:9,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:5e4,formattedValue:5,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#F5A623"}},{id:"node-4",name:"\u9875\u97624",layerName:"\u5C42\u7EA70",measure:{name:"DAU",value:57e8,formattedValue:57,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:6e4,formattedValue:6,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#F5A623"}},{id:"node-5",name:"\u9875\u97625",layerName:"\u5C42\u7EA71",measure:{name:"DAU",value:24e9,formattedValue:240,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:16e5,formattedValue:160,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#4A90E2"}},{id:"node-6",name:"\u9875\u97626",layerName:"\u5C42\u7EA71",measure:{name:"DAU",value:66e8,formattedValue:66,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:11e4,formattedValue:11,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#4A90E2"}},{id:"node-7",name:"\u9875\u97627",layerName:"\u5C42\u7EA72",measure:{name:"DAU",value:5e9,formattedValue:50,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:5e5,formattedValue:50,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#7ED321"}},{id:"node-8",name:"\u9875\u97628",layerName:"\u5C42\u7EA72",measure:{name:"DAU",value:5e9,formattedValue:50,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:5e5,formattedValue:50,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#7ED321"}},{id:"node-9",name:"\u9875\u97629",layerName:"\u5C42\u7EA72",measure:{name:"DAU",value:9e10,formattedValue:90,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:5e5,formattedValue:50,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#7ED321"}},{id:"node-10",name:"\u9875\u976210",layerName:"\u5C42\u7EA72",measure:{name:"DAU",value:1e11,formattedValue:100,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:1e5,formattedValue:10,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#7ED321"}},{id:"node-11",name:"\u9875\u976211",layerName:"\u5C42\u7EA72",measure:{name:"DAU",value:1e9,formattedValue:10,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:9e4,formattedValue:9,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#7ED321"}},{id:"node-12",name:"\u9875\u976212",layerName:"\u5C42\u7EA72",measure:{name:"DAU",value:6e8,formattedValue:6,formattedUnit:"\u4E07"},relatedMeasures:[{name:"MAU",value:1e4,formattedValue:1,formattedUnit:"\u4E07"}],compareMeasures:[],style:{stroke:"#7ED321"}}],edges:[{id:"edge-0",source:"node-0",target:"node-5",measure:{name:"DAU",value:175e8,formattedValue:175,formattedUnit:"\u4E07"}},{id:"edge-1",source:"node-1",target:"node-5",measure:{name:"DAU",value:55e8,formattedValue:55,formattedUnit:"\u4E07"}},{id:"edge-2",source:"node-2",target:"node-5",measure:{name:"DAU",value:1e9,formattedValue:10,formattedUnit:"\u4E07"}},{id:"edge-3",source:"node-3",target:"node-6",measure:{name:"DAU",value:9e8,formattedValue:9,formattedUnit:"\u4E07"}},{id:"edge-4",source:"node-5",target:"node-7",measure:{name:"DAU",value:5e9,formattedValue:50,formattedUnit:"\u4E07"}},{id:"edge-5",source:"node-5",target:"node-8",measure:{name:"DAU",value:5e9,formattedValue:50,formattedUnit:"\u4E07"}},{id:"edge-6",source:"node-5",target:"node-9",measure:{name:"DAU",value:9e9,formattedValue:90,formattedUnit:"\u4E07"}},{id:"edge-7",source:"node-5",target:"node-10",measure:{name:"DAU",value:5e9,formattedValue:50,formattedUnit:"\u4E07"}},{id:"edge-8",source:"node-6",target:"node-11",measure:{name:"DAU",value:1e9,formattedValue:10,formattedUnit:"\u4E07"}},{id:"edge-9",source:"node-4",target:"node-6",measure:{name:"DAU",value:57e8,formattedValue:57,formattedUnit:"\u4E07"}},{id:"edge-11",source:"node-6",target:"node-12",measure:{name:"DAU",value:6e8,formattedValue:6,formattedUnit:"\u4E07"}},{id:"edge-12",source:"node-6",target:"node-10",measure:{name:"DAU",value:5e9,formattedValue:50,formattedUnit:"\u4E07"}}]},p=function(x){var y=x.data,m=[{name:"DAU",value:y.measure.formattedValue+y.measure.formattedUnit},{name:"MAU",value:y.relatedMeasures[0].formattedValue+y.relatedMeasures[0].formattedUnit}];return(0,l.jsxs)("div",{className:"user-flow-node",children:[(0,l.jsx)("div",{className:"user-flow-node-name",children:y.name}),m.map(function(k){return(0,l.jsxs)("div",{className:"user-flow-node-metric",children:[(0,l.jsx)("div",{className:"user-flow-node-metric-name",children:k.name}),(0,l.jsx)("div",{className:"user-flow-node-metric-value",children:k.value})]},k.name)})]})},r=function(x){var y=["node-5","node-6"],m=function(E){return x.nodes.find(function(C){return C.id===E})};return x.edges.forEach(function(k){k.data||(k.data={});var E=y.includes(k.source);k.data.type=E?"split":"proportion",k.data.ratio=k.measure.value/m(E?k.source:k.target).measure.value}),x};n.default=function(d){var x={autoFit:"view",padding:8,data:r(f),node:{style:{component:function(m){return(0,l.jsx)(p,{data:m})},size:[160,90]}},edge:{style:{stroke:function(m){return m.data.type==="split"?"l(0) 0:#F04864 0.5:#7EC2F3 1:#1890FF":"l(0) 0:#1890FF 0.5:#7EC2F3 1:#F04864"},labelText:function(m){var k=m.data,E=k.type,C=k.ratio,D=E==="split"?"\u5206\u6D41":"\u5360\u6BD4";return"".concat(D," ").concat((Number(C)*100).toFixed(2),"%")},labelBackground:!0}},transforms:function(m){return[].concat(c()(m),[{type:"map-edge-line-width",key:"map-edge-line-width",value:function(E){return E.data.ratio},minValue:0,maxValue:1,minLineWidth:1,maxLineWidth:32}])},layout:{type:"dagre",nodesep:40,ranksep:200}};return(0,l.jsxs)(l.Fragment,{children:[(0,l.jsx)(b,{}),(0,l.jsx)(u.Z,{title:d.data.page,style:{maxWidth:"100%",minWidth:"450px"},children:(0,l.jsx)("div",{style:{height:"400px"},children:(0,l.jsx)(s.FlowDirectionGraph,a()({},x))})})]})}},10333:function(h,n,e){e.r(n);var P=e(5574),a=e.n(P),v=e(67294),c=e(55682),o=e(34041),i=e(21403),s=e(86250),t=e(4421),u=e(88773),l=e(85893);function g(b){console.log(b);var f=(0,t.y)()||{},p=f.updateMessage,r=(0,v.useState)(b.data.pages[0]),d=a()(r,2),x=d[0],y=d[1];return(0,l.jsx)(c.Z,{title:"Please select a page",children:(0,l.jsxs)(s.Z,{gap:8,children:[(0,l.jsx)(o.default,{value:x,onChange:y,options:b.data.pages.map(function(m){return{label:m,value:m}})}),(0,l.jsx)(i.Z,{onClick:function(){p({id:(0,u.Z)(),role:"assistant",cards:[{code:"PagePVChartCard",data:{page:x}}]})},children:"OK"}),(0,l.jsx)(i.Z,{onClick:function(){var k,E;(k=b.context.onInput)===null||k===void 0||(E=k.onSubmit)===null||E===void 0||E.call(k,{query:x})},children:"Send"})]})})}n.default=g},79122:function(h,n,e){e.r(n),e.d(n,{default:function(){return z}});var P=e(5574),a=e.n(P),v=e(19632),c=e.n(v),o=e(15009),i=e.n(o),s=e(99289),t=e.n(s),u=e(97857),l=e.n(u),g=e(68400),b=e.n(g),f=e(44987),p=e.n(f),r=e(67294),d=e(73935),x=e(4393),y=e(86250),m=e(53531),k=e(48045),E=e(21403),C=e(78234),D=e(39693),M=e.n(D),S=e(50130),T=e(67369),I=e(88773),O=e(7354),L=e(29041),w=e(31295),B=e(76289),ne=e(60227),V=e(98839),W=e(81644),J=e(18081),q=e(17738),te=e(99387),K=e(9053),j=e(85893),H,Q=(0,K.vJ)(H||(H=b()([`
.sps-chat-anywhere-chat-welcome {
  height: auto !important;
}
`])));function z(){var U=(0,r.useRef)();window.ref=U;var ie=(0,te.useDemoContext)(),Y=ie.demoContext,ae=ie.getDemoContext,pe=ie.setDemoContext,A=(0,r.useRef)({});(0,C.Z)(function(){U.current.updateSession(q.default.get())});var X=(0,r.useCallback)(function(){var N=arguments.length>0&&arguments[0]!==void 0?arguments[0]:"finished",Z=arguments.length>1?arguments[1]:void 0;A.current.answer.msgStatus=N,A.current.answer.cards=A.current.answer.cards.map(function(G){return G.code==="Text"?(0,S.L)("Text",l()(l()({},G.data),{},{msgStatus:N})):G.code==="DeepThink"?(0,S.L)("DeepThink",l()(l()({},G.data),{},{loading:!1})):G}),N==="interrupted"&&A.current.answer.cards.push((0,S.L)("Interrupted",{})),A.current.answer.cards.push((0,S.L)("Footer",{left:(0,j.jsx)(T.p4,{data:[{icon:(0,j.jsx)(ne.Z,{}),label:"\u91CD\u65B0\u751F\u6210",onClick:function(){be(A.current.answer)}}]}),right:Z?(0,j.jsx)(T.hw,{data:[["\u8F93\u5165tokens",Z.prompt_tokens],["\u8F93\u51FAtokens",Z.completion_tokens]]}):null})),U.current.setLoading(!1),d.flushSync(function(){U.current.updateMessage(A.current.answer)}),me()},[Y]),me=(0,r.useCallback)(t()(i()().mark(function N(){return i()().wrap(function(G){for(;;)switch(G.prev=G.next){case 0:d.flushSync(function(){U.current.updateSessionMessages(U.current.getMessages())}),q.default.set(U.current.getSession());case 2:case"end":return G.stop()}},N)})),[Y]),ye=(0,r.useCallback)(function(){var N=t()(i()().mark(function Z(G){var re,ee,_,oe,le,$,F,se,Pe,ge,he,fe,ce;return i()().wrap(function(R){for(;;)switch(R.prev=R.next){case 0:return A.current.answer={id:(0,I.Z)(),cards:[],content:"",role:"assistant",msgStatus:"generating"},U.current.updateMessage(A.current.answer),A.current.abortController=new AbortController,re=G.map(function(ve){return l()(l()({},ve),{},{content:ve.content||(ve.cards||[]).reduce(function(Se,Ce){return Se+(Ce.code==="Text"?Ce.data.content:"")},"")})}),R.next=6,fetch("https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer ".concat(ae().apikey)},body:JSON.stringify({model:ae().model,enable_thinking:ae().enable_thinking,messages:re,stream:!0,stream_options:{include_usage:!0}}),signal:A.current.abortController.signal});case 6:if(ee=R.sent,ee.status===200){R.next=9;break}return R.abrupt("return");case 9:_="",oe="",le=!1,$=!1,R.prev=13,se=p()((0,O.Z)({readableStream:ee.body},{openaiCompatible:!0}));case 15:return R.next=17,se.next();case 17:if(!(le=!(Pe=R.sent).done)){R.next=36;break}if(ge=Pe.value,A.current.answer.msgStatus!=="interrupted"){R.next=22;break}return A.current.abortController.abort(),R.abrupt("break",36);case 22:if(ge.data!=="[DONE]"){R.next=25;break}return X("finished"),R.abrupt("break",36);case 25:if(ce=JSON.parse(ge.data),!ce.usage){R.next=29;break}return X("finished",ce.usage),R.abrupt("break",36);case 29:_+=((he=ce.choices[0])===null||he===void 0?void 0:he.delta.content)||"",oe+=((fe=ce.choices[0])===null||fe===void 0?void 0:fe.delta.reasoning_content)||"",A.current.answer.cards=M()([oe&&(0,S.L)("DeepThink",{content:oe,loading:!_}),_&&(0,S.L)("Text",{content:_,animation:!0,msgStatus:"generating"})]),U.current.updateMessage(A.current.answer);case 33:le=!1,R.next=15;break;case 36:R.next=42;break;case 38:R.prev=38,R.t0=R.catch(13),$=!0,F=R.t0;case 42:if(R.prev=42,R.prev=43,!(le&&se.return!=null)){R.next=47;break}return R.next=47,se.return();case 47:if(R.prev=47,!$){R.next=50;break}throw F;case 50:return R.finish(47);case 51:return R.finish(42);case 52:case"end":return R.stop()}},Z,null,[[13,38,42,52],[43,,47,51]])}));return function(Z){return N.apply(this,arguments)}}(),[Y]),Ee=(0,r.useCallback)(function(){var N=t()(i()().mark(function Z(G){var re,ee,_,oe;return i()().wrap(function($){for(;;)switch($.prev=$.next){case 0:if(ae().apikey){$.next=3;break}return document.dispatchEvent(new CustomEvent("settingOpen")),$.abrupt("return");case 3:Me(),A.current.answer=void 0,A.current.query={id:(0,I.Z)(),cards:[{code:"Text",data:{content:G.query,msgStatus:"finished"}}],role:"user",msgStatus:"finished"},(re=G.fileList)!==null&&re!==void 0&&re.length&&(ee=[],_=[],oe=G.fileList.reduce(function(F,se){return[].concat(c()(F),c()(se))},[]),oe.forEach(function(F){F.type.startsWith("image")?ee.push(F):_.push(F)}),ee.length&&A.current.query.cards.push((0,S.L)("Images",ee.map(function(F){return F.response}))),_.length&&A.current.query.cards.push((0,S.L)("Files",_.map(function(F){return{filename:F.name,bytes:F.size}})))),U.current.setLoading(!0),d.flushSync(function(){U.current.updateMessage(A.current.query)}),U.current.scrollToBottom(),ye(U.current.getMessages());case 11:case"end":return $.stop()}},Z)}));return function(Z){return N.apply(this,arguments)}}(),[Y]),ke=(0,r.useCallback)(function(){X("interrupted")},[Y]),Me=(0,r.useCallback)(function(){try{A.current.answer.cards=A.current.answer.cards.filter(function(N){return N.code!=="Footer"}),U.current.updateMessage(A.current.answer)}catch(N){}},[Y]),be=(0,r.useCallback)(function(N){d.flushSync(function(){U.current.removeMessage(N)});var Z=U.current.getMessages();ye(Z)},[Y]),de=Y.enable_thinking,De=(0,r.useState)([]),xe=a()(De,2),Te=xe[0],Oe=xe[1];return(0,j.jsxs)("div",{style:{height:"100vh"},children:[(0,j.jsx)(Q,{}),(0,j.jsx)(L.ZP,{cardConfig:{MyCard:ue},uiConfig:{welcome:(0,j.jsx)(w.Z,{style:{marginTop:"calc(50vh - 128px)"},title:"Nice to meet you!",desc:"How can I help you today?"}),bubbleList:{pagination:!0},disclaimer:"AI can also make mistakes, so please check and use it carefully",header:(0,j.jsx)(J.default,{}),logo:(0,j.jsx)("div",{style:{fontFamily:"Montserrat",fontWeight:"bold"},children:"Spark Chat"}),quickInput:(0,j.jsxs)(j.Fragment,{children:[(0,j.jsx)(E.Z,{size:"small",onClick:function(){d.flushSync(function(){U.current.updateMessage({id:(0,I.Z)(),cards:[{code:"MyCard",data:[["MyCard",1e4]]}],role:"assistant",msgStatus:"finished"})}),U.current.scrollToBottom()},children:"quickInput"}),(0,j.jsx)(E.Z,{size:"small",onClick:function(){},children:"test case A"}),(0,j.jsx)(E.Z,{size:"small",children:"test case B"})]})},ref:U,onInput:{morePrefixActions:(0,j.jsx)(j.Fragment,{children:de!==void 0?(0,j.jsx)(E.Z,{type:"text",color:de?"primary":"default",variant:de?"filled":"text",icon:(0,j.jsx)(V.Z,{}),style:{padding:"0 6px",gap:6},onClick:function(){return pe({enable_thinking:!de})},children:"DeepThink"}):null}),onSubmit:Ee,maxLength:1e5,beforeSubmit:function(){return Promise.resolve(!0)},suggestions:[{label:"Draw a picture",value:"draw"}]},onStop:ke,onRegenerate:be,onSessionKeyChange:function(Z){A.current.answer=void 0,A.current.query=void 0,q.default.set({currentSessionKey:Z})},onUpload:[{multiple:!1,icon:(0,j.jsx)(W.Z,{}),customRequest:function(Z){Z.onSuccess({url:URL.createObjectURL(Z.file)})}},{multiple:!1,icon:(0,j.jsx)(W.Z,{}),customRequest:function(Z){Z.onSuccess({url:URL.createObjectURL(Z.file)})}}]})]})}function ue(U){var ie=(0,B.TQ)(function(X){return X.onInput}),Y=(0,r.useState)(""),ae=a()(Y,2),pe=ae[0],A=ae[1];return(0,j.jsx)("div",{children:(0,j.jsxs)(x.Z,{title:"my card",size:"small",children:[(0,j.jsx)(y.Z,{gap:32,children:U.data.map(function(X){return(0,j.jsx)(m.Z,{title:X[0],value:X[1]},X[0])})}),(0,j.jsx)("br",{}),(0,j.jsxs)(y.Z,{gap:8,children:[(0,j.jsx)(k.Z,{placeholder:"\u8BF7\u8F93\u5165",onChange:function(me){return A(me.target.value)}})," ",(0,j.jsx)(E.Z,{onClick:function(){ie.onSubmit({query:pe})},children:"\u63D0\u4EA4"})]})]})})}},99387:function(h,n,e){e.r(n),e.d(n,{DemoContext:function(){return t},DemoContextProvider:function(){return u},useDemoContext:function(){return l}});var P=e(97857),a=e.n(P),v=e(5574),c=e.n(v),o=e(67294),i=e(79785),s=e(85893),t=o.createContext(void 0),u=function(b){var f=(0,i.Z)({model:"qwen-turbo",apikey:localStorage.getItem("apikey")||"",theme:localStorage.getItem("theme")||"carbon",enable_thinking:void 0}),p=c()(f,3),r=p[0],d=p[1],x=p[2];return(0,s.jsx)(t.Provider,{value:{demoContext:r,getDemoContext:x,setDemoContext:function(m){return d(function(k){var E=a()(a()({},k),m);return E.model==="qwen3-32b"&&E.enable_thinking===void 0&&(E.enable_thinking=!0),E.model!=="qwen3-32b"&&(E.enable_thinking=void 0),localStorage.setItem("apikey",E.apikey),localStorage.setItem("theme",E.theme),E})}},children:b.children})},l=function(){return o.useContext(t)}},82512:function(h,n,e){e.r(n),e.d(n,{default:function(){return l}});var P=e(67294),a=e(77577),v=e(85418),c=e(83622),o=e(99387),i=e(9838),s=e(85893),t=[{key:"qwen-turbo",label:(0,s.jsx)(u,{name:"Qwen-Turbo"})},{key:"qwen-max",label:(0,s.jsx)(u,{name:"Qwen-Max"})},{key:"qwq-plus",label:(0,s.jsx)(u,{name:"Qwen-QwQ-Plus"})},{key:"qwen3-32b",label:(0,s.jsx)(u,{name:"Qwen3-32B"})}];function u(g){var b=g.name,f=b===void 0?"":b,p=g.size,r=p===void 0?12:p,d=g.arrow,x=d===void 0?!1:d,y=(0,a.default)(),m=y.styles;return(0,s.jsxs)("div",{className:m.model,children:[(0,s.jsx)("img",{src:"https://img.alicdn.com/imgextra/i4/O1CN01SdyYTH1Kfz6skC36o_!!6000000001192-2-tps-96-96.png",className:m.modelLogo}),(0,s.jsx)("div",{className:m.modelName,style:{fontSize:r},children:f}),x&&(0,s.jsx)(i.Z,{})]})}function l(){var g=(0,o.useDemoContext)(),b=g.demoContext,f=g.setDemoContext,p=t.find(function(r){return r.key===b.model});return(0,s.jsx)(v.Z,{menu:{items:t,selectable:!0,onSelect:function(d){var x=d.key;f({model:x})}},children:(0,s.jsx)(c.ZP,{type:"text",style:{padding:"0 6px"},children:p!=null&&p.label?P.cloneElement(p.label,{size:16,arrow:!0}):null})})}},33588:function(h,n,e){e.r(n),e.d(n,{default:function(){return d}});var P=e(15009),a=e.n(P),v=e(99289),c=e.n(v),o=e(5574),i=e.n(o),s=e(67294),t=e(9966),u=e(64057),l=e(45286),g=e(21403),b=e(31912),f=e(99387),p=e(87697),r=e(85893);function d(){var y=t.Z.useForm(),m=i()(y,1),k=m[0],E=(0,s.useState)(!1),C=i()(E,2),D=C[0],M=C[1],S=(0,f.useDemoContext)(),T=S.getDemoContext,I=S.setDemoContext;return(0,s.useEffect)(function(){var O=function(){return M(!0)};return document.addEventListener("settingOpen",O),function(){document.removeEventListener("settingOpen",O)}},[]),(0,r.jsxs)(r.Fragment,{children:[(0,r.jsx)(u.Z,{icon:(0,r.jsx)(p.Z,{}),onClick:function(){return M(!0)}}),(0,r.jsx)(l.Z,{title:"Setting",afterOpenChange:function(){k.setFieldsValue({apikey:T().apikey})},onClose:function(){return M(!1)},open:D,footer:(0,r.jsx)(g.Z,{type:"primary",onClick:c()(a()().mark(function O(){var L;return a()().wrap(function(B){for(;;)switch(B.prev=B.next){case 0:return B.next=2,k.validateFields();case 2:L=B.sent,I({apikey:L.apikey}),M(!1);case 5:case"end":return B.stop()}},O)})),children:"Save"}),children:(0,r.jsxs)(t.Z,{form:k,children:[(0,r.jsx)(t.Z.Item,{name:"apikey",label:"API Key",rules:[{required:!0}],children:(0,r.jsx)(b.Z,{})}),(0,r.jsx)("a",{target:"_blank",href:"https://help.aliyun.com/zh/model-studio/developer-reference/get-api-key",children:"How to get apikey? "})]})})]})}function x(y){var m=y.value,k=y.onChange;return _jsxs(Flex,{gap:8,align:"center",children:[_jsx(Radio.Group,{options:[{label:"Carbon",value:"carbon"},{label:"Purple",value:"purple"}],value:m,onChange:function(C){return k(C.target.value)}}),_jsx(ColorPicker,{value:m,onChange:function(C){return k(C.toCssString())}})]})}},18081:function(h,n,e){e.r(n),e.d(n,{default:function(){return o}});var P=e(82512),a=e(33588),v=e(77577),c=e(85893);function o(){var i=(0,v.default)(),s=i.styles;return(0,c.jsxs)("div",{className:s.header,children:[(0,c.jsx)(P.default,{}),(0,c.jsx)("div",{style:{flex:1}}),(0,c.jsx)(a.default,{})]})}},77577:function(h,n,e){e.r(n);var P=e(68400),a=e.n(P),v=e(9053),c,o,i,s;n.default=(0,v.kc)(function(t){var u=t.css,l=t.token;return{header:u(c||(c=a()([`
    height: 100%;
    display: flex;
    align-items: center;
    justify-content: space-between;
    padding: 0 12px 0 12px;
    width: 100%;
  `]))),model:u(o||(o=a()([`
    height: 20px;
    display: flex;
    align-items: center;
    gap: 8px;
  `]))),modelLogo:u(i||(i=a()([`
    width: 20px;
    height: 20px;
    background-color: `,`;
    border-radius: 6px;
  `])),l.colorBgBase),modelName:u(s||(s=a()([`
    font-weight: 500;
    line-height: 20px;
    color: `,`;
  `])),l.colorText)}})},17738:function(h,n,e){e.r(n);var P=e(52677),a=e.n(P),v=e(97857),c=e.n(v),o={get:function(){var s=localStorage.getItem("chat-anywhere-session"),t;return s?t=JSON.parse(s):t={sessionList:[{label:Date.now().toString(),key:"9ed1150f-6046-4917-a124-6e85ba1ea933",messages:[[]]}],currentSessionKey:"9ed1150f-6046-4917-a124-6e85ba1ea933",currentRegenerateIndex:0},t.sessionList=(t.sessionList||[]).map(function(u){return c()(c()({},u),{},{messages:(u.messages||[]).map(function(l){return(l||[]).map(function(g){return!g||a()(g)!=="object"?g:c()(c()({},g),{},{history:!0})})})})}),t},set:function(s){var t=o.get()||{};localStorage.setItem("chat-anywhere-session",JSON.stringify(c()(c()({},t),s)))}};n.default=o},9212:function(h,n,e){e.r(n);var P=e(9783),a=e.n(P),v=e(5574),c=e.n(v),o=e(67294),i=e(64057),s=e(27646),t=e(16500),u=e(93967),l=e.n(u),g=e(13888),b=e(85893),f=function(r){var d,x=r.title,y=r.startTime,m=r.data,k=r.thumbnail,E=r.finished,C=r.onFullscreen,D=r.className,M=(0,o.useState)(E?(d=m.result)===null||d===void 0?void 0:d.endTime:Date.now()),S=c()(M,2),T=S[0],I=S[1];(0,o.useEffect)(function(){var L;return E?I(m.result.endTime):L=setInterval(function(){I(Date.now())},1e3),function(){clearInterval(L)}},[E]);var O=(0,t.formatDuration)(y,T);return(0,b.jsxs)("div",{className:l()(s.default.container,D,a()({},s.default.thumbnail,k)),children:[(0,b.jsxs)("div",{className:s.default.leftContent,children:[(0,b.jsx)(g.Z,{style:{fontSize:20}}),(0,b.jsx)("span",{className:s.default.title,children:x})]}),(0,b.jsxs)("div",{className:s.default.rightContent,children:[(0,b.jsx)("span",{className:s.default.duration,children:O}),(0,b.jsx)(i.Z,{className:s.default.fullscreenBtn,iconType:"spark-fullscreen-line",onClick:C,bordered:!1})]})]})};n.default=f},14358:function(h,n,e){e.r(n);var P=e(67294),a=e(26831),v=e(78499),c=e(85893),o=function(s){var t=s.data,u=s.activeTab,l=s.onTabChange,g=(0,P.useMemo)(function(){var p,r,d=[];return(p=t.result)!==null&&p!==void 0&&p.content&&d.push({key:"report",label:"Report"}),Array.isArray(t.steps)&&t.steps.length>0&&d.push({key:"process",label:"Process"}),Array.isArray((r=t.site)===null||r===void 0?void 0:r.list)&&t.site.list.length>0&&d.push({key:"source",label:"Source"}),d},[t]),b=function(r){l==null||l(r)},f=function(){var r;(r=t.result)!==null&&r!==void 0&&r.fileKey};return(0,c.jsx)("div",{className:v.default.tabsContainer,children:(0,c.jsx)(a.Z,{type:"segmented",items:g,activeKey:u,onChange:b})})};n.default=o},95639:function(h,n,e){e.r(n),e.d(n,{default:function(){return b}});var P=e(5574),a=e.n(P),v=e(67294),c=e(14358),o={container:"ufws3oMoFmALwjJ8M1W8",content:"RdbCkwzismA_YLaMsq_A"},i=e(14074),s=e(61889),t=e(78328),u=e(33331),l=e(85893),g=function(p){var r,d=p.data,x=p.viewIndex,y=(0,v.useState)("process"),m=a()(y,2),k=m[0],E=m[1];return(0,l.jsx)(u.default,{header:(0,l.jsx)(u.RightPanelHeader,{title:d.title,actions:null}),content:(0,l.jsxs)(u.RightPanelContent,{children:[(0,l.jsx)(c.default,{data:d,activeTab:k,onTabChange:E}),(0,l.jsxs)("div",{className:o.content,children:[k==="report"&&(0,l.jsx)(i.default,{data:d.result}),k==="process"&&(0,l.jsx)(s.default,{list:d.steps,viewIndex:x}),k==="source"&&(0,l.jsx)(t.default,{list:(r=d.site)===null||r===void 0?void 0:r.list})]})]})})},b=g},61889:function(h,n,e){e.r(n),e.d(n,{default:function(){return u}});var P=e(67294),a=e(66109),v={container:"tB7sEO03UwE8oAkSAVyK",siteListTitle:"PhWS9JaVX0wBTcMZVv8D",content:"MA_GYuDB0LM190oODwg4"},c=e(43050),o=e(93967),i=e.n(o),s=e(85893),t=function(g){var b=g.list,f=(0,P.useRef)([]);return(0,P.useEffect)(function(){},[g.viewIndex]),(0,s.jsx)("div",{className:v.container,children:b.map(function(p,r){var d;return(0,s.jsx)("div",{ref:function(y){return f.current[r]=y},children:(d=p.list)===null||d===void 0?void 0:d.map(function(x){return(0,s.jsxs)(s.Fragment,{children:[(0,s.jsx)(a.Z,{content:x.content,className:v.content}),Array.isArray(x.siteList)&&x.siteList.length>0&&(0,s.jsxs)("div",{className:i()(v.siteList),children:[(0,s.jsx)("div",{className:v.siteListTitle,children:"Research websites"}),(0,s.jsx)(c.default,{list:x.siteList})]})]})})},r)})})},u=t},78328:function(h,n,e){e.r(n),e.d(n,{default:function(){return i}});var P=e(67294),a={container:"X5NIjNOwr4ku1R6PDsXs",title:"iVErpSCwzrcpZoIOWfdG",content:"GgNo9xarYR2oufvNIqcU"},v=e(37050),c=e(85893),o=function(t){var u=t.list;return(0,c.jsxs)("div",{className:a.container,children:[(0,c.jsxs)("div",{className:a.title,children:["Searched ",u.length," web pages"]}),(0,c.jsx)("div",{className:a.content,children:(0,c.jsx)(v.default,{list:u})})]})},i=o},14074:function(h,n,e){e.r(n),e.d(n,{default:function(){return i}});var P=e(67294),a=e(66109),v={container:"G0FucIJl_SM6O1PhP4G4"},c=e(85893),o=function(t){var u=t.data;return(0,c.jsx)(a.Z,{content:u.content,className:v.container})},i=o},2126:function(h,n,e){e.r(n);var P=e(67294),a=e(71460),v=e(62142),c=e(83202),o=e(85893),i=function(t){var u=t.count,l=t.onClick;return(0,o.jsxs)("div",{className:a.default.moreWrapper,onClick:l,children:[(0,o.jsxs)("div",{className:a.default.more,children:[(0,o.jsx)(v.Z,{className:a.default.linkIcon}),(0,o.jsxs)("span",{className:a.default.title,children:["Read More (",u,")"]})]}),(0,o.jsx)(c.Z,{className:a.default.rightIcon})]})};n.default=i},70260:function(h,n,e){e.r(n);var P=e(67294),a=e(71460),v=e(93967),c=e.n(v),o=e(85893),i=function(t){var u=t.data,l=t.type,g=l===void 0?"simple":l;return(0,o.jsx)("div",{className:c()(a.default.urlWrapper,a.default[g]),onClick:function(){u.url&&window.open(u.url,"_blank")},children:(0,o.jsxs)("div",{className:a.default.url,children:[(0,o.jsx)("div",{className:a.default.icon,children:u.icon&&(0,o.jsx)("img",{src:u.icon,className:a.default.iconImage,alt:""})}),(0,o.jsx)("span",{className:a.default.title,children:u.title}),(0,o.jsx)("span",{className:a.default.description,children:u.description})]})})};n.default=i},37050:function(h,n,e){e.r(n);var P=e(67294),a=e(39914),v=e(70260),c=e(85893),o=function(s){var t=s.list;return(0,c.jsx)("div",{className:a.default.container,children:(0,c.jsx)("div",{className:a.default.content,children:t.map(function(u){return(0,c.jsx)(v.default,{data:u,type:"detail"},u.title)})})})};n.default=o},43050:function(h,n,e){e.r(n),e.d(n,{default:function(){return g}});var P=e(5574),a=e.n(P),v=e(67294),c={container:"tXJ3yFZ1XkEW4iZD78eO",content:"ycDUiLEwUBAhwl0lbZeH",collapseButton:"yq2PG6sfnv4SUsDe65CY"},o=e(70260),i=e(2126),s=e(21403),t=e(28387),u=e(85893),l=function(f){var p=f.list,r=(0,v.useState)(!1),d=a()(r,2),x=d[0],y=d[1],m=(0,v.useMemo)(function(){return p.length<=8||x?[p,0]:[p.slice(0,7),p.length-7]},[p,x]),k=a()(m,2),E=k[0],C=k[1];return(0,u.jsxs)("div",{className:c.container,children:[(0,u.jsxs)("div",{className:c.content,children:[E.map(function(D){return(0,u.jsx)(o.default,{data:D,type:"simple"},D.title)}),C>0&&(0,u.jsx)(i.default,{count:C,onClick:function(){return y(!0)}})]}),x&&(0,u.jsx)(s.Z,{size:"small",type:"textCompact",icon:(0,u.jsx)(t.Z,{}),onClick:function(){return y(!1)},className:c.collapseButton,children:"Collapse"})]})},g=l},2196:function(h,n,e){e.r(n),e.d(n,{default:function(){return u}});var P=e(67294),a=e(42119),v={loading:"bA12hcXTs8DqYrdpnbwA","sps-steps-loading":"AjbdrZYgAbrKvf8lM8QU",stepsContainer:"BN21aDAF7Fwl9lu9HnaH"},c=e(67055),o=e(40443),i=e(77900),s=e(85893),t=function(g){var b=g.list,f=b===void 0?[]:b,p=g.onStepClick;return(0,s.jsx)("div",{className:v.stepsContainer,children:(0,s.jsx)(a.Z,{direction:"vertical",size:"small",items:f.map(function(r,d){return{title:(0,s.jsx)(c.Z,{title:r.title,children:r.title}),status:r.status==="RUNNING"?"process":r.status==="FINISHED"?"finish":"wait",icon:r.status==="RUNNING"?(0,s.jsx)(o.Z,{className:v.loading}):(0,s.jsx)(i.Z,{}),onClick:function(){}}})})})},u=t},10447:function(h,n,e){e.r(n),e.d(n,{default:function(){return C}});var P=e(9783),a=e.n(P),v=e(97857),c=e.n(v),o=e(13769),i=e.n(o),s=e(5574),t=e.n(s),u=e(67294),l={container:"XIeL2YwmWL4MmHOuQtuO",content:"dXDQeobpRzke1xJiHOpH",list:"NKS46qSwz2kQWN4OctVw",view:"Wosm6Mld9f8qG2uv2tpP",time:"CkOhOZ1WApMC6YYmxxkm",thumbnail:"xIBcDW1Rw_sF7rh569Av",finished:"OS2saoCRFdggfVfQEzz_",header:"TJff9gIqUQpyjdFY098D"},g=e(9212),b=e(2196),f=e(61889),p=e(93967),r=e.n(p),d=e(95639),x=e(73935),y=e(23630),m=e(85893),k=["steps","result"],E=function(M){var S,T=(0,y.default)(),I=t()(T,2),O=I[0],L=I[1],w=(0,u.useState)(0),B=t()(w,2),ne=B[0],V=B[1],W=(0,u.useMemo)(function(){return Date.now()},[]),J=(0,u.useMemo)(function(){var K,j=((K=M.data)===null||K===void 0?void 0:K.data)||{},H=j.steps,Q=H===void 0?[]:H,z=j.result,ue=i()(j,k);return c()({steps:Q.map(function(U){return c()(c()({},U),{},{startTime:parseInt(U.startTime||"0"),endTime:parseInt(U.endTime||"0")})}),result:c()(c()({},z),{},{startTime:parseInt((z==null?void 0:z.startTime)||"0"),endTime:parseInt((z==null?void 0:z.endTime)||"0")})},ue)},[(S=M.data)===null||S===void 0?void 0:S.data]),q=(0,u.useMemo)(function(){var K=Math.min(J.steps.reduce(function(j,H){return Math.min(j,H.startTime)},Number.MAX_SAFE_INTEGER),J.result.startTime||Number.MAX_SAFE_INTEGER);return K===Number.MAX_SAFE_INTEGER?W:K},[J.steps,J.result,W]),te=(0,u.useMemo)(function(){return M.data.msgStatus==="finished"},[M.data.msgStatus]);return(0,m.jsxs)("div",{className:r()(l.container,a()({},l.thumbnail,O)),children:[(0,m.jsx)(g.default,{className:l.header,title:J.title,startTime:q,data:J,finished:te,thumbnail:O,onFullscreen:function(){return L(!0)}}),(0,m.jsxs)("div",{className:l.content,children:[(0,m.jsx)("div",{className:l.list,children:(0,m.jsx)(b.default,{list:J.steps,onStepClick:V})}),(0,m.jsx)("div",{className:l.view,children:(0,m.jsx)(f.default,{list:J.steps,viewIndex:ne})})]}),O&&x.createPortal((0,m.jsx)(d.default,{data:J}),document.getElementById("right-panel"))]})},C=E},16500:function(h,n,e){e.r(n),e.d(n,{formatDuration:function(){return P}});var P=function(v,c){var o=Math.floor((c-v)/1e3),i=Math.floor(o/3600),s=Math.floor(o%3600/60),t=o%60,u=s.toString().padStart(2,"0"),l=t.toString().padStart(2,"0");return i>0||s>0?"".concat(u," minutes ").concat(l," seconds"):"".concat(l," seconds")}},23681:function(h,n,e){e.r(n),e.d(n,{default:function(){return s}});var P=e(60227),a=e(89698),v=e(36855),c=e(28373),o=e(67369),i=e(85893);function s(){return(0,i.jsx)(o.p4,{data:[{icon:(0,i.jsx)(P.Z,{}),label:"",onClick:function(){}},{icon:(0,i.jsx)(a.Z,{}),label:"",onClick:function(){}},{icon:(0,i.jsx)(v.Z,{}),label:"",onClick:function(){}},{icon:(0,i.jsx)(c.Z,{}),label:"",onClick:function(){}}]})}},48911:function(h,n,e){e.r(n),e.d(n,{default:function(){return r}});var P=e(5574),a=e.n(P),v=e(8894),c=e(88584),o=e(36231),i=e(88180),s=e(4146),t=e(64057),u=e(21403),l=e(67294),g=e(73935),b=e(33331),f=e(23630),p=e(85893);function r(x){var y=(0,f.default)(),m=a()(y,2),k=m[0],E=m[1],C=x.data,D=(0,i.default)(),M=D.styles,S=(0,p.jsx)(s.ZP,{value:C.content,language:"html",readOnly:!0});return(0,p.jsxs)("div",{className:M.webDev,children:[(0,p.jsxs)("div",{className:M.header,children:[(0,p.jsxs)("div",{className:M.left,children:[(0,p.jsx)(v.Z,{style:{fontSize:20}}),(0,p.jsx)("span",{className:M.title,children:C.title})]}),k?null:(0,p.jsx)(t.Z,{onClick:function(){return E(!k)},bordered:!1,icon:(0,p.jsx)(c.Z,{})})]}),k?g.createPortal((0,p.jsx)(d,{code:S,title:C.title,loading:C.loading,content:C.content}),document.getElementById("right-panel")):(0,p.jsx)("div",{className:M.content,children:S})]})}function d(x){var y=x.code,m=x.title,k=x.loading,E=x.content,C=(0,l.useState)(!1),D=a()(C,2),M=D[0],S=D[1],T=(0,i.default)(),I=T.styles,O=M?(0,p.jsx)("iframe",{className:I.iframe,srcDoc:E}):y;return(0,p.jsx)(b.default,{header:(0,p.jsx)(b.RightPanelHeader,{title:m,actions:(0,p.jsx)(u.Z,{icon:(0,p.jsx)(o.Z,{}),type:"primaryLess",loading:k,onClick:function(){k||S(!M)},children:M?"Exit":"Preview"})}),content:(0,p.jsx)(b.RightPanelContent,{children:O})})}},88180:function(h,n,e){e.r(n);var P=e(9053);n.default=(0,P.kc)(function(a){var v=a.css,c=a.token;return{webDev:{width:"100%",backgroundColor:c.colorBgBase,border:"1px solid ".concat(c.colorBorderSecondary),borderRadius:c.borderRadius,overflow:"hidden"},header:{display:"flex",justifyContent:"space-between",alignItems:"center",height:48,padding:"0 16px"},left:{display:"flex",alignItems:"center"},title:{fontFamily:"claude clone",fontSize:14,marginLeft:8,fontWeight:600},content:{},iframe:{border:0,display:"block",width:"100%",height:"100%"}}})},35922:function(h,n,e){e.r(n),n.default="data:application/octet-stream;base64,"},77333:function(h,n,e){e.r(n),e.d(n,{default:function(){return S}});var P=e(15009),a=e.n(P),v=e(99289),c=e.n(v),o=e(88773),i=e(29041),s=e(67294),t=e(81644),u=e(73935),l=e(47103),g=e(48775),b=e(22059),f=e(95233),p=e(23089),r=e(7715),d=e(93513),x=e(13021),y=e(48911),m=e(10447),k=e(9053),E=(0,k.kc)(function(I){var O=I.css,L=I.token;return{container:{display:"flex",height:"100%",background:L.colorBgBase},chat:{height:"100%",transition:"width .3s ease-out"}}}),C=e(33331),D=e(23681),M=e(85893);function S(){var I=E(),O=I.styles,L=(0,s.useContext)(r.DemoContext),w=L.demoContext,B=L.setDemoContext,ne=L.getDemoContext,V=(0,s.useRef)(),W=(0,s.useRef)({}),J=(0,s.useCallback)(function(){var K=c()(a()().mark(function j(H){return a()().wrap(function(z){for(;;)switch(z.prev=z.next){case 0:return V.current.setLoading(!0),B({initial:!1}),W.current.answer=void 0,W.current.query={id:(0,o.Z)(),cards:[{code:"Text",data:{content:H.query,msgStatus:"finished"}}],role:"user",msgStatus:"finished"},u.flushSync(function(){V.current.updateMessage(W.current.query)}),W.current.answer={id:(0,o.Z)(),role:"assistant",content:"",cards:[],msgStatus:"generating"},V.current.updateMessage(W.current.answer),z.next=9,(0,x.default)({getDemoContext:ne,currentQA:W,ref:V});case 9:return z.next=11,T(100);case 11:q("finished"),V.current.setLoading(!1);case 13:case"end":return z.stop()}},j)}));return function(j){return K.apply(this,arguments)}}(),[]),q=(0,s.useCallback)(function(){var K=arguments.length>0&&arguments[0]!==void 0?arguments[0]:"finished",j=V.current.getMessages().find(function(Q){return Q.id===W.current.answer.id}),H=(0,d.produce)(j,function(Q){Q.msgStatus=K,(Q.cards||[]).forEach(function(z){z.data.msgStatus=K,z.data.loading=!1}),Q.cards.push({code:"Footer",data:{left:(0,M.jsx)(D.default,{})}})});V.current.updateMessage(H)},[]),te=(0,s.useCallback)(function(){},[]);return(0,M.jsxs)("div",{className:O.container,children:[(0,M.jsx)("div",{className:O.chat,style:{width:w.canvas?480:"100%"},children:(0,M.jsx)(i.ZP,{cardConfig:{WebDev:y.default,DeepResearch:m.default},uiConfig:{disclaimer:"AI can also make mistakes, so please check carefully and use it with caution",header:(0,M.jsx)(l.default,{chat:V}),welcome:(0,M.jsx)(g.default,{})},ref:V,onInput:{zoomable:!w.initial,morePrefixActions:(0,M.jsx)(f.default,{}),beforeUI:(0,M.jsx)(b.default,{}),afterUI:(0,M.jsx)(p.default,{onSubmit:J}),placeholder:"please type here",onSubmit:J},onStop:te,onUpload:[{multiple:!1,icon:(0,M.jsx)(t.Z,{}),customRequest:function(j){j.onSuccess({url:URL.createObjectURL(j.file)})}}]})}),(0,M.jsx)(C.RightPanelPortal,{})]})}function T(I){return new Promise(function(O){return setTimeout(O,I)})}},47103:function(h,n,e){e.r(n),e.d(n,{default:function(){return u}});var P=e(2350),a=e(31079),v=e(64057),c=e(67294),o=e(18426),i=e(49331),s=e(86250),t=e(85893);function u(l){var g=(0,P.default)(),b=g.styles,f=(0,c.useContext)(i.ConfigProviderContext),p=f.dark,r=f.setDark;return(0,t.jsxs)("div",{className:b.header,children:[(0,t.jsx)(a.default,{size:"small"}),(0,t.jsx)(s.Z,{gap:8,children:(0,t.jsx)(v.Z,{bordered:!1,onClick:function(){return l.chat.current.removeAllMessages()},icon:(0,t.jsx)(o.Z,{})})})]})}},2350:function(h,n,e){e.r(n);var P=e(68400),a=e.n(P),v=e(9053),c;n.default=(0,v.kc)(function(o){var i=o.css,s=o.token;return{header:i(c||(c=a()([`
    width: 100%;
    height: 64px;
    padding: 0 24px 0 24px;
    display: flex;
    align-items: center;
    justify-content: space-between;
  `])))}})},22059:function(h,n,e){e.r(n),e.d(n,{default:function(){return s}});var P=e(8894),a=e(13888),v=e(66672),c=e(67294),o=e(7715),i=e(85893);function s(){var t=(0,c.useContext)(o.DemoContext),u=t.demoContext,l=t.setDemoContext;return u.initial&&!u.mode?null:(0,i.jsx)(v.Z.ModeSelect,{options:[{icon:(0,i.jsx)(P.Z,{}),label:"Web Development",value:"WebDev"},{icon:(0,i.jsx)(a.Z,{}),label:"Deep Research",value:"DeepResearch"}],value:u.mode,onChange:function(b){return l({mode:b})}})}},31079:function(h,n,e){e.r(n),e.d(n,{default:function(){return b}});var P=e(97857),a=e.n(P),v=e(77512),c=e(67294),o=e(85136),i=e(9838),s=e(97634),t=e(7715),u=e(85893),l="https://gw.alicdn.com/imgextra/i1/O1CN01qYTgPn1Oaj6j7oc14_!!6000000001722-55-tps-24-25.svg",g=[{value:"claude-3.5-sonnet",label:"Claude 3.5 Sonnet"},{value:"claude-4.1-Opus",label:"Claude 4.1 Opus"}];function b(p){var r=p.size,d=r===void 0?"large":r,x=(0,o.default)(),y=x.styles,m=x.cx,k=(0,c.useContext)(t.DemoContext),E=k.demoContext,C=k.setDemoContext,D=g.find(function(M){return M.value===E.model});return(0,u.jsx)("div",{children:(0,u.jsx)(v.Z,{menu:{items:g.map(function(M){return{key:M.value,label:(0,u.jsx)(f,a()(a()({},M),{},{onClick:function(){C({model:M.value})},selected:E.model===M.value}))}})},children:(0,u.jsxs)("div",{className:m(y.display,y[d]),children:[(0,u.jsx)("img",{src:l}),D==null?void 0:D.label,(0,u.jsx)(i.Z,{})]})})})}function f(p){var r=(0,o.default)(),d=r.styles;return(0,u.jsxs)("div",{onClick:function(){return p.onClick()},className:d.item,children:[(0,u.jsx)("div",{className:d.itemIcon,children:(0,u.jsx)("img",{src:l,width:14,height:14})}),(0,u.jsx)("div",{className:d.itemTitle,children:p.label}),p.selected&&(0,u.jsx)(s.Z,{})]})}},85136:function(h,n,e){e.r(n);var P=e(9053);n.default=(0,P.kc)(function(a){var v=a.css,c=a.token;return{display:{fontFamily:"claude clone",display:"flex",alignItems:"center",gap:8,cursor:"pointer",color:c.colorText},small:{fontSize:14,height:20,img:{width:20,height:20,padding:2.5,background:c.colorPrimaryBg,borderRadius:4}},large:{fontSize:20,height:40,img:{width:24,height:24}},item:{display:"flex",alignItems:"center",gap:8},itemIcon:{width:20,height:20,display:"flex",alignItems:"center",justifyContent:"center",borderRadius:4},itemTitle:{fontFamily:"claude clone"}}})},95233:function(h,n,e){e.r(n),e.d(n,{default:function(){return b}});var P=e(97857),a=e.n(P),v=e(9783),c=e.n(v),o=e(21403),i=e(7715),s=e(67294),t=e(98839),u=e(29986),l=e(9053),g=e(85893);function b(){var r=(0,s.useContext)(i.DemoContext),d=r.demoContext,x=r.setDemoContext;return(0,g.jsxs)(g.Fragment,{children:[(0,g.jsx)(f,{icon:(0,g.jsx)(t.Z,{}),selected:d.enableThinking,onClick:function(){return x({enableThinking:!d.enableThinking})},children:"Deep Thinking"}),(0,g.jsx)(f,{selected:d.enableSearch,icon:(0,g.jsx)(u.Z,{}),onClick:function(){return x({enableSearch:!d.enableSearch})},children:"Web Search"})]})}function f(r){var d=p(),x=d.styles,y=d.cx;return(0,g.jsx)(o.Z,a()(a()({},r),{},{className:y(x.btn,c()({},x.selected,r.selected))}))}var p=(0,l.kc)(function(r){var d=r.token;return{btn:{gap:6},selected:{color:"".concat(d.colorPrimary," !important"),backgroundColor:"".concat(d.colorPrimaryBg," !important"),borderColor:"".concat(d.colorPrimaryBg," !important")}}})},93822:function(h,n,e){e.r(n),e.d(n,{default:function(){return p}});var P=e(68400),a=e.n(P),v=e(39146),c=e(9053),o=e(85893),i,s,t,u,l,g,b,f=[{prompt:"deep research on the topic of city pop"},{prompt:"deep research on the topic of city pop"},{prompt:"deep research on the topic of city pop"}];function p(d){var x=r(),y=x.styles;return(0,o.jsx)("div",{className:y.preset,children:f.map(function(m){return(0,o.jsxs)("div",{className:y.presetItem,onClick:function(){return d.onSubmit({query:m.prompt})},children:[(0,o.jsxs)("div",{className:y.children,children:[(0,o.jsx)("img",{width:24,height:24,src:"https://gw.alicdn.com/imgextra/i1/O1CN01K7jgEj1sywWTkPSGY_!!6000000005836-55-tps-40-40.svg"}),(0,o.jsx)("div",{className:y.prompt,children:m.prompt}),(0,o.jsxs)("div",{className:y.skeletonContainer,children:[(0,o.jsx)("div",{className:y.skeletonLine}),(0,o.jsx)("div",{className:y.skeletonLine}),(0,o.jsx)("div",{className:y.skeletonShortLine})]})]}),(0,o.jsxs)("div",{className:y.content,children:[(0,o.jsx)("div",{className:y.title,children:m.prompt}),(0,o.jsx)(v.Z,{})]})]},m.prompt)})})}var r=(0,c.kc)(function(d){var x=d.token,y=d.css,m=d.cx,k=m(y(i||(i=a()([`
    position: absolute;
    bottom: 0;
    left: 0;
    right: 0;
    background-color: `,`;
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 12px;
    height: 48px;
    padding: 0 12px;
    border-top: 1px solid `,`;
    opacity: 0;
    transition: all 0.3s ease;
  `])),x.colorBgBase,x.colorBorderSecondary));return{preset:{marginTop:32,display:"flex",justifyContent:"center",gap:12},presetItem:y(s||(s=a()([`
      position: relative;
      display: flex;
      border: 1px solid `,`;
      border-radius: 8px;
      height: 200px;
      flex: 1;
      cursor: pointer;
      overflow: hidden;
      padding: 24px;
      background-color: `,`;
      &:hover {
        border-color: `,`;

        .`,` {
          opacity: 1;
        }
      }
    `])),x.colorBorderSecondary,x.colorFillTertiary,x.colorPrimary,k),children:{display:"flex",flexDirection:"column",gap:10,height:"100%",width:"100%",backgroundColor:x.colorBgBase,borderRadius:8,padding:"20px 12px"},prompt:y(t||(t=a()([`
      font-size: 12px;
      line-height: 20px;
      font-weight: 500;
      color: `,`;
      text-overflow: ellipsis;
      overflow: hidden;
      display: -webkit-box;
      -webkit-line-clamp: 1;
      -webkit-box-orient: vertical;
    `])),x.colorText),content:k,title:y(u||(u=a()([`
      font-size: 14px;
      color: `,`;
      text-overflow: ellipsis;
      overflow: hidden;
      white-space: nowrap;
    `])),x.colorText),skeletonContainer:y(l||(l=a()([`
      display: flex;
      flex-direction: column;
      gap: 8px;
    `]))),skeletonLine:y(g||(g=a()([`
      width: 100%;
      height: 8px;
      background-color: `,`;
      border-radius: 99px;
    `])),x.colorFillTertiary),skeletonShortLine:y(b||(b=a()([`
      width: 50%;
      height: 8px;
      background-color: `,`;
      border-radius: 99px;
    `])),x.colorFillTertiary)}})},50571:function(h,n,e){e.r(n),e.d(n,{default:function(){return l}});var P=e(68400),a=e.n(P),v=e(39146),c=e(9053),o=e(85893),i,s,t,u=[{prompt:"write a landing page for a fruit shop",image:"https://gw.alicdn.com/imgextra/i1/O1CN01ObcQyY1vyQveMjl76_!!6000000006241-55-tps-390-200.svg"},{prompt:"write a portfolio website",image:"https://gw.alicdn.com/imgextra/i2/O1CN01NciUQw1tzgmhPLmfF_!!6000000005973-55-tps-390-200.svg"}];function l(b){var f=g(),p=f.styles;return(0,o.jsx)("div",{className:p.preset,children:u.map(function(r){return(0,o.jsxs)("div",{className:p.presetItem,onClick:function(){return b.onSubmit({query:r.prompt})},children:[(0,o.jsx)("img",{src:r.image,alt:r.prompt}),(0,o.jsxs)("div",{className:p.content,children:[(0,o.jsx)("div",{className:p.title,children:r.prompt}),(0,o.jsx)(v.Z,{})]})]},r.prompt)})})}var g=(0,c.kc)(function(b){var f=b.token,p=b.css,r=b.cx,d=r(p(i||(i=a()([`
    position: absolute;
    bottom: 0;
    left: 0;
    right: 0;
    background-color: `,`;
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 12px;
    height: 48px;
    border-top: 1px solid `,`;
    opacity: 0;
    transition: all 0.3s ease;
  `])),f.colorBgBase,f.colorBorderSecondary));return{preset:{marginTop:32,display:"flex",justifyContent:"center",gap:12},presetItem:p(s||(s=a()([`
      position: relative;
      display: flex;
      border: 1px solid `,`;
      border-radius: 8px;
      cursor: pointer;
      overflow: hidden;
      &:hover {
        border-color: `,`;

        .`,` {
          opacity: 1;
        }
      }
    `])),f.colorBorderSecondary,f.colorPrimary,d),content:d,title:p(t||(t=a()([`
      font-size: 14px;
      color: `,`;
    `])),f.colorText)}})},23089:function(h,n,e){e.r(n),e.d(n,{default:function(){return f}});var P=e(67294),a=e(7715),v=e(8894),c=e(13888),o=e(19458),i=e(55841),s=e(21403),t=e(9053),u=e(50571),l=e(93822),g=e(85893),b=[{icon:(0,g.jsx)(v.Z,{}),label:"Web Development",value:"WebDev",type:"mode"},{icon:(0,g.jsx)(c.Z,{}),label:"Deep Research",value:"DeepResearch",type:"mode"},{icon:(0,g.jsx)(o.Z,{}),label:"Write Article",value:"Write Article",type:"quick"},{icon:(0,g.jsx)(i.Z,{}),label:"Brainstorm",value:"Brainstorm",type:"quick"}];function f(r){var d=(0,P.useContext)(a.DemoContext),x=d.demoContext,y=d.setDemoContext,m=p(),k=m.styles;if(!x.initial)return null;var E=x.mode;return E==="WebDev"?(0,g.jsx)(u.default,{onSubmit:r.onSubmit}):E==="DeepResearch"?(0,g.jsx)(l.default,{onSubmit:r.onSubmit}):(0,g.jsx)("div",{className:k.preset,children:b.map(function(C){return(0,g.jsx)("div",{children:(0,g.jsx)(s.Z,{icon:C.icon,onClick:function(){C.type==="mode"?y({mode:C.value}):r.onSubmit({query:C.value})},children:C.label})},C.value)})})}var p=(0,t.kc)(function(r){var d=r.token;return{preset:{marginTop:32,display:"flex",justifyContent:"center",gap:12}}})},33331:function(h,n,e){e.r(n),e.d(n,{RightPanelContent:function(){return u},RightPanelHeader:function(){return l},RightPanelPortal:function(){return b},default:function(){return t},useRightPanel:function(){return g}});var P=e(67294),a=e(9053),v=(0,a.kc)(function(f){var p=f.css,r=f.token;return{rightPanel:{display:"flex",flexDirection:"column",height:"100%",borderLeft:"1px solid ".concat(r.colorBorderSecondary)},header:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px",height:64,background:r.colorFillTertiary},headerLeft:{display:"flex",alignItems:"center",gap:8},headerRight:{display:"flex",alignItems:"center",gap:8},title:{fontFamily:"claude clone",fontSize:16,marginLeft:8,fontWeight:600},content:{flex:1,overflow:"auto"}}}),c=e(7715),o=e(64057),i=e(89102),s=e(85893);function t(f){var p=v(),r=p.styles,d=(0,P.useContext)(c.DemoContext),x=d.demoContext,y=d.setDemoContext,m=d.getDemoContext,k=f.header,E=f.content;return(0,P.useEffect)(function(){return y({canvas:!0}),function(){y({canvas:!1})}},[]),(0,s.jsxs)("div",{className:r.rightPanel,children:[k,E]})}var u=function(p){var r=v(),d=r.styles;return(0,s.jsx)("div",{className:d.content,children:p.children})},l=function(p){var r=(0,P.useContext)(c.DemoContext),d=r.demoContext,x=r.setDemoContext,y=r.getDemoContext,m=v(),k=m.styles;return(0,s.jsxs)("div",{className:k.header,children:[(0,s.jsxs)("div",{className:k.headerLeft,children:[(0,s.jsx)(o.Z,{bordered:!1,icon:(0,s.jsx)(i.Z,{}),onClick:function(){x({canvas:!1})}}),(0,s.jsx)("span",{className:k.title,children:p.title})]}),(0,s.jsx)("div",{className:k.headerRight,children:p.actions})]})},g=function(){},b=function(){return(0,s.jsx)("div",{id:"right-panel",style:{flex:1,width:0}})}},48775:function(h,n,e){e.r(n),e.d(n,{default:function(){return s}});var P=e(68400),a=e.n(P),v=e(9053),c=e(31079),o=e(85893),i;function s(){var l=t(),g=l.styles;return(0,o.jsxs)(o.Fragment,{children:[(0,o.jsx)(u,{}),(0,o.jsxs)("div",{className:g.welcome,children:[(0,o.jsx)("div",{className:g.h1,children:"Welcome to LLMs Experience"}),(0,o.jsxs)("div",{className:g.h2,children:["Start with ",(0,o.jsx)(c.default,{size:"large"})]})]})]})}var t=(0,v.kc)(function(l){var g=l.css,b=l.token;return{welcome:{display:"flex",flexDirection:"column",alignItems:"center",gap:4,marginTop:"calc(10vh + 80px + 64px)",marginBottom:20},h1:{fontFamily:"claude clone",fontSize:32,lineHeight:"48px",fontWeight:"500"},h2:{fontFamily:"claude clone",display:"flex",justifyContent:"center",alignItems:"center",gap:16,fontSize:20,lineHeight:"32px",color:b.colorTextSecondary}}}),u=(0,v.vJ)(i||(i=a()([`
.sps-chat-anywhere-chat-welcome {
  height: auto !important;  
}`])))},49331:function(h,n,e){e.r(n),e.d(n,{ConfigProviderContext:function(){return u},default:function(){return l}});var P=e(97857),a=e.n(P),v=e(5574),c=e.n(v),o=e(21766),i=e(36238),s=e(67294),t=e(85893),u=s.createContext(void 0);function l(g){var b=(0,s.useState)(!1),f=c()(b,2),p=f[0],r=f[1],d=p?i.darkTheme:i.lightTheme;return(0,t.jsx)(t.Fragment,{children:(0,t.jsx)(u.Provider,{value:{dark:p,setDark:r},children:(0,t.jsx)(o.ZP,a()(a()({},d),{},{style:{height:"100%"},iconfont:"https://at.alicdn.com/t/a/font_4807885_6wjcpzayws6.js",prefix:"sps",prefixCls:"sps",children:g.children}))})})}},7715:function(h,n,e){e.r(n),e.d(n,{DemoContext:function(){return t},default:function(){return u}});var P=e(97857),a=e.n(P),v=e(5574),c=e.n(v),o=e(79785),i=e(67294),s=e(85893),t=i.createContext(void 0);function u(l){var g=(0,o.Z)({model:"claude-3.5-sonnet",enableSearch:!1,enableThinking:!1,initial:!0,mode:void 0,canvas:!1}),b=c()(g,3),f=b[0],p=b[1],r=b[2];return(0,s.jsx)(t.Provider,{value:{demoContext:f,setDemoContext:function(x){p(function(y){return a()(a()({},y),x)})},getDemoContext:r},children:l.children})}},36238:function(h,n,e){e.r(n),e.d(n,{darkTheme:function(){return c},lightTheme:function(){return o}});var P=e(81759),a={borderRadiusXS:4,borderRadiusSM:6,borderRadius:8,borderRadiusLG:12,borderRadiusXL:16,borderRadiusFull:999,wireframe:!1,colorPrimaryBg:"rgba(239, 237, 229, 1)",colorPrimaryBgHover:"rgba(239, 237, 229, 1)",colorPrimaryBorder:"rgba(186, 90, 59, 1)",colorPrimaryBorderHover:"rgba(194, 98, 66, 1)",colorPrimaryHover:"rgba(194, 98, 66, 1)",colorPrimary:"rgba(208, 110, 78, 1)",colorPrimaryActive:"rgba(194, 98, 66, 1)",colorPrimaryTextHover:"rgba(186, 90, 59, 1)",colorPrimaryText:"rgba(186, 90, 59, 1)",colorPrimaryTextActive:"rgba(186, 90, 59, 1)",colorTextBase:"rgba(26, 26, 29, 1)",colorBgBase:"rgba(255, 255, 255, 1)",colorText:"rgba(20, 20, 19, 0.88)",colorTextSecondary:"rgba(20, 20, 19, 0.65)",colorTextTertiary:"rgba(20, 20, 19, 0.45)",colorTextQuaternary:"rgba(20, 20, 19, 0.25)",colorBorder:"rgba(193, 192, 187, 1)",colorBorderSecondary:"rgba(234, 233, 231, 1)",colorFill:"rgba(43, 27, 0, 0.12)",colorFillSecondary:"rgba(43, 27, 0, 0.08)",colorFillTertiary:"rgba(43, 27, 0, 0.03)",colorFillQuaternary:"rgba(43, 27, 0, 0.02)",colorBgContainer:"rgba(255, 255, 255, 1)",colorBgElevated:"rgba(255, 255, 255, 1)",colorBgLayout:"rgba(249, 248, 244, 1)",colorBgSpotlight:"rgba(20, 20, 19, 0.85)",colorBgMask:"rgba(0, 0, 0, 0.45)",colorInfo:"rgba(11, 131, 241, 1)",colorInfoHover:"rgba(94, 188, 255, 1)",colorInfoBorder:"rgba(135, 209, 255, 1)",colorInfoText:"rgba(11, 131, 241, 1)",colorInfoBg:"rgba(230, 247, 255, 1)",colorInfoBgHover:"rgba(176, 227, 255, 1)",colorInfoBorderHover:"rgba(94, 188, 255, 1)",colorSuccess:"rgba(91, 185, 139, 1)",colorSuccessHover:"rgba(171, 212, 190, 1)",colorSuccessBg:"rgba(235, 250, 240, 1)",colorSuccessBgHover:"rgba(223, 237, 228, 1)",colorSuccessBorder:"rgba(211, 224, 217, 1)",colorSuccessBorderHover:"rgba(171, 212, 190, 1)",colorWarning:"rgba(250, 173, 20, 1)",colorWarningHover:"rgba(255, 214, 102, 1)",colorWarningBg:"rgba(255, 251, 230, 1)",colorWarningBgHover:"rgba(255, 241, 184, 1)",colorWarningBorder:"rgba(255, 229, 143, 1)",colorWarningBorderHover:"rgba(255, 214, 102, 1)",colorError:"rgba(255, 77, 79, 1)",colorErrorHover:"rgba(255, 120, 117, 1)",colorErrorBg:"rgba(255, 242, 240, 1)",colorErrorBgHover:"rgba(255, 241, 240, 1)",colorErrorBorder:"rgba(255, 204, 199, 1)",colorErrorBorderHover:"rgba(255, 163, 158, 1)",colorLink:"rgba(0, 77, 255, 1)",boxShadow:"0px 4px 6px 0px rgba(0, 0, 0, 0.08)",boxShadowSecondary:"0px 12px 24px -16px rgba(0, 0, 0, 0.06), 0px 8px 40px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)",colorTextWhite:"rgba(255, 255, 255, 1)",colorFillDisable:"rgba(235, 235, 232, 1)",colorPurple:"rgba(97, 92, 237, 1)",colorPurpleHover:"rgba(97, 92, 237, 1)",colorPurpleBg:"rgba(226, 222, 255, 0.8)",colorPink:"rgba(236, 72, 153, 1)",colorPinkHover:"rgba(236, 72, 153, 1)",colorPinkBg:"rgba(251, 220, 239, 0.8)",colorYellow:"rgba(234, 179, 8, 1)",colorYellowHover:"rgba(234, 179, 8, 1)",colorYellowBg:"rgba(255, 250, 184, 0.8)",colorOrange:"rgba(250, 129, 37, 1)",colorOrangeHover:"rgba(250, 129, 37, 1)",colorOrangeBg:"rgba(255, 232, 201, 0.8)",colorTeal:"rgba(20, 184, 166, 1)",colorTealHover:"rgba(20, 184, 166, 1)",colorTealBg:"rgba(200, 244, 233, 0.8)",colorBlue:"rgba(0, 144, 255, 1)",colorBlueHover:"rgba(0, 144, 255, 1)",colorBlueBg:"rgba(194, 229, 255, 0.8)",colorMauve:"rgba(142, 140, 153, 1)",colorMauveHover:"rgba(142, 140, 153, 1)",colorMauveBg:"rgba(239, 240, 243, 0.8)",boxShadowTertiary:"0px 12px 32px -16px rgba(0, 0, 0, 0.24), 0px 12px 60px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)",boxShadowTertiaryLeft:"-12px 0px 32px -16px rgba(0, 0, 0, 0.24), -12px 0px 60px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)",boxShadowInput:"0px 12px 24px -16px rgba(54, 54, 73, 0.04), 0px 12px 40px 0px rgba(51, 51, 71, 0.08), 0px 0px 1px 0px rgba(44, 44, 54, 0.02)"},v={borderRadiusXS:4,borderRadiusSM:6,borderRadius:8,borderRadiusLG:12,borderRadiusXL:16,borderRadiusFull:999,wireframe:!1,colorPrimaryBg:"rgba(41, 35, 29, 1)",colorPrimaryBgHover:"rgba(70, 60, 50, 1)",colorPrimaryBorder:"rgba(133, 80, 40, 1)",colorPrimaryBorderHover:"rgba(146, 102, 52, 1)",colorPrimaryHover:"rgba(156, 88, 57, 1)",colorPrimary:"rgba(156, 88, 57, 1)",colorPrimaryActive:"rgba(146, 102, 52, 1)",colorPrimaryTextHover:"rgba(133, 80, 40, 1)",colorPrimaryText:"rgba(160, 96, 63, 1)",colorPrimaryTextActive:"rgba(133, 80, 40, 1)",colorTextBase:"rgba(230, 230, 230, 1)",colorBgBase:"rgba(15, 13, 12, 1)",colorText:"rgba(230, 230, 230, 0.9)",colorTextSecondary:"rgba(180, 180, 180, 0.8)",colorTextTertiary:"rgba(255, 255, 255, 0.6)",colorTextQuaternary:"rgba(255, 255, 255, 0.3)",colorBorder:"rgba(82, 76, 73, 1)",colorBorderSecondary:"rgba(50, 46, 43, 1)",colorFill:"rgba(82, 79, 74, 0.32)",colorFillSecondary:"rgba(83, 79, 72, 0.24)",colorFillTertiary:"rgba(50, 50, 50, 0.26)",colorFillQuaternary:"rgba(67, 64, 59, 0.12)",colorBgContainer:"rgba(52, 50, 47, 1)",colorBgElevated:"rgba(47, 42, 39, 1)",colorBgLayout:"rgba(26, 26, 24, 1)",colorBgSpotlight:"rgba(10, 10, 10, 0.85)",colorBgMask:"rgba(0, 0, 0, 0.6)",colorInfo:"rgba(11, 131, 241, 1)",colorInfoHover:"rgba(94, 188, 255, 1)",colorInfoBorder:"rgba(57, 144, 201, 1)",colorInfoText:"rgba(11, 131, 241, 1)",colorInfoBg:"rgba(17, 34, 51, 1)",colorInfoBgHover:"rgba(38, 72, 102, 1)",colorInfoBorderHover:"rgba(94, 188, 255, 1)",colorSuccess:"rgba(91, 185, 139, 1)",colorSuccessHover:"rgba(116, 164, 124, 1)",colorSuccessBg:"rgba(41, 54, 49, 1)",colorSuccessBgHover:"rgba(63, 81, 71, 1)",colorSuccessBorder:"rgba(115, 163, 119, 1)",colorSuccessBorderHover:"rgba(171, 212, 190, 1)",colorWarning:"rgba(250, 173, 20, 1)",colorWarningHover:"rgba(255, 140, 40, 1)",colorWarningBg:"rgba(45, 35, 26, 1)",colorWarningBgHover:"rgba(77, 59, 35, 1)",colorWarningBorder:"rgba(255, 166, 40, 1)",colorWarningBorderHover:"rgba(255, 214, 102, 1)",colorError:"rgba(255, 77, 79, 1)",colorErrorHover:"rgba(238, 101, 78, 1)",colorErrorBg:"rgba(43, 11, 8, 1)",colorErrorBgHover:"rgba(74, 42, 40, 1)",colorErrorBorder:"rgba(238, 91, 78, 1)",colorErrorBorderHover:"rgba(255, 163, 158, 1)",colorLink:"rgba(77, 125, 255, 1)",colorTextWhite:"rgba(255, 255, 255, 1)",colorFillDisable:"rgba(90, 90, 90, 1)",colorPurple:"rgba(97, 92, 237, 1)",colorPurpleHover:"rgba(97, 92, 237, 1)",colorPurpleBg:"rgba(60, 53, 85, 1)",colorPink:"rgba(236, 72, 153, 1)",colorPinkHover:"rgba(236, 72, 153, 1)",colorPinkBg:"rgba(74, 42, 58, 1)",colorYellow:"rgba(234, 179, 8, 1)",colorYellowHover:"rgba(234, 179, 8, 1)",colorYellowBg:"rgba(77, 71, 32, 1)",colorOrange:"rgba(250, 129, 37, 1)",colorOrangeHover:"rgba(250, 129, 37, 1)",colorOrangeBg:"rgba(255, 232, 201, 0.8)",colorTeal:"rgba(20, 184, 166, 1)",colorTealHover:"rgba(20, 184, 166, 1)",colorTealBg:"rgba(42, 70, 64, 1)",colorBlue:"rgba(0, 144, 255, 1)",colorBlueHover:"rgba(0, 144, 255, 1)",colorBlueBg:"rgba(42, 59, 78, 0.8)",colorMauve:"rgba(142, 140, 153, 1)",colorMauveHover:"rgba(142, 140, 153, 1)",colorMauveBg:"rgba(46, 46, 50, 0.8)",boxShadow:"0px 4px 6px 0px rgba(255, 255, 255, 0.08)",boxShadowSecondary:"0px 12px 24px -16px rgba(255, 255, 255, 0.04), 0px 8px 40px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.03)",boxShadowTertiary:"0px 12px 32px -16px rgba(255, 255, 255, 0.16), 0px 12px 60px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.04)",boxShadowTertiaryLeft:"-12px 0px 32px -16px rgba(255, 255, 255, 0.16), -12px 0px 60px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.04)",boxShadowInput:"0px 12px 24px -16px rgba(254, 254, 255, 0.04), 0px 12px 40px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.02)"},c=(0,P.Z)(v,!0),o=(0,P.Z)(a,!1)},23630:function(h,n,e){e.r(n);var P=e(5574),a=e.n(P),v=e(67294),c=e(7715),o=e(45210),i={};function s(t){var u=(0,v.useRef)(t||Math.random().toString()),l=(0,v.useContext)(c.DemoContext),g=l.demoContext,b=(0,v.useState)(i[u.current]),f=a()(b,2),p=f[0],r=f[1];return(0,v.useEffect)(function(){i[u.current]=[p,r]},[p,r]),(0,o.Z)(function(){delete i[u.current]}),(0,v.useEffect)(function(){if(!g.canvas)for(var d in i)i[d][0]=!1,i[d][1](!1)},[g.canvas]),[p,function(d){for(var x in i)i[x][0]=!1,i[x][1](!1);i[u.current]=d,r(d)}]}n.default=s},30383:function(h,n,e){e.r(n),e.d(n,{default:function(){return b}});var P=e(15009),a=e.n(P),v=e(64599),c=e.n(v),o=e(97857),i=e.n(o),s=e(99289),t=e.n(s),u=e(93513),l=e(28262),g=e(34226);function b(r,d){return f.apply(this,arguments)}function f(){return f=t()(a()().mark(function r(d,x){var y,m,k,E,C;return a()().wrap(function(M){for(;;)switch(M.prev=M.next){case 0:return M.next=2,(0,g.default)(1e3);case 2:return y=x.current.getMessages().find(function(S){return S.id===d.current.answer.id}),m=(0,u.produce)(y,function(S){S.cards.push({code:"Text",data:{content:"I am conducting in-depth research on the topic you provided.",msgStatus:"finished",className:"claude-clone-font"}}),S.cards.push({code:"DeepResearch",data:{data:{title:"loading...",steps:[]}}})}),x.current.updateMessage(m),M.next=7,(0,g.default)(100);case 7:k=c()(p()),M.prev=8,C=a()().mark(function S(){var T,I,O;return a()().wrap(function(w){for(;;)switch(w.prev=w.next){case 0:return T=E.value,w.next=3,(0,g.default)(1e3);case 3:I=x.current.getMessages().find(function(B){return B.id===d.current.answer.id}),O=(0,u.produce)(I,function(B){B.cards[1].data.data=T}),x.current.updateMessage(O);case 6:case"end":return w.stop()}},S)}),k.s();case 11:if((E=k.n()).done){M.next=15;break}return M.delegateYield(C(),"t0",13);case 13:M.next=11;break;case 15:M.next=20;break;case 17:M.prev=17,M.t1=M.catch(8),k.e(M.t1);case 20:return M.prev=20,k.f(),M.finish(20);case 23:case"end":return M.stop()}},r,null,[[8,17,20,23]])})),f.apply(this,arguments)}function p(){var r=[{title:l.default.title,steps:[]},{title:l.default.title,steps:l.default.steps.map(function(d){return i()(i()({},d),{},{status:"RUNNING"})})},{title:l.default.title,steps:l.default.steps.map(function(d,x){return x>0?i()(i()({},d),{},{status:"RUNNING"}):i()(i()({},d),{},{status:"FINISHED"})})},{title:l.default.title,steps:l.default.steps.map(function(d,x){return x>1?i()(i()({},d),{},{status:"RUNNING"}):i()(i()({},d),{},{status:"FINISHED"})})},{title:l.default.title,steps:l.default.steps.map(function(d,x){return i()(i()({},d),{},{status:"FINISHED"})})},l.default];return r}},28262:function(h,n,e){e.r(n),n.default={title:"Free AI tool research",steps:[{title:"Exploring free AI tools",status:"FINISHED",startTime:"2024-01-15T09:00:00Z",endTime:"2024-01-15T10:30:00Z",list:[{content:`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.`,siteList:[{title:"City Pop - Wikipedia"},{title:"City Pop - Wikipedia"},{title:"City Pop - Wikipedia"},{title:"City Pop - Wikipedia"},{title:"City Pop - Wikipedia"},{title:"City Pop - Wikipedia"},{title:"City Pop - Wikipedia"},{title:"City Pop - Wikipedia"},{title:"City Pop - Wikipedia"},{title:"City Pop - Wikipedia"}]}]},{title:"Considering search strategies",status:"FINISHED",startTime:"2024-01-15T10:30:00Z",endTime:"2024-01-15T12:00:00Z",list:[{content:`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics.`,siteList:[{title:"City Pop - Wikipedia"}]}]},{title:"Optimizing discovery methods",status:"FINISHED",startTime:"2024-01-15T12:00:00Z",endTime:"2024-01-15T13:30:00Z",list:[{content:`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics.`}]}],site:{list:[{title:"City Pop - Wikipedia",description:"City Pop - Wikipedia"},{title:"City Pop - Wikipedia",description:"City Pop - Wikipedia"},{title:"City Pop - Wikipedia",description:"City Pop - Wikipedia"}]},result:{fileKey:"deep_research_ai_healthcare_20240115",startTime:"2024-01-15T09:00:00Z",endTime:"2024-01-15T15:00:00Z",status:"FINISHED",content:`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.`}}},13021:function(h,n,e){e.r(n),e.d(n,{default:function(){return u}});var P=e(15009),a=e.n(P),v=e(99289),c=e.n(v),o=e(75397),i=e(67379),s=e(50123),t=e(30383);function u(g){return l.apply(this,arguments)}function l(){return l=c()(a()().mark(function g(b){var f,p,r,d;return a()().wrap(function(y){for(;;)switch(y.prev=y.next){case 0:if(f=b.getDemoContext,p=b.currentQA,r=b.ref,d=f(),d.mode!=="WebDev"){y.next=6;break}return y.next=5,(0,s.default)(p,r);case 5:return y.abrupt("return",y.sent);case 6:if(d.mode!=="DeepResearch"){y.next=10;break}return y.next=9,(0,t.default)(p,r);case 9:return y.abrupt("return",y.sent);case 10:if(!d.enableThinking){y.next=14;break}return y.next=13,(0,o.default)(p,r);case 13:return y.abrupt("return",y.sent);case 14:return y.next=16,(0,i.default)(p,r);case 16:return y.abrupt("return",y.sent);case 17:case"end":return y.stop()}},g)})),l.apply(this,arguments)}},67379:function(h,n,e){e.r(n),e.d(n,{default:function(){return l}});var P=e(15009),a=e.n(P),v=e(64599),c=e.n(v),o=e(99289),i=e.n(o),s=e(93513),t=e(34226),u=function(){for(var f=`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.
`,p=[];f.length>0;){var r=Math.floor(Math.random()*100)+1,d=f.slice(0,r);f=f.slice(r),p.push(d)}return p};function l(b,f){return g.apply(this,arguments)}function g(){return g=i()(a()().mark(function b(f,p){var r,d,x;return a()().wrap(function(m){for(;;)switch(m.prev=m.next){case 0:r=c()(u()),m.prev=1,x=a()().mark(function k(){var E,C,D;return a()().wrap(function(S){for(;;)switch(S.prev=S.next){case 0:return E=d.value,S.next=3,(0,t.default)(E.length*5);case 3:C=p.current.getMessages().find(function(T){return T.id===f.current.answer.id}),D=(0,s.produce)(C,function(T){T.cards.length===0?T.cards.push({code:"Text",data:{content:E,msgStatus:"generating",className:"claude-clone-font"}}):T.cards[0].data.content=T.cards[0].data.content+E}),p.current.updateMessage(D);case 6:case"end":return S.stop()}},k)}),r.s();case 4:if((d=r.n()).done){m.next=8;break}return m.delegateYield(x(),"t0",6);case 6:m.next=4;break;case 8:m.next=13;break;case 10:m.prev=10,m.t1=m.catch(1),r.e(m.t1);case 13:return m.prev=13,r.f(),m.finish(13);case 16:case"end":return m.stop()}},b,null,[[1,10,13,16]])})),g.apply(this,arguments)}},34226:function(h,n,e){e.r(n),e.d(n,{default:function(){return P}});function P(a){return new Promise(function(v){return setTimeout(v,a)})}},75397:function(h,n,e){e.r(n),e.d(n,{default:function(){return l}});var P=e(15009),a=e.n(P),v=e(64599),c=e.n(v),o=e(99289),i=e.n(o),s=e(93513),t=e(34226),u=function(){for(var f=`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.
  
City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.`,p=[];f.length>0;){var r=Math.floor(Math.random()*100)+1,d=f.slice(0,r);f=f.slice(r),p.push(d)}return p};function l(b,f){return g.apply(this,arguments)}function g(){return g=i()(a()().mark(function b(f,p){var r,d,x,y,m,k;return a()().wrap(function(C){for(;;)switch(C.prev=C.next){case 0:r=c()(u()),C.prev=1,x=a()().mark(function D(){var M,S,T;return a()().wrap(function(O){for(;;)switch(O.prev=O.next){case 0:return M=d.value,O.next=3,(0,t.default)(M.length*5);case 3:S=p.current.getMessages().find(function(L){return L.id===f.current.answer.id}),T=(0,s.produce)(S,function(L){L.cards.length===0?L.cards.push({code:"DeepThink",data:{content:M,loading:!0,className:"claude-clone-font"}}):L.cards[0].data.content=L.cards[0].data.content+M}),p.current.updateMessage(T);case 6:case"end":return O.stop()}},D)}),r.s();case 4:if((d=r.n()).done){C.next=8;break}return C.delegateYield(x(),"t0",6);case 6:C.next=4;break;case 8:C.next=13;break;case 10:C.prev=10,C.t1=C.catch(1),r.e(C.t1);case 13:return C.prev=13,r.f(),C.finish(13);case 16:y=c()(u()),C.prev=17,k=a()().mark(function D(){var M,S,T;return a()().wrap(function(O){for(;;)switch(O.prev=O.next){case 0:return M=m.value,O.next=3,(0,t.default)(M.length*5);case 3:S=p.current.getMessages().find(function(L){return L.id===f.current.answer.id}),T=(0,s.produce)(S,function(L){L.cards[0].data.loading=!1,L.cards[1]?L.cards[1].data.content=L.cards[1].data.content+M:L.cards.push({code:"Text",data:{content:M,msgStatus:"generating",className:"claude-clone-font"}})}),p.current.updateMessage(T);case 6:case"end":return O.stop()}},D)}),y.s();case 20:if((m=y.n()).done){C.next=24;break}return C.delegateYield(k(),"t2",22);case 22:C.next=20;break;case 24:C.next=29;break;case 26:C.prev=26,C.t3=C.catch(17),y.e(C.t3);case 29:return C.prev=29,y.f(),C.finish(29);case 32:case"end":return C.stop()}},b,null,[[1,10,13,16],[17,26,29,32]])})),g.apply(this,arguments)}},50123:function(h,n,e){e.r(n),e.d(n,{default:function(){return l}});var P=e(15009),a=e.n(P),v=e(64599),c=e.n(v),o=e(99289),i=e.n(o),s=e(93513),t=e(34226),u=e(19791);function l(f,p){return g.apply(this,arguments)}function g(){return g=i()(a()().mark(function f(p,r){var d,x,y,m,k;return a()().wrap(function(C){for(;;)switch(C.prev=C.next){case 0:return C.next=2,(0,t.default)(2e3);case 2:d=r.current.getMessages().find(function(D){return D.id===p.current.answer.id}),x=(0,s.produce)(d,function(D){D.cards.push({code:"Text",data:{content:"I am creating a document for your idea.",msgStatus:"finished",className:"claude-clone-font"}}),D.cards.push({code:"WebDev",data:{loading:!0,content:"",title:p.current.query.cards[0].data.content}})}),r.current.updateMessage(x),y=c()(b()),C.prev=6,k=a()().mark(function D(){var M,S,T;return a()().wrap(function(O){for(;;)switch(O.prev=O.next){case 0:return M=m.value,O.next=3,(0,t.default)(M.length);case 3:S=r.current.getMessages().find(function(L){return L.id===p.current.answer.id}),T=(0,s.produce)(S,function(L){L.cards[1].data.content=L.cards[1].data.content+M}),r.current.updateMessage(T);case 6:case"end":return O.stop()}},D)}),y.s();case 9:if((m=y.n()).done){C.next=13;break}return C.delegateYield(k(),"t0",11);case 11:C.next=9;break;case 13:C.next=18;break;case 15:C.prev=15,C.t1=C.catch(6),y.e(C.t1);case 18:return C.prev=18,y.f(),C.finish(18);case 21:case"end":return C.stop()}},f,null,[[6,15,18,21]])})),g.apply(this,arguments)}function b(){for(var f=(0,u.default)(),p=[];f.length>0;){var r=Math.floor(Math.random()*100)+1,d=f.slice(0,r);f=f.slice(r),p.push(d)}return p}},19791:function(h,n,e){e.r(n),n.default=function(){return`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>My Portfolio</title>
    <style>
        * {
            margin: 0;
            padding: 0;
            box-sizing: border-box;
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
        }
        
        body {
            background-color: #f8f9fa;
            color: #333;
            line-height: 1.6;
        }
        
        header {
            background-color: #2c3e50;
            color: white;
            padding: 2rem 0;
            text-align: center;
        }
        
        nav {
            background-color: #34495e;
            position: sticky;
            top: 0;
            z-index: 100;
        }
        
        nav ul {
            display: flex;
            justify-content: center;
            list-style: none;
        }
        
        nav ul li a {
            color: white;
            display: block;
            padding: 1rem 2rem;
            text-decoration: none;
            transition: background-color 0.3s;
        }
        
        nav ul li a:hover {
            background-color: #2c3e50;
        }
        
        section {
            max-width: 1200px;
            margin: 0 auto;
            padding: 4rem 2rem;
        }
        
        .hero {
            text-align: center;
            padding: 6rem 2rem;
        }
        
        .hero h1 {
            font-size: 3.5rem;
            margin-bottom: 1rem;
            color: #3498db;
        }
        
        .hero p {
            font-size: 1.5rem;
            max-width: 800px;
            margin: 0 auto 2rem;
        }
        
        .btn {
            display: inline-block;
            background-color: #3498db;
            color: white;
            padding: 0.8rem 1.8rem;
            border-radius: 50px;
            text-decoration: none;
            font-weight: bold;
            transition: all 0.3s;
            margin-top: 1rem;
        }
        
        .btn:hover {
            background-color: #2980b9;
            transform: translateY(-3px);
            box-shadow: 0 5px 15px rgba(0, 0, 0, 0.1);
        }
        
        h2 {
            text-align: center;
            font-size: 2.5rem;
            margin-bottom: 3rem;
            color: #2c3e50;
            position: relative;
        }
        
        h2:after {
            content: '';
            display: block;
            width: 80px;
            height: 4px;
            background-color: #3498db;
            margin: 1rem auto;
        }
        
        .about-content {
            display: flex;
            align-items: center;
            gap: 4rem;
        }
        
        .about-img {
            flex: 1;
            border-radius: 10px;
            overflow: hidden;
            box-shadow: 0 10px 30px rgba(0, 0, 0, 0.1);
        }
        
        .about-img img {
            width: 100%;
            height: auto;
            display: block;
        }
        
        .about-text {
            flex: 1;
        }
        
        .about-text h3 {
            font-size: 1.8rem;
            margin-bottom: 1rem;
            color: #2c3e50;
        }
        
        .skills {
            display: flex;
            flex-wrap: wrap;
            gap: 1rem;
            margin-top: 2rem;
        }
        
        .skill {
            background-color: #ecf0f1;
            padding: 0.5rem 1.5rem;
            border-radius: 50px;
            font-weight: 500;
            color: #2c3e50;
        }
        
        .projects {
            display: grid;
            grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));
            gap: 2rem;
        }
        
        .project {
            background-color: white;
            border-radius: 10px;
            overflow: hidden;
            box-shadow: 0 5px 15px rgba(0, 0, 0, 0.1);
            transition: transform 0.3s;
        }
        
        .project:hover {
            transform: translateY(-10px);
        }
        
        .project img {
            width: 100%;
            height: 200px;
            object-fit: cover;
        }
        
        .project-content {
            padding: 1.5rem;
        }
        
        .project-content h3 {
            font-size: 1.5rem;
            margin-bottom: 0.5rem;
            color: #2c3e50;
        }
        
        .project-content p {
            margin-bottom: 1rem;
            color: #7f8c8d;
        }
        
        .project-links {
            display: flex;
            gap: 1rem;
        }
        
        .project-links a {
            color: #3498db;
            text-decoration: none;
            font-weight: 500;
        }
        
        .project-links a:hover {
            text-decoration: underline;
        }
        
        .contact-content {
            display: flex;
            gap: 4rem;
        }
        
        .contact-info {
            flex: 1;
        }
        
        .contact-info p {
            margin-bottom: 1.5rem;
            display: flex;
            align-items: center;
        }
        
        .contact-info i {
            margin-right: 1rem;
            color: #3498db;
            font-size: 1.2rem;
        }
        
        .contact-form {
            flex: 1;
        }
        
        .form-group {
            margin-bottom: 1.5rem;
        }
        
        .form-group label {
            display: block;
            margin-bottom: 0.5rem;
            color: #2c3e50;
            font-weight: 500;
        }
        
        .form-group input,
        .form-group textarea {
            width: 100%;
            padding: 0.8rem;
            border: 1px solid #ddd;
            border-radius: 5px;
            font-size: 1rem;
        }
        
        .form-group textarea {
            height: 150px;
            resize: vertical;
        }
        
        footer {
            background-color: #2c3e50;
            color: white;
            text-align: center;
            padding: 2rem;
            margin-top: 4rem;
        }
        
        .social-links {
            margin: 1rem 0;
        }
        
        .social-links a {
            color: white;
            margin: 0 0.8rem;
            font-size: 1.5rem;
            transition: color 0.3s;
        }
        
        .social-links a:hover {
            color: #3498db;
        }
        
        @media (max-width: 768px) {
            .about-content,
            .contact-content {
                flex-direction: column;
            }
            
            .hero h1 {
                font-size: 2.5rem;
            }
            
            .hero p {
                font-size: 1.2rem;
            }
            
            .about-img,
            .about-text,
            .contact-info,
            .contact-form {
                flex: none;
                width: 100%;
            }
            
            nav ul {
                flex-direction: column;
            }
            
            nav ul li a {
                text-align: center;
            }
        }
    </style>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
</head>
<body>
    <header>
        <h1>John Doe</h1>
        <p>Full-Stack Developer & Designer</p>
    </header>
    
    <nav>
        <ul>
            <li><a href="#home">Home</a></li>
            <li><a href="#about">About</a></li>
            <li><a href="#projects">Projects</a></li>
            <li><a href="#contact">Contact</a></li>
        </ul>
    </nav>
    
    <section id="home" class="hero">
        <h1>Hi, I'm John Doe</h1>
        <p>A passionate full-stack developer creating amazing web experiences</p>
        <a href="#projects" class="btn">View My Work</a>
    </section>
    
    <section id="about">
        <h2>About Me</h2>
        <div class="about-content">
            <div class="about-img">
                <img src="https://images.unsplash.com/photo-1507003211169-0a1d58bc3589?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80" alt="John Doe">
            </div>
            <div class="about-text">
                <h3>Who I Am</h3>
                <p>I'm a full-stack developer with over 5 years of experience building web applications and digital experiences. I specialize in JavaScript frameworks, responsive design, and creating intuitive user interfaces.</p>
                <p>My journey in web development started when I built my first website at the age of 16. Since then, I've worked with startups and established companies to bring their digital visions to life.</p>
                <p>When I'm not coding, you can find me hiking, reading tech blogs, or experimenting with new technologies.</p>
                <div class="skills">
                    <span class="skill">HTML5</span>
                    <span class="skill">CSS3</span>
                    <span class="skill">JavaScript</span>
                    <span class="skill">React</span>
                    <span class="skill">Node.js</span>
                    <span class="skill">Python</span>
                    <span class="skill">MongoDB</span>
                    <span class="skill">UI/UX Design</span>
                </div>
            </div>
        </div>
    </section>
    
    <section id="projects">
        <h2>My Projects</h2>
        <div class="projects">
            <div class="project">
                <img src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80" alt="E-Commerce Platform">
                <div class="project-content">
                    <h3>E-Commerce Platform</h3>
                    <p>A full-featured online shopping platform with payment integration, user authentication, and admin dashboard.</p>
                    <div class="project-links">
                        <a href="#"><i class="fas fa-external-link-alt"></i> Live Demo</a>
                        <a href="#"><i class="fab fa-github"></i> Source Code</a>
                    </div>
                </div>
            </div>
            <div class="project">
                <img src="https://images.unsplash.com/photo-1551650975-87deedd944c3?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80" alt="Task Management App">
                <div class="project-content">
                    <h3>Task Management App</h3>
                    <p>A collaborative task management application with real-time updates, team features, and progress tracking.</p>
                    <div class="project-links">
                        <a href="#"><i class="fas fa-external-link-alt"></i> Live Demo</a>
                        <a href="#"><i class="fab fa-github"></i> Source Code</a>
                    </div>
                </div>
            </div>
            <div class="project">
                <img src="https://images.unsplash.com/photo-1542831371-29b0f74f6b5b?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80" alt="Weather Dashboard">
                <div class="project-content">
                    <h3>Weather Dashboard</h3>
                    <p>A responsive weather application that displays current conditions, forecasts, and interactive maps.</p>
                    <div class="project-links">
                        <a href="#"><i class="fas fa-external-link-alt"></i> Live Demo</a>
                        <a href="#"><i class="fab fa-github"></i> Source Code</a>
                    </div>
                </div>
            </div>
        </div>
    </section>
    
    <section id="contact">
        <h2>Contact Me</h2>
        <div class="contact-content">
            <div class="contact-info">
                <p><i class="fas fa-envelope"></i> john.doe@example.com</p>
                <p><i class="fas fa-phone"></i> +1 (555) 123-4567</p>
                <p><i class="fas fa-map-marker-alt"></i> San Francisco, CA</p>
                <p>I'm currently available for freelance work or full-time positions. If you have a project in mind or just want to connect, feel free to reach out!</p>
            </div>
            <div class="contact-form">
                <form>
                    <div class="form-group">
                        <label for="name">Name</label>
                        <input type="text" id="name" required>
                    </div>
                    <div class="form-group">
                        <label for="email">Email</label>
                        <input type="email" id="email" required>
                    </div>
                    <div class="form-group">
                        <label for="message">Message</label>
                        <textarea id="message" required></textarea>
                    </div>
                    <button type="submit" class="btn">Send Message</button>
                </form>
            </div>
        </div>
    </section>
    
    <footer>
        <div class="social-links">
            <a href="#"><i class="fab fa-github"></i></a>
            <a href="#"><i class="fab fa-linkedin"></i></a>
            <a href="#"><i class="fab fa-twitter"></i></a>
            <a href="#"><i class="fab fa-dribbble"></i></a>
        </div>
        <p>&copy; 2023 John Doe. All rights reserved.</p>
    </footer>

    <script>
        // Smooth scrolling for navigation links
        document.querySelectorAll('nav a, .btn').forEach(link => {
            link.addEventListener('click', function(e) {
                e.preventDefault();
                const targetId = this.getAttribute('href');
                const targetSection = document.querySelector(targetId);
                window.scrollTo({
                    top: targetSection.offsetTop,
                    behavior: 'smooth'
                });
            });
        });

        // Form submission
        const form = document.querySelector('form');
        form.addEventListener('submit', function(e) {
            e.preventDefault();
            alert('Thank you for your message! I will get back to you soon.');
            form.reset();
        });
    <\/script>
</body>
</html>`}},23995:function(h,n,e){e.r(n),e.d(n,{default:function(){return v}});var P=e(9053),a=e(85893);function v(o){var i=o.children,s=c(),t=s.styles;return(0,a.jsxs)("div",{className:t.container,children:[(0,a.jsx)("video",{src:"https://cloud.video.taobao.com/vod/F7ULxg_UaBXJe9XRA2IVhiyjjYRZ10ythvs6iv-yoJg.mp4",autoPlay:!0,loop:!0,muted:!0,playsInline:!0,className:t.video}),i]})}var c=(0,P.kc)(function(o){var i=o.token;return{container:{display:"flex",alignItems:"center",justifyContent:"center",position:"relative",width:"100%",height:"100%",padding:70},video:{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",zIndex:-1}}})},40248:function(h,n,e){e.r(n),e.d(n,{default:function(){return g}});var P=e(15009),a=e.n(P),v=e(99289),c=e.n(v),o=e(5574),i=e.n(o),s=e(67294),t=e(66672),u=e(9053),l=e(85893);function g(f){var p=b(),r=p.styles,d=s.useState(""),x=i()(d,2),y=x[0],m=x[1];return(0,l.jsx)(t.Z,{loading:f.loading,className:r.container,value:y,onChange:m,onSubmit:c()(a()().mark(function k(){return a()().wrap(function(C){for(;;)switch(C.prev=C.next){case 0:f.onSubmit(y),m("");case 2:case"end":return C.stop()}},k)})),placeholder:"Search what you want to know"})}var b=(0,u.kc)(function(f){var p=f.css,r=f.token;return{container:{}}})},4454:function(h,n,e){e.r(n),e.d(n,{default:function(){return u}});var P=e(68400),a=e.n(P),v=e(83962),c=e(50867),o=e(9053),i=e(85893),s,t;function u(g){var b=g.messages,f=l(),p=f.styles;return(0,i.jsx)(v.xy,{cardConfig:{},children:(0,i.jsx)(c.Z.List,{classNames:{wrapper:p.wrapper,list:p.list},items:b})})}var l=(0,o.kc)(function(g){var b=g.css,f=g.token;return{wrapper:b(s||(s=a()([`
    height: 0;
    flex: 1;
    
  `]))),list:b(t||(t=a()([`
  `])))}})},90879:function(h,n,e){e.r(n),e.d(n,{default:function(){return E}});var P=e(68400),a=e.n(P),v=e(19632),c=e.n(v),o=e(5574),i=e.n(o),s=e(9053),t=e(67369),u=e(88773),l=e(16288),g=e(4454),b=e(40248),f=e(67294),p=e(89698),r=e(60227),d=e(36855),x=e(28373),y=e(85893),m,k;function E(){var M=C(),S=M.styles,T=(0,f.useState)([{role:"user",content:"what is citypop",id:"1"},{role:"assistant",cards:[{code:"DeepThink",data:{content:'City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period. City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s. Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist. Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.'}},{code:"Text",data:{content:'City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period. City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s. Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist. Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.'}},{code:"Footer",data:{left:(0,y.jsx)(t.p4,{data:[{icon:(0,y.jsx)(p.Z,{}),onClick:function(){}},{icon:(0,y.jsx)(r.Z,{}),onClick:function(){}},{icon:(0,y.jsx)(d.Z,{}),onClick:function(){}},{icon:(0,y.jsx)(x.Z,{}),onClick:function(){}}]})}}],id:"2"}]),I=i()(T,2),O=I[0],L=I[1];return(0,y.jsxs)(y.Fragment,{children:[(0,y.jsx)(D,{}),(0,y.jsx)(l.Vaso,{className:S.container,radius:20,depth:.5,dispersion:0,blur:10,children:(0,y.jsxs)("div",{className:S.chat,children:[(0,y.jsx)(g.default,{messages:O}),(0,y.jsx)(b.default,{onSubmit:function(){L(function(B){return[].concat(c()(B),[{role:"user",content:"what is citypop",id:(0,u.Z)()}])}),setTimeout(function(){L(function(B){return[].concat(c()(B),[{role:"assistant",id:(0,u.Z)(),content:'City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period. City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s. Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist. Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.'}])})},1e3)},loading:!1})]})})]})}var C=(0,s.kc)(function(M){var S=M.token,T=M.css,I=M.prefixCls;return{container:T(m||(m=a()([`
      width: 100%;
      height: 100%;
      border: 1px solid rgba(200, 196, 191, .5);
      border-radius: 20px;
      overflow: hidden;
    `]))),chat:{display:"flex",flexDirection:"column",maxWidth:800,margin:"0 auto",padding:"20px 0",height:"100%"}}}),D=(0,s.vJ)(k||(k=a()([`
  .ant-bubble-content-filled {
    border: 1px solid rgba(200, 196, 191, .5);
    background-color: rgba(200, 196, 191, .1) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;
  }

  .ant-accordion-deep-thinking {
    border-color:  rgba(200, 196, 191, .5) !important;
  }

  .ant-accordion-group-header-close {
    border: 1px solid rgba(200, 196, 191, .5) !important;
    background-color: rgba(200, 196, 191, .1) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;


  }

  .ant-accordion-group-icon-success {
    color: #fff !important;
  }

  .ant-sender {
    background-color: rgba(54, 53, 52, 0.3) !important;
    border-color: rgba(200, 196, 191, .5) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;
    
  }

  .ant-sender textarea::placeholder,
  .ant-sender textarea {
   color: rgba(231, 231, 237, .88) !important;
  }

  .ant-sender-actions-list .ant-btn {
    background-color: rgba(200, 196, 191, .3) !important;
    border-color: rgba(200, 196, 191, .5) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;


  }

  .ant-bubble-list-scroll-to-bottom .ant-btn {
    background-color: rgba(200, 196, 191, .3) !important;
    border-color: rgba(200, 196, 191, .5) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;



    &:hover {
      color: #fff !important;
      background-color: rgba(200, 196, 191, .5) !important;
      border-color: rgba(200, 196, 191, .5) !important;
    }
  }

  .ant-bubble-footer-left .ant-btn {
    border-color: rgba(200, 196, 191, .5) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;
  }

`])))},36598:function(h,n,e){e.r(n),e.d(n,{default:function(){return i}});var P=e(5574),a=e.n(P),v=e(9053),c=e(67294),o=e(85893);function i(t){var u=s(),l=u.styles,g=(0,c.useState)(0),b=a()(g,2),f=b[0],p=b[1];return(0,c.useEffect)(function(){p(1)},[]),(0,o.jsxs)("div",{className:l.block,style:{opacity:f},children:[(0,o.jsxs)("div",{className:l.header,"data-anchor":t.title,children:[(0,o.jsx)("div",{className:l.icon,children:t.icon}),(0,o.jsx)("div",{className:l.title,children:t.title})]}),(0,o.jsx)("div",{className:l.content,children:t.children})]})}var s=(0,v.kc)(function(t){var u=t.css,l=t.token;return{block:{marginTop:24,marginBottom:48,transition:"opacity 1s ease-in-out"},header:{display:"flex",alignItems:"center",gap:8,color:l.colorText,marginBottom:12},icon:{fontSize:24},title:{fontSize:18,textTransform:"capitalize"},content:{}}})},52022:function(h,n,e){e.r(n),e.d(n,{default:function(){return o}});var P=e(66109),a=e(36598),v=e(23211),c=e(85893);function o(i){return console.log(i),(0,c.jsx)(a.default,{title:"answer",icon:(0,c.jsx)(v.Z,{}),children:(0,c.jsx)(P.Z,{cursor:i.loading,content:i.content,baseLineHeight:1.8571428571428572})})}},28212:function(h,n,e){e.r(n),e.d(n,{default:function(){return v}});var P=e(14082),a=e(85893);function v(c){var o=c.loading,i=c.content;return(0,a.jsx)(P.Z,{defaultOpen:!0,content:i,loading:o,title:o?"Deep thinking":"Completed deep thinking",className:"perplexcity-deep-thinking"})}},17560:function(h,n,e){e.r(n),e.d(n,{default:function(){return i}});var P=e(11289),a=e(9053),v=e(36598),c=e(16494),o=e(85893);function i(t){var u=t.images,l=s(),g=l.styles;return(0,o.jsx)(v.default,{title:"images",icon:(0,o.jsx)(P.Z,{}),children:(0,o.jsx)("div",{className:g.images,children:(0,o.jsx)(c.Z.PreviewGroup,{children:u.map(function(b,f){return(0,o.jsx)(c.Z,{src:b,style:{width:180.55172413793102,height:136,borderRadius:12,overflow:"hidden"}},f)})})})})}var s=(0,a.kc)(function(t){var u=t.css,l=t.token;return{images:{display:"flex",gap:12,padding:12,overflow:"auto",backgroundColor:l.colorFillSecondary,borderRadius:l.borderRadius,"&::-webkit-scrollbar":{display:"none"},".sps-image-mask":{borderRadius:12}}}})},42733:function(h,n,e){e.r(n),e.d(n,{default:function(){return i}});var P=e(29986),a=e(38850),v=e(36598),c=e(9053),o=e(85893);function i(t){var u=s(),l=u.styles;return(0,o.jsx)(v.default,{title:"related",icon:(0,o.jsx)(P.Z,{}),children:(0,o.jsx)("div",{className:l.related,children:t.related.map(function(g,b){return(0,o.jsxs)("div",{className:l.item,children:[(0,o.jsx)(a.Z,{className:l.icon}),g]},b)})})})}var s=(0,c.kc)(function(t){var u=t.css,l=t.token;return{related:{display:"flex",flexDirection:"column",gap:8},item:{height:24,gap:8,display:"flex",alignItems:"center",cursor:"pointer",color:l.colorTextTertiary,"&:hover":{color:l.colorText}},icon:{fontSize:20,color:l.colorPrimary}}})},96298:function(h,n,e){e.r(n),e.d(n,{default:function(){return u}});var P=e(97857),a=e.n(P),v=e(66109),c=e(36598),o=e(54446),i=e(9053),s=e(85893);function t(g){var b=l(),f=b.styles;return(0,s.jsxs)("div",{className:f.item,children:[(0,s.jsx)("img",{className:f.logo,src:g.logo}),(0,s.jsxs)("div",{className:f.content,children:[(0,s.jsx)("div",{className:f.title,children:g.title}),(0,s.jsx)("a",{className:f.url,href:g.url,target:"_blank",children:g.url})]})]})}function u(g){var b=l(),f=b.styles;return(0,s.jsx)(c.default,{title:"sources",icon:(0,s.jsx)(o.Z,{}),children:(0,s.jsx)("div",{className:f.sources,children:g.sources.map(function(p,r){return(0,s.jsxs)("div",{className:f.source,children:[(0,s.jsx)(t,a()({},p)),(0,s.jsx)(v.Z,{content:p.content,baseLineHeight:1.8571428571428572})]},r)})})})}var l=(0,i.kc)(function(g){var b=g.css,f=g.token;return{item:{display:"flex",alignItems:"center",gap:8,marginBottom:8},logo:{width:32,height:32,borderRadius:32},content:{},source:{},sources:{display:"flex",flexDirection:"column",gap:24},title:{fontSize:12,lineHeight:"20px",color:f.colorText},url:{fontSize:12,lineHeight:"20px",color:f.colorTextTertiary,textDecoration:"none","&:hover":{color:f.colorText}}}})},86956:function(h,n,e){e.r(n),e.d(n,{default:function(){return s}});var P=e(5574),a=e.n(P),v=e(67294),c=e(7441),o=e(26367),i=e(85893);function s(t){var u=(0,v.useState)([]),l=a()(u,2),g=l[0],b=l[1],f=(0,c.default)(),p=f.styles,r=(0,v.useRef)(null);return(0,v.useEffect)(function(){var d,x=(d=t.contentRef)===null||d===void 0||(d=d.current)===null||d===void 0?void 0:d.querySelectorAll("[data-anchor]");if(x!=null&&x.length){var y=Array.from(x);b(y)}},[t.data]),(0,i.jsxs)(i.Fragment,{children:[(0,i.jsx)("div",{className:p.content,ref:r,children:t.children}),g.length?(0,i.jsx)(o.Scrollspy,{sectionRefs:g,rootSelector:"#perplexcity-bubble-list>div",children:function(x){var y=x.currentElementIndexInViewport;return console.log(y),(0,i.jsx)("div",{children:"..."})}}):null]})}},39095:function(h,n,e){e.r(n),e.d(n,{default:function(){return k}});var P=e(97857),a=e.n(P),v=e(7441),c=e(17560),o=e(52022),i=e(42733),s=e(96298),t=e(67294),u=Object.defineProperty,l=Object.getOwnPropertySymbols,g=Object.prototype.hasOwnProperty,b=Object.prototype.propertyIsEnumerable,f=(E,C,D)=>C in E?u(E,C,{enumerable:!0,configurable:!0,writable:!0,value:D}):E[C]=D,p=(E,C)=>{for(var D in C||(C={}))g.call(C,D)&&f(E,D,C[D]);if(l)for(var D of l(C))b.call(C,D)&&f(E,D,C[D]);return E};const r=E=>t.createElement("svg",p({xmlns:"http://www.w3.org/2000/svg",fill:"none",width:32,height:32},E),t.createElement("defs",null,t.createElement("clipPath",{id:"Logo_svg__a"},t.createElement("rect",{width:32,height:32,rx:8}))),t.createElement("g",{clipPath:"url(#Logo_svg__a)"},t.createElement("rect",{width:32,height:32,rx:8,fill:"var(--sps-color-text-base)"}),t.createElement("rect",{x:11,y:8,width:3,height:6,rx:1.5,fill:"var(--sps-color-bg-layout)"}),t.createElement("rect",{x:18,y:8,width:3,height:6,rx:1.5,fill:"var(--sps-color-bg-layout)"})));var d="data:image/svg+xml;base64,",x=e(28212),y=e(86956),m=e(85893);function k(E){var C=E.data,D=C.deepThinking,M=C.images,S=C.content,T=C.sources,I=C.related,O=(0,v.default)(),L=O.styles;return(0,m.jsxs)("div",{className:L.container,children:[(0,m.jsx)("div",{className:L.avatar,children:(0,m.jsx)(r,{})}),(0,m.jsxs)(y.default,{data:E.data,children:[D&&(0,m.jsx)(x.default,a()({},D)),M&&(0,m.jsx)(c.default,{images:M}),S&&(0,m.jsx)(o.default,a()({},S)),T&&(0,m.jsx)(s.default,{sources:T}),I&&(0,m.jsx)(i.default,{related:I})]})]})}},7441:function(h,n,e){e.r(n);var P=e(68400),a=e.n(P),v=e(9053),c;n.default=(0,v.kc)(function(o){var i=o.css,s=o.token;return{container:i(c||(c=a()([`
    width: 100%;
    display: flex;
    gap: 12px;
  `]))),content:{width:0,flex:1},avatar:{width:32,height:32,borderRadius:8,overflow:"hidden"}}})},6736:function(h,n,e){e.r(n),e.d(n,{default:function(){return u}});var P=e(29626),a=e(68400),v=e.n(a),c=e(9053),o,i=(0,c.kc)(function(l){var g=l.css,b=l.token;return{container:g(o||(o=v()([`
    width: 100%;
    display: flex;
    gap: 12px;
  `]))),avatar:{backgroundColor:b.colorPrimary},content:{width:0,flex:1,lineHeight:"32px",fontSize:20,fontWeight:500,color:b.colorText},hr:{marginTop:20,borderBottom:"1px dashed ".concat(b.colorBorder)}}}),s=e(88926),t=e(85893);function u(l){var g=i(),b=g.styles;return(0,t.jsxs)("div",{className:b.container,children:[(0,t.jsx)(P.Z,{className:b.avatar,size:32,icon:(0,t.jsx)(s.Z,{style:{fontSize:20}})}),(0,t.jsxs)("div",{className:b.content,children:[l.data.content,(0,t.jsx)("div",{className:b.hr})]})]})}},41298:function(h,n,e){e.r(n),e.d(n,{default:function(){return d}});var P=e(15009),a=e.n(P),v=e(99289),c=e.n(v),o=e(5574),i=e.n(o),s=e(10325),t=e(67294),u=e(89872),l=e(9053),g=(0,l.kc)(function(x){var y=x.css,m=x.token;return{container:{height:"100%",background:m.colorBgLayout},chat:{display:"flex",flexDirection:"column",height:"100%"},disclaimer:{margin:"auto 0 0 0"}}}),b=e(28585),f=e(74459),p=e(76748),r=e(85893);function d(){var x=g(),y=x.styles,m=(0,t.useState)([]),k=i()(m,2),E=k[0],C=k[1],D=(0,t.useState)(!1),M=i()(D,2),S=M[0],T=M[1];return(0,r.jsx)("div",{className:y.container,children:(0,r.jsxs)("div",{className:y.chat,children:[E.length?(0,r.jsx)(f.default,{messages:E}):(0,r.jsx)(u.default,{}),(0,r.jsx)(b.default,{loading:S,onSubmit:function(){var I=c()(a()().mark(function O(L){return a()().wrap(function(B){for(;;)switch(B.prev=B.next){case 0:return T(!0),C([{role:"user",cards:[{code:"Query",data:{content:"What is CityPop?"}}]},{role:"assistant",msgStatus:"generating",cards:[{code:"Answer",data:{deepThinking:{loading:!0,content:"..."}}}]}]),B.next=4,(0,p.default)({messages:E,setMessages:C});case 4:T(!1);case 5:case"end":return B.stop()}},O)}));return function(O){return I.apply(this,arguments)}}()}),(0,r.jsx)("div",{className:y.disclaimer,children:(0,r.jsx)(s.Z,{})})]})})}},76748:function(h,n,e){e.r(n),e.d(n,{default:function(){return g}});var P=e(15009),a=e.n(P),v=e(64599),c=e.n(v),o=e(99289),i=e.n(o),s=e(93513),t={deepThinking:{loading:!1,content:`Okay, the user is asking about "citypop." Let me start by recalling what I know about that term. I remember that City Pop is a genre of music, but I need to make sure I'm accurate.

First, when did City Pop originate? I think it was in Japan during the late 1970s and 1980s. That makes sense because Japan's economy was booming then, leading to a vibrant urban culture. The genre probably blends Western styles like funk, jazz, and disco with Japanese pop elements. Artists like Tatsuro Yamashita and Mariya Takeuchi come to mind. Oh, and there's that song "Plastic Love" by Mariya Takeuchi that went viral online recently, which might have sparked renewed interest.

I should also mention the themes of City Pop songs\u2014they often reflect city life, romance, and leisure, fitting the upbeat and cosmopolitan vibe of the time. The visual style associated with it includes retro anime and vibrant cityscapes, which adds to its aesthetic appeal. It's interesting how the internet, especially platforms like YouTube and Spotify, helped revive this genre internationally. Maybe I should note that connection between modern digital culture and the resurgence of City Pop.

Wait, are there any subgenres or key characteristics I should highlight? The use of synthesizers, smooth vocals, and lush production are typical. Also, City Pop was influenced by yacht rock and soft rock from the West. It's important to link it to Japan's economic bubble era, as that context explains the music's optimistic and luxurious feel.

I need to ensure I don't confuse it with other Japanese genres like J-pop or Enka. City Pop is distinct in its fusion with Western sounds. Including notable albums, like Tatsuro Yamashita's "For You" or Anri's "Timely!!", could be helpful. Oh, and the connection to future funk and vaporwave genres that sample City Pop tracks is a good point for its modern influence.`},images:["https://gw.alicdn.com/imgextra/i3/O1CN01zgK3yS1FAm7uKIHWc_!!6000000000447-0-tps-1232-928.jpg","https://gw.alicdn.com/imgextra/i2/O1CN01Vasr7w1PzHxyioPPm_!!6000000001911-0-tps-1232-928.jpg","https://gw.alicdn.com/imgextra/i4/O1CN01M98Xnc26huPmyBon2_!!6000000007694-0-tps-1232-928.jpg","https://gw.alicdn.com/imgextra/i1/O1CN01EJ1oqo1J01Hx71bgB_!!6000000000965-0-tps-1232-928.jpg","https://gw.alicdn.com/imgextra/i3/O1CN01zgK3yS1FAm7uKIHWc_!!6000000000447-0-tps-1232-928.jpg","https://gw.alicdn.com/imgextra/i2/O1CN01Vasr7w1PzHxyioPPm_!!6000000001911-0-tps-1232-928.jpg","https://gw.alicdn.com/imgextra/i4/O1CN01M98Xnc26huPmyBon2_!!6000000007694-0-tps-1232-928.jpg","https://gw.alicdn.com/imgextra/i1/O1CN01EJ1oqo1J01Hx71bgB_!!6000000000965-0-tps-1232-928.jpg"],content:{content:`City Pop (\u30B7\u30C6\u30A3\u30FC\u30DD\u30C3\u30D7) is a subgenre of Japanese pop music that emerged in the late 1970s and peaked in popularity during the 1980s. It is characterized by its fusion of various musical styles, including funk, disco, R&B, soft rock, and boogie, often reflecting the urban and sophisticated lifestyle of city dwellers. The genre is known for its polished production, catchy melodies, and nostalgic themes, often evoking a sense of escapism and leisure associated with the vibrant city life.
## Key Features of City Pop

- Musical Influences: City Pop draws heavily from Western music trends of the time, particularly adult-oriented genres like funk and disco. It incorporates elements of jazz, lounge music, and even tropical sounds.
- Cultural Context: The genre developed during a period of rapid economic growth in Japan, which influenced its themes of urban sophistication and leisure. It served as a soundtrack for the cosmopolitan lifestyle of the era.
- Aesthetic: City Pop is often associated with vibrant and colorful artwork, featuring themes of summer, beaches, and urban nightlife. The cover art typically reflects a retro-futuristic vision, with bright colors and nostalgic imagery.
`,loading:!1},sources:[{title:"City Pop",logo:"https://gw.alicdn.com/imgextra/i3/O1CN0134H6gj1MP9sJ1T17L_!!6000000001426-55-tps-32-32.svg",url:"https://en.wikipedia.org/wiki/City_Pop",content:`## City pop

City Pop (\u30B7\u30C6\u30A3\u30FC\u30DD\u30C3\u30D7) is a subgenre of Japanese pop music that emerged in the late 1970s and peaked in popularity during the 1980s. It is characterized by its fusion of various musical styles, including funk, disco, R&B, soft rock, and boogie, often reflecting the urban and sophisticated lifestyle of city dwellers. The genre is known for its polished production, catchy melodies, and nostalgic themes, often evoking a sense of escapism and leisure associated with the vibrant city life.`},{title:"City Pop",logo:"https://gw.alicdn.com/imgextra/i3/O1CN0134H6gj1MP9sJ1T17L_!!6000000001426-55-tps-32-32.svg",url:"https://en.wikipedia.org/wiki/City_Pop",content:`## City pop

City Pop (\u30B7\u30C6\u30A3\u30FC\u30DD\u30C3\u30D7) is a subgenre of Japanese pop music that emerged in the late 1970s and peaked in popularity during the 1980s. It is characterized by its fusion of various musical styles, including funk, disco, R&B, soft rock, and boogie, often reflecting the urban and sophisticated lifestyle of city dwellers. The genre is known for its polished production, catchy melodies, and nostalgic themes, often evoking a sense of escapism and leisure associated with the vibrant city life.`}],related:["How did Japan's economic boom influence the development of City Pop?","What are the key characteristics of City Pop music?","How did Japan's economic boom influence the development of City Pop?","What are the key characteristics of City Pop music?"]},u=[{deepThinking:{loading:!0,content:t.deepThinking.content.slice(0,20)}},{deepThinking:{loading:!0,content:t.deepThinking.content.slice(0,40)}},{deepThinking:{loading:!0,content:t.deepThinking.content.slice(0,60)}},{deepThinking:{loading:!0,content:t.deepThinking.content.slice(0,80)}},{deepThinking:{loading:!0,content:t.deepThinking.content.slice(0,100)}},{deepThinking:{loading:!0,content:t.deepThinking.content.slice(0,120)}},{deepThinking:t.deepThinking},{deepThinking:t.deepThinking,images:t.images.slice(0,1)},{deepThinking:t.deepThinking,images:t.images.slice(0,2)},{deepThinking:t.deepThinking,images:t.images.slice(0,3)},{deepThinking:t.deepThinking,images:t.images.slice(0,4)},{deepThinking:t.deepThinking,images:t.images},{deepThinking:t.deepThinking,images:t.images,content:{loading:!0,content:t.content.content.slice(0,20)}},{deepThinking:t.deepThinking,images:t.images,content:{loading:!0,content:t.content.content.slice(0,40)}},{deepThinking:t.deepThinking,images:t.images,content:{loading:!0,content:t.content.content.slice(0,60)}},{deepThinking:t.deepThinking,images:t.images,content:{loading:!0,content:t.content.content.slice(0,80)}},{deepThinking:t.deepThinking,images:t.images,content:t.content},{deepThinking:t.deepThinking,images:t.images,content:t.content,sources:t.sources.slice(0,1)},{deepThinking:t.deepThinking,images:t.images,content:t.content,sources:t.sources},{deepThinking:t.deepThinking,images:t.images,content:t.content,sources:t.sources,related:t.related}];function l(f){return new Promise(function(p){return setTimeout(p,f)})}function g(f){return b.apply(this,arguments)}function b(){return b=i()(a()().mark(function f(p){var r,d,x,y,m;return a()().wrap(function(E){for(;;)switch(E.prev=E.next){case 0:return r=p.messages,d=p.setMessages,E.next=3,l(500);case 3:x=c()(u),E.prev=4,m=a()().mark(function C(){var D;return a()().wrap(function(S){for(;;)switch(S.prev=S.next){case 0:return D=y.value,S.next=3,l(1e3);case 3:d(function(T){var I=T[0],O=T[1];return[I,(0,s.produce)(O,function(L){L.cards[0].data=D})]});case 4:case"end":return S.stop()}},C)}),x.s();case 7:if((y=x.n()).done){E.next=11;break}return E.delegateYield(m(),"t0",9);case 9:E.next=7;break;case 11:E.next=16;break;case 13:E.prev=13,E.t1=E.catch(4),x.e(E.t1);case 16:return E.prev=16,x.f(),E.finish(16);case 19:return E.next=21,l(100);case 21:case"end":return E.stop()}},f,null,[[4,13,16,19]])})),b.apply(this,arguments)}},28585:function(h,n,e){e.r(n),e.d(n,{default:function(){return b}});var P=e(15009),a=e.n(P),v=e(99289),c=e.n(v),o=e(5574),i=e.n(o),s=e(67294),t=e(66672),u=e(9053),l=(0,u.kc)(function(f){var p=f.css,r=f.token;return{container:{width:"800px !important",margin:"0 auto"}}}),g=e(85893);function b(f){var p=l(),r=p.styles,d=s.useState(""),x=i()(d,2),y=x[0],m=x[1];return(0,g.jsx)(t.Z,{loading:f.loading,className:r.container,value:y,onChange:m,onSubmit:c()(a()().mark(function k(){return a()().wrap(function(C){for(;;)switch(C.prev=C.next){case 0:f.onSubmit(y),m("");case 2:case"end":return C.stop()}},k)})),placeholder:"Search what you want to know"})}},74459:function(h,n,e){e.r(n),e.d(n,{default:function(){return b}});var P=e(83962),a=e(50867),v=e(68400),c=e.n(v),o=e(9053),i,s,t=(0,o.kc)(function(f){var p=f.css,r=f.token;return{wrapper:p(i||(i=c()([`
    height: 0;
    flex: 1;
    width: 844px;
    margin: 0 auto;
    left: -22px;
    > div::-webkit-scrollbar {
      width: 0;
    }
  `]))),list:p(s||(s=c()([`
    padding: 16px 0 0 0 !important;
  `])))}}),u=e(6736),l=e(39095),g=e(85893);function b(f){var p=f.messages,r=t(),d=r.styles;return(0,g.jsx)(P.xy,{cardConfig:{Query:u.default,Answer:l.default},children:(0,g.jsx)(a.Z.List,{id:"perplexcity-bubble-list",classNames:{wrapper:d.wrapper,list:d.list},items:p})})}},89872:function(h,n,e){e.r(n),e.d(n,{default:function(){return s}});var P=e(68400),a=e.n(P),v=e(57657),c=e(9053),o=e(85893),i;function s(){var l=t(),g=l.styles;return(0,o.jsxs)(o.Fragment,{children:[(0,o.jsx)(u,{}),(0,o.jsx)("div",{className:g.welcome,children:(0,o.jsx)(v.default,{})})]})}var t=(0,c.kc)(function(l){var g=l.css,b=l.token;return{welcome:{display:"flex",flexDirection:"column",alignItems:"center",gap:4,marginTop:"calc(10vh + 80px + 64px)",marginBottom:32}}}),u=(0,c.vJ)(i||(i=a()([`
.sps-chat-anywhere-chat-welcome {
  height: auto !important;  
}`])))},70539:function(h,n,e){e.r(n),e.d(n,{ConfigProviderContext:function(){return u},default:function(){return l}});var P=e(97857),a=e.n(P),v=e(5574),c=e.n(v),o=e(21766),i=e(75960),s=e(67294),t=e(85893),u=s.createContext(void 0);function l(g){var b=(0,s.useState)(!0),f=c()(b,2),p=f[0],r=f[1],d=p?i.darkTheme:i.lightTheme;return(0,t.jsx)(t.Fragment,{children:(0,t.jsx)(u.Provider,{value:{dark:p,setDark:r},children:(0,t.jsx)(o.ZP,a()(a()({},d),{},{style:{height:"100%"},iconfont:"https://at.alicdn.com/t/a/font_4807885_6wjcpzayws6.js",prefix:"sps",prefixCls:"sps",children:g.children}))})})}},18531:function(h,n,e){e.r(n),e.d(n,{DemoContext:function(){return t},default:function(){return u}});var P=e(97857),a=e.n(P),v=e(5574),c=e.n(v),o=e(79785),i=e(67294),s=e(85893),t=i.createContext(void 0);function u(l){var g=(0,o.Z)({model:"claude-3.5-sonnet",enableSearch:!1,enableThinking:!1,initial:!0,mode:void 0,canvas:!1}),b=c()(g,3),f=b[0],p=b[1],r=b[2];return(0,s.jsx)(t.Provider,{value:{demoContext:f,setDemoContext:function(x){p(function(y){return a()(a()({},y),x)})},getDemoContext:r},children:l.children})}},75960:function(h,n,e){e.r(n),e.d(n,{darkTheme:function(){return c},lightTheme:function(){return o}});var P=e(81759),a={borderRadiusXS:4,borderRadiusSM:6,borderRadius:8,borderRadiusLG:12,borderRadiusXL:16,borderRadiusFull:999,wireframe:!1,colorPrimaryBg:"rgba(239, 237, 229, 1)",colorPrimaryBgHover:"rgba(239, 237, 229, 1)",colorPrimaryBorder:"rgba(186, 90, 59, 1)",colorPrimaryBorderHover:"rgba(194, 98, 66, 1)",colorPrimaryHover:"rgba(194, 98, 66, 1)",colorPrimary:"rgba(208, 110, 78, 1)",colorPrimaryActive:"rgba(194, 98, 66, 1)",colorPrimaryTextHover:"rgba(186, 90, 59, 1)",colorPrimaryText:"rgba(186, 90, 59, 1)",colorPrimaryTextActive:"rgba(186, 90, 59, 1)",colorTextBase:"rgba(26, 26, 29, 1)",colorBgBase:"rgba(255, 255, 255, 1)",colorText:"rgba(20, 20, 19, 0.88)",colorTextSecondary:"rgba(20, 20, 19, 0.65)",colorTextTertiary:"rgba(20, 20, 19, 0.45)",colorTextQuaternary:"rgba(20, 20, 19, 0.25)",colorBorder:"rgba(193, 192, 187, 1)",colorBorderSecondary:"rgba(234, 233, 231, 1)",colorFill:"rgba(43, 27, 0, 0.12)",colorFillSecondary:"rgba(43, 27, 0, 0.08)",colorFillTertiary:"rgba(43, 27, 0, 0.03)",colorFillQuaternary:"rgba(43, 27, 0, 0.02)",colorBgContainer:"rgba(255, 255, 255, 1)",colorBgElevated:"rgba(255, 255, 255, 1)",colorBgLayout:"rgba(249, 248, 244, 1)",colorBgSpotlight:"rgba(20, 20, 19, 0.85)",colorBgMask:"rgba(0, 0, 0, 0.45)",colorInfo:"rgba(11, 131, 241, 1)",colorInfoHover:"rgba(94, 188, 255, 1)",colorInfoBorder:"rgba(135, 209, 255, 1)",colorInfoText:"rgba(11, 131, 241, 1)",colorInfoBg:"rgba(230, 247, 255, 1)",colorInfoBgHover:"rgba(176, 227, 255, 1)",colorInfoBorderHover:"rgba(94, 188, 255, 1)",colorSuccess:"rgba(91, 185, 139, 1)",colorSuccessHover:"rgba(171, 212, 190, 1)",colorSuccessBg:"rgba(235, 250, 240, 1)",colorSuccessBgHover:"rgba(223, 237, 228, 1)",colorSuccessBorder:"rgba(211, 224, 217, 1)",colorSuccessBorderHover:"rgba(171, 212, 190, 1)",colorWarning:"rgba(250, 173, 20, 1)",colorWarningHover:"rgba(255, 214, 102, 1)",colorWarningBg:"rgba(255, 251, 230, 1)",colorWarningBgHover:"rgba(255, 241, 184, 1)",colorWarningBorder:"rgba(255, 229, 143, 1)",colorWarningBorderHover:"rgba(255, 214, 102, 1)",colorError:"rgba(255, 77, 79, 1)",colorErrorHover:"rgba(255, 120, 117, 1)",colorErrorBg:"rgba(255, 242, 240, 1)",colorErrorBgHover:"rgba(255, 241, 240, 1)",colorErrorBorder:"rgba(255, 204, 199, 1)",colorErrorBorderHover:"rgba(255, 163, 158, 1)",colorLink:"rgba(0, 77, 255, 1)",boxShadow:"0px 4px 6px 0px rgba(0, 0, 0, 0.08)",boxShadowSecondary:"0px 12px 24px -16px rgba(0, 0, 0, 0.06), 0px 8px 40px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)",colorTextWhite:"rgba(255, 255, 255, 1)",colorFillDisable:"rgba(235, 235, 232, 1)",colorPurple:"rgba(97, 92, 237, 1)",colorPurpleHover:"rgba(97, 92, 237, 1)",colorPurpleBg:"rgba(226, 222, 255, 0.8)",colorPink:"rgba(236, 72, 153, 1)",colorPinkHover:"rgba(236, 72, 153, 1)",colorPinkBg:"rgba(251, 220, 239, 0.8)",colorYellow:"rgba(234, 179, 8, 1)",colorYellowHover:"rgba(234, 179, 8, 1)",colorYellowBg:"rgba(255, 250, 184, 0.8)",colorOrange:"rgba(250, 129, 37, 1)",colorOrangeHover:"rgba(250, 129, 37, 1)",colorOrangeBg:"rgba(255, 232, 201, 0.8)",colorTeal:"rgba(20, 184, 166, 1)",colorTealHover:"rgba(20, 184, 166, 1)",colorTealBg:"rgba(200, 244, 233, 0.8)",colorBlue:"rgba(0, 144, 255, 1)",colorBlueHover:"rgba(0, 144, 255, 1)",colorBlueBg:"rgba(194, 229, 255, 0.8)",colorMauve:"rgba(142, 140, 153, 1)",colorMauveHover:"rgba(142, 140, 153, 1)",colorMauveBg:"rgba(239, 240, 243, 0.8)",boxShadowTertiary:"0px 12px 32px -16px rgba(0, 0, 0, 0.24), 0px 12px 60px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)",boxShadowTertiaryLeft:"-12px 0px 32px -16px rgba(0, 0, 0, 0.24), -12px 0px 60px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)",boxShadowInput:"0px 12px 24px -16px rgba(54, 54, 73, 0.04), 0px 12px 40px 0px rgba(51, 51, 71, 0.08), 0px 0px 1px 0px rgba(44, 44, 54, 0.02)"},v={borderRadiusXS:4,borderRadiusSM:6,borderRadius:8,borderRadiusLG:12,borderRadiusXL:16,borderRadiusFull:999,wireframe:!1,colorPrimaryBg:"rgba(41, 35, 29, 1)",colorPrimaryBgHover:"rgba(70, 60, 50, 1)",colorPrimaryBorder:"rgba(133, 80, 40, 1)",colorPrimaryBorderHover:"rgba(146, 102, 52, 1)",colorPrimaryHover:"rgba(156, 88, 57, 1)",colorPrimary:"rgba(156, 88, 57, 1)",colorPrimaryActive:"rgba(146, 102, 52, 1)",colorPrimaryTextHover:"rgba(133, 80, 40, 1)",colorPrimaryText:"rgba(160, 96, 63, 1)",colorPrimaryTextActive:"rgba(133, 80, 40, 1)",colorTextBase:"rgba(230, 230, 230, 1)",colorBgBase:"rgba(15, 13, 12, 1)",colorText:"rgba(230, 230, 230, 0.9)",colorTextSecondary:"rgba(180, 180, 180, 0.8)",colorTextTertiary:"rgba(255, 255, 255, 0.6)",colorTextQuaternary:"rgba(255, 255, 255, 0.3)",colorBorder:"rgba(82, 76, 73, 1)",colorBorderSecondary:"rgba(50, 46, 43, 1)",colorFill:"rgba(82, 79, 74, 0.32)",colorFillSecondary:"rgba(83, 79, 72, 0.24)",colorFillTertiary:"rgba(50, 50, 50, 0.26)",colorFillQuaternary:"rgba(67, 64, 59, 0.12)",colorBgContainer:"rgba(52, 50, 47, 1)",colorBgElevated:"rgba(47, 42, 39, 1)",colorBgLayout:"rgba(26, 26, 24, 1)",colorBgSpotlight:"rgba(10, 10, 10, 0.85)",colorBgMask:"rgba(0, 0, 0, 0.6)",colorInfo:"rgba(11, 131, 241, 1)",colorInfoHover:"rgba(94, 188, 255, 1)",colorInfoBorder:"rgba(57, 144, 201, 1)",colorInfoText:"rgba(11, 131, 241, 1)",colorInfoBg:"rgba(17, 34, 51, 1)",colorInfoBgHover:"rgba(38, 72, 102, 1)",colorInfoBorderHover:"rgba(94, 188, 255, 1)",colorSuccess:"rgba(91, 185, 139, 1)",colorSuccessHover:"rgba(116, 164, 124, 1)",colorSuccessBg:"rgba(41, 54, 49, 1)",colorSuccessBgHover:"rgba(63, 81, 71, 1)",colorSuccessBorder:"rgba(115, 163, 119, 1)",colorSuccessBorderHover:"rgba(171, 212, 190, 1)",colorWarning:"rgba(250, 173, 20, 1)",colorWarningHover:"rgba(255, 140, 40, 1)",colorWarningBg:"rgba(45, 35, 26, 1)",colorWarningBgHover:"rgba(77, 59, 35, 1)",colorWarningBorder:"rgba(255, 166, 40, 1)",colorWarningBorderHover:"rgba(255, 214, 102, 1)",colorError:"rgba(255, 77, 79, 1)",colorErrorHover:"rgba(238, 101, 78, 1)",colorErrorBg:"rgba(43, 11, 8, 1)",colorErrorBgHover:"rgba(74, 42, 40, 1)",colorErrorBorder:"rgba(238, 91, 78, 1)",colorErrorBorderHover:"rgba(255, 163, 158, 1)",colorLink:"rgba(77, 125, 255, 1)",colorTextWhite:"rgba(255, 255, 255, 1)",colorFillDisable:"rgba(90, 90, 90, 1)",colorPurple:"rgba(97, 92, 237, 1)",colorPurpleHover:"rgba(97, 92, 237, 1)",colorPurpleBg:"rgba(60, 53, 85, 1)",colorPink:"rgba(236, 72, 153, 1)",colorPinkHover:"rgba(236, 72, 153, 1)",colorPinkBg:"rgba(74, 42, 58, 1)",colorYellow:"rgba(234, 179, 8, 1)",colorYellowHover:"rgba(234, 179, 8, 1)",colorYellowBg:"rgba(77, 71, 32, 1)",colorOrange:"rgba(250, 129, 37, 1)",colorOrangeHover:"rgba(250, 129, 37, 1)",colorOrangeBg:"rgba(255, 232, 201, 0.8)",colorTeal:"rgba(20, 184, 166, 1)",colorTealHover:"rgba(20, 184, 166, 1)",colorTealBg:"rgba(42, 70, 64, 1)",colorBlue:"rgba(0, 144, 255, 1)",colorBlueHover:"rgba(0, 144, 255, 1)",colorBlueBg:"rgba(42, 59, 78, 0.8)",colorMauve:"rgba(142, 140, 153, 1)",colorMauveHover:"rgba(142, 140, 153, 1)",colorMauveBg:"rgba(46, 46, 50, 0.8)",boxShadow:"0px 4px 6px 0px rgba(255, 255, 255, 0.08)",boxShadowSecondary:"0px 12px 24px -16px rgba(255, 255, 255, 0.04), 0px 8px 40px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.03)",boxShadowTertiary:"0px 12px 32px -16px rgba(255, 255, 255, 0.16), 0px 12px 60px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.04)",boxShadowTertiaryLeft:"-12px 0px 32px -16px rgba(255, 255, 255, 0.16), -12px 0px 60px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.04)",boxShadowInput:"0px 12px 24px -16px rgba(254, 254, 255, 0.04), 0px 12px 40px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.02)"},c=(0,P.Z)(v,!0),o=(0,P.Z)(a,!1)},27646:function(h,n,e){e.r(n),n.default={container:"rdClPJKK6krNdAKrayo6",leftContent:"k4k6kmgtB8ZNyrdG55QZ",rightContent:"XXOwciCprQWuAnNCLvTn",icon:"CexBsNjvlDInJWZxxToN",title:"i5grgM3P5m0nnJTQects",duration:"wNBC8jGxRM2OjxpI97gt",thumbnail:"YfrCXzqTyngz6a4YLCC3",fullscreenBtn:"ATj6FDKF36UqZtobOx6c"}},78499:function(h,n,e){e.r(n),n.default={tabsContainer:"mFvDHp2mftcmOSv8Idt0"}},71460:function(h,n,e){e.r(n),n.default={urlWrapper:"wYcT3qwVmJQC5uHJI3zK",moreWrapper:"HykrAXEjCY_ZxcQ8EAaw",simple:"cwknv4q0ykcywGx6sRnr",description:"tJ2OB0MGLj4Emrp7RnNI",detail:"uccPh0HrwtIOQRktydem",title:"OLOIn54JLHA_eZKTea39",url:"ZrTRGYVvg8vbJiMpoVeg",more:"DXPRprkG8IVT2arUyoIZ",icon:"okjPHB7YI0UMELSpff71",iconImage:"GySxot1HnfceTJDwEzRc",linkIcon:"_Qtt7v2ENM2eE1UGJkyQ",rightIcon:"L416r3gTpZQZCwX2sI5S"}},39914:function(h,n,e){e.r(n),n.default={container:"O5yEZdOEOcYwaeBlNyP7",content:"sDAyutlTwy0SiSNLXtXY"}},54627:function(h,n){n.Z=`import { createContext, useContextSelector } from 'use-context-selector';
import { useMemo, useState, useCallback } from 'react';

export type Locale = 'cn' | 'en';

// \u56FD\u9645\u5316\u6587\u6848\u5B9A\u4E49
const messages = {
  cn: {
    // Approval \u76F8\u5173
    'approval.title': '\u4EBA\u5DE5\u5E72\u9884',
    'approval.pending': '\u8BF7\u786E\u8BA4\u662F\u5426\u6267\u884C\u8BE5\u64CD\u4F5C',
    'approval.confirmed': '\u786E\u8BA4\u6267\u884C\u4EFB\u52A1',
    'approval.canceled': '\u53D6\u6D88\u6267\u884C\u4EFB\u52A1',
    'approval.cancel': '\u53D6\u6D88\u6267\u884C',
    'approval.confirm': '\u786E\u8BA4\u6267\u884C',
    'approval.taskRunning': '\u5F53\u524D\u6709\u6B63\u5728\u6267\u884C\u7684\u4EFB\u52A1\uFF0C\u65E0\u6CD5\u53D1\u9001\u65B0\u7684\u4EFB\u52A1',
    
    // ApprovalCancelPopover \u76F8\u5173
    'cancelPopover.title': '\u53D6\u6D88\u539F\u56E0',
    'cancelPopover.placeholder': '\u8BF7\u8F93\u5165\u539F\u56E0\uFF0C\u4EE5\u4FBF\u5927\u6A21\u578B\u505A\u8FDB\u4E00\u6B65\u89C4\u5212',
    'cancelPopover.cancel': '\u53D6\u6D88',
    'cancelPopover.confirm': '\u786E\u8BA4',
    'cancelPopover.options.notNeeded': '\u4E0D\u9700\u8981',
    'cancelPopover.options.poorResult': '\u6548\u679C\u4E0D\u7406\u60F3',
    'cancelPopover.options.tooSlow': '\u7B49\u5F85\u65F6\u95F4\u4E45',
    'cancelPopover.options.wrongInput': '\u8F93\u5165\u9519\u8BEF',

    // \u901A\u7528
    'common.save': '\u4FDD\u5B58',
    'common.cancel': '\u53D6\u6D88',
    'common.confirm': '\u786E\u8BA4',
    'common.delete': '\u5220\u9664',
    'common.edit': '\u7F16\u8F91',
    'common.loading': '\u52A0\u8F7D\u4E2D...',
    'common.saveSuccess': '\u4FDD\u5B58\u6210\u529F',
    'common.saveFailed': '\u4FDD\u5B58\u5931\u8D25',

    // Actions \u76F8\u5173
    'actions.regenerate': '\u91CD\u65B0\u751F\u6210',

    // MessageImport \u76F8\u5173
    'messageImport.title': 'Sessions \u6570\u636E\u5BFC\u5165',
    'messageImport.placeholder': '\u8F93\u5165 JSON \u6570\u636E\u4EE5\u8986\u76D6\u5F53\u524D sessions',
    'messageImport.saveToLocalStorage': '\u4FDD\u5B58\u5230 LocalStorage',
  },
  en: {
    // Approval related
    'approval.title': 'Human Intervention',
    'approval.pending': 'Please confirm whether to execute this operation',
    'approval.confirmed': 'Confirmed to execute task',
    'approval.canceled': 'Canceled task execution',
    'approval.cancel': 'Cancel',
    'approval.confirm': 'Confirm',
    'approval.taskRunning': 'A task is currently running, cannot send new task',
    
    // ApprovalCancelPopover related
    'cancelPopover.title': 'Cancel Reason',
    'cancelPopover.placeholder': 'Please enter the reason for better AI planning',
    'cancelPopover.cancel': 'Cancel',
    'cancelPopover.confirm': 'Confirm',
    'cancelPopover.options.notNeeded': 'Not needed',
    'cancelPopover.options.poorResult': 'Poor result',
    'cancelPopover.options.tooSlow': 'Too slow',
    'cancelPopover.options.wrongInput': 'Wrong input',

    // Common
    'common.save': 'Save',
    'common.cancel': 'Cancel',
    'common.confirm': 'Confirm',
    'common.delete': 'Delete',
    'common.edit': 'Edit',
    'common.loading': 'Loading...',
    'common.saveSuccess': 'Saved successfully',
    'common.saveFailed': 'Failed to save',

    // Actions related
    'actions.regenerate': 'Regenerate',

    // MessageImport related
    'messageImport.title': 'Import Sessions Data',
    'messageImport.placeholder': 'Enter JSON data to override current sessions',
    'messageImport.saveToLocalStorage': 'Save to LocalStorage',
  },
};

export type MessageKey = keyof typeof messages.cn;
type Messages = Record<MessageKey, string>;

export interface I18nContextValue {
  locale: Locale;
  setLocale: (locale: Locale) => void;
  t: (key: MessageKey, params?: Record<string, string | number>) => string;
  messages: Messages;
}

const ChatAnywhereI18nContext = createContext<I18nContextValue | undefined>(undefined);

export function useChatAnywhereI18n<Selected>(selector: (value: I18nContextValue) => Selected): Selected {
  try {
    const context = useContextSelector(ChatAnywhereI18nContext, selector);
    return context;
  } catch (error) {
    return {} as Selected;
  }
}

// \u4FBF\u6377 hook\uFF1A\u76F4\u63A5\u83B7\u53D6\u7FFB\u8BD1\u51FD\u6570
export function useTranslation() {
  const t = useChatAnywhereI18n((ctx) => ctx?.t);
  const locale = useChatAnywhereI18n((ctx) => ctx?.locale);
  const setLocale = useChatAnywhereI18n((ctx) => ctx?.setLocale);
  return { t, locale, setLocale };
}

export interface ChatAnywhereI18nContextProviderProps {
  children: React.ReactNode;
  defaultLocale?: Locale;
}

export function ChatAnywhereI18nContextProvider(props: ChatAnywhereI18nContextProviderProps) {
  const { children, defaultLocale = 'en' } = props;
  const [locale, setLocale] = useState<Locale>(defaultLocale);

  const t = useCallback((key: MessageKey, params?: Record<string, string | number>): string => {
    let message = messages[locale][key] || key;
    
    // \u652F\u6301\u53C2\u6570\u66FF\u6362\uFF0C\u5982 t('hello', { name: 'World' }) => "Hello, World"
    if (params) {
      Object.entries(params).forEach(([paramKey, value]) => {
        message = message.replace(new RegExp(\`\\\\{\${paramKey}\\\\}\`, 'g'), String(value));
      });
    }
    
    return message;
  }, [locale]);

  const value = useMemo<I18nContextValue>(() => ({
    locale,
    setLocale,
    t,
    messages: messages[locale],
  }), [locale, setLocale, t]);

  return (
    <ChatAnywhereI18nContext.Provider value={value}>
      {children}
    </ChatAnywhereI18nContext.Provider>
  );
}

export default ChatAnywhereI18nContext;
`},95285:function(h,n){n.Z=`import Demo from '../starter';


export default Demo`},3225:function(h,n){n.Z=`import { SparkFileCodeLine } from "@agentscope-ai/icons";
import { IconButton, Drawer, Input, Button, message } from "@agentscope-ai/design";
import { useState, useEffect } from "react";
import { useTranslation } from "../../core/Context/ChatAnywhereI18nContext";

const STORAGE_KEY = "agent-scope-runtime-webui-sessions";

export default function (props) {
  const { t } = useTranslation();
  const [open, setOpen] = useState(false);
  const [sessionValue, setSessionValue] = useState("");

  useEffect(() => {
    if (open) {
      const storedValue = localStorage.getItem(STORAGE_KEY) || "";
      setSessionValue(storedValue);
    }
  }, [open]);

  const handleSave = () => {
    try {
      localStorage.setItem(STORAGE_KEY, sessionValue);
      message.success(t?.('common.saveSuccess') || '\u4FDD\u5B58\u6210\u529F');
      location.reload();
    } catch (e) {
      message.error(t?.('common.saveFailed') || '\u4FDD\u5B58\u5931\u8D25');
    }
  };

  return <>
    <IconButton onClick={() => setOpen(true)} icon={<SparkFileCodeLine />} bordered={false} />
    <Drawer
      destroyOnHidden
      open={open}
      onClose={() => setOpen(false)}
      title={t?.('messageImport.title') || 'Sessions \u6570\u636E\u5BFC\u5165'}
      styles={{ body: { padding: 16 }, header: { padding: 8 } }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <Input.TextArea
          value={sessionValue}
          onChange={(e) => setSessionValue(e.target.value)}
          placeholder={t?.('messageImport.placeholder') || '\u8F93\u5165 JSON \u6570\u636E\u4EE5\u8986\u76D6\u5F53\u524D sessions'}
          rows={10}
          style={{ fontFamily: "monospace" }}
        />
        <Button type="primary" onClick={handleSave}>
          {t?.('messageImport.saveToLocalStorage') || '\u4FDD\u5B58\u5230 LocalStorage'}
        </Button>
      </div>
    </Drawer>
  </>
}`},4413:function(h,n){n.Z=`import { Form, Input, InputNumber, Space, ColorPicker, Flex } from 'antd';
import { createStyles } from 'antd-style';


interface FormItemProps {
  name: string | string[];
  label: string;
  isList?: boolean;
  children: any;
  normalize?: (value: any) => any;
}


const useStyles = createStyles(({ token, css }) => ({
  label: {
    marginBottom: 6,
    fontSize: 12,
    color: token.colorTextSecondary,
  },

}));

export default function FormItem(props: FormItemProps) {
  const { styles } = useStyles();


  const node = props.isList ?
    <Form.List name={props.name}>{props.children}</Form.List> :
    <Form.Item name={props.name} normalize={props.normalize}>{props.children}</Form.Item>;


  return <div>
    {props.label && <div className={styles.label}>{props.label}</div>}
    {node}
  </div>

}`},27069:function(h,n){n.Z=`import React, { useEffect } from 'react';
import { Form, Input, ColorPicker, Flex, Splitter, Divider, InputNumber, Select } from 'antd';
import { createStyles } from 'antd-style';
import { Button, IconButton, Switch } from '@agentscope-ai/design'
import { SparkDeleteLine, SparkPlusLine } from '@agentscope-ai/icons';
import FormItem from './FormItem';
import defaultConfig from './defaultConfig';

const useStyles = createStyles(({ token, css }) => ({
  container: {
    height: '100%',
    display: 'flex',
    flexDirection: 'column',
  },

  form: {
    height: 0,
    flex: 1,
    padding: '8px 16px 16px 16px',
    overflow: 'auto',
  },
  actions: {
    padding: 16,
    display: 'flex',
    borderTop: \`1px solid \${token.colorBorderSecondary}\`,
    justifyContent: 'flex-end',
    gap: 16,
  }

}));

interface OptionsEditorProps {
  value?: any;
  onChange?: any;
}

const OptionsEditor: React.FC<OptionsEditorProps> = ({
  value,
  onChange,
}) => {
  const { styles } = useStyles();
  const [form] = Form.useForm();


  const handleSave = () => {
    form.validateFields().then((values) => {
      onChange(values);
    });
  };

  const handleReset = () => {
    form.setFieldsValue(defaultConfig);
  };

  return (
    <div className={styles.container}>
      <Form
        className={styles.form}
        form={form}
        layout="vertical"
        initialValues={value}
      >


        <Divider orientation="left">General</Divider>

        <FormItem name={['theme', 'locale']} label="Language">
          <Select
            options={[
              { label: '\u4E2D\u6587', value: 'cn' },
              { label: 'English', value: 'en' },
            ]}
          />
        </FormItem>

        <Divider orientation="left">Theme</Divider>

        <FormItem name={['theme', 'colorPrimary']} label="colorPrimary" normalize={value => value.toHexString()}>
          <ColorPicker />
        </FormItem>

        <FormItem name={['theme', 'colorBgBase']} label="colorBgBase" normalize={value => value.toHexString()}>
          <ColorPicker />
        </FormItem>

        <FormItem name={['theme', 'colorTextBase']} label="colorTextBase" normalize={value => value.toHexString()}>
          <ColorPicker />
        </FormItem>

        <FormItem name={['theme', 'darkMode']} label="darkMode" >
          <Switch />
        </FormItem>

        <FormItem name={['theme', 'leftHeader', 'logo']} label="leftHeader.logo" >
          <Input />
        </FormItem>

        <FormItem name={['theme', 'leftHeader', 'title']} label="leftHeader.title" >
          <Input />
        </FormItem>

        <Divider orientation="left">Sender</Divider>


        <FormItem name={['sender', 'disclaimer']} label="disclaimer" >
          <Input />
        </FormItem>


        <FormItem name={['sender', 'attachments']} label="attachments" >
          <Switch />
        </FormItem>


        <FormItem name={['sender', 'maxLength']} label="maxLength" >
          <InputNumber min={1000} />
        </FormItem>

        <Divider orientation="left">Welcome</Divider>


        <FormItem name={['welcome', 'greeting']} label="greeting" >
          <Input />
        </FormItem>

        <FormItem name={['welcome', 'description']} label="description" >
          <Input />
        </FormItem>

        <FormItem name={['welcome', 'avatar']} label="avatar" >
          <Input />
        </FormItem>


        <FormItem name={['welcome', 'prompts']} isList label="prompts" >
          {(fields, { add, remove }) => {
            return <div>
              {fields.map(field => {
                return <Flex key={field.key} gap={6}>
                  <Form.Item style={{ flex: 1 }} key={field.key} name={[field.name, 'value']}>
                    <Input />
                  </Form.Item>
                  <IconButton icon={<SparkPlusLine />} onClick={() => add({})}></IconButton>
                  <IconButton icon={<SparkDeleteLine />} onClick={() => remove(field.name)}></IconButton>
                </Flex>
              })}
            </div>
          }}
        </FormItem>


        <Divider orientation="left">API</Divider>

        <FormItem name={['api', 'baseURL']} label="baseURL" >
          <Input />
        </FormItem>

        <FormItem name={['api', 'token']} label="token" >
          <Input />
        </FormItem>
      </Form>

      <div className={styles.actions}>
        <Button onClick={handleReset}>Reset</Button>
        <Button type="primary" onClick={handleSave}>
          Save & Copy
        </Button>
      </div>
    </div>
  );
};

export default OptionsEditor;

`},65975:function(h,n){n.Z=`export default () => {
  return {
    theme: {
      locale: 'en',
      colorPrimary: '#615CED',
      darkMode: true,
      prefix: 'agentscope-runtime-webui',
      leftHeader: {
        logo: 'https://img.alicdn.com/imgextra/i2/O1CN01lmoGYn1kjoXATy4PX_!!6000000004720-2-tps-200-200.png',
        title: 'Runtime WebUI',
      },
    },
    sender: {
      // @ts-ignore
      attachments: true,
      maxLength: 10000,
      disclaimer:
        'AI can also make mistakes, so please check carefully and use it with caution',
      suggestions: [
        {
          label: 'Draw a picture',
          value: 'draw',
        },
        {
          label: 'Check some knowledge',
          value: 'knowledge',
        },
      ],
    },

    welcome: {
      greeting: 'Hello, how can I help you today?',
      nick: 'Assistant',
      description:
        'I am a helpful assistant that can help you with your questions.',
      avatar:
        'https://img.alicdn.com/imgextra/i2/O1CN01lmoGYn1kjoXATy4PX_!!6000000004720-2-tps-200-200.png',

      prompts: [
        {
          value: 'Hello',
        },
        {
          value: 'How are you?',
        },
        {
          value: 'What can you do?',
        },
      ],
    },
    api: {
      baseURL: '',
      token: '',
    },
  };
};
`},36499:function(h,n){n.Z=`import { SparkFileCodeLine, SparkSettingLine } from "@agentscope-ai/icons";
import { IconButton, Drawer } from "@agentscope-ai/design";
import { useState } from "react";
import OptionsEditor from "./OptionsEditor";
import { Flex } from "antd";

interface OptionsPanelProps {
  value?: any;
  onChange?: any;
}

export default function OptionsPanel(props: OptionsPanelProps) {
  const [open, setOpen] = useState(false);

  return <>
    <IconButton onClick={() => setOpen(true)} icon={<SparkSettingLine />} bordered={false} />
    <Drawer
      destroyOnHidden
      open={open}
      onClose={() => setOpen(false)}
      styles={{ body: { padding: 0 }, header: { padding: 8 } }}>
      <OptionsEditor value={props.value} onChange={v => {
        setOpen(false);
        props.onChange(v);
      }} />
    </Drawer>
  </>
}`},5914:function(h,n){n.Z=`import { AgentScopeRuntimeWebUI, IAgentScopeRuntimeWebUIRef, ChatInput } from '@agentscope-ai/chat';
import OptionsPanel from './OptionsPanel';
import { useMemo, useRef } from 'react';
import defaultConfig from './OptionsPanel/defaultConfig';
import { useLocalStorageState } from 'ahooks';
import { Flex } from 'antd';
import MessageImport from './MessageImport';


export default function () {

  const chatRef = useRef<IAgentScopeRuntimeWebUIRef>(null);
  // @ts-ignore
  window.chatRef = chatRef;

  const [optionsConfig, setOptionsConfig] = useLocalStorageState('agent-scope-runtime-webui-options', {
    defaultValue: defaultConfig(),
    listenStorageChange: true,
  });

  const options = useMemo(() => {
    const rightHeader = <Flex gap={16}>
      <OptionsPanel value={optionsConfig} onChange={v => {
        setOptionsConfig(prev => ({
          ...prev,
          ...v,
        }));
      }} />

      <MessageImport />
    </Flex>;

    return {
      ...optionsConfig,
      customToolRenderConfig: {
        // 'weather search mock': Weather,
      },
      session: {
        multiple: true,
      },
      sender: {
        ...optionsConfig.sender,
        beforeUI: <ChatInput.BeforeUIContainer>
          <Flex gap={6}>
            {
              optionsConfig.welcome.prompts.map(prompt => (
                <a key={prompt.value} onClick={() => {
                  chatRef.current?.input.submit({ query: prompt.value });
                }}>{prompt.value}</a>
              ))
            }
          </Flex>
        </ChatInput.BeforeUIContainer>,
        attachments: optionsConfig.sender.attachments ? {
          customRequest(options) {
            // \u6A21\u62DF\u4E0A\u4F20\u8FDB\u5EA6
            options.onProgress({
              percent: 100,
            });
            // \u5F53\u524D\u662F\u4E00\u4E2A mock \u7684\u4E0A\u4F20\u884C\u4E3A
            // \u5B9E\u9645\u60C5\u51B5\u9700\u8981\u5177\u4F53\u5B9E\u73B0\u4E00\u4E2A\u6587\u4EF6\u4E0A\u4F20\u670D\u52A1\uFF0C\u5C06\u6587\u4EF6\u8F6C\u5316\u4E3A url
            options.onSuccess({
              url: URL.createObjectURL(options.file as Blob)
            });
          }
        } : undefined,
      },
      theme: {
        ...optionsConfig.theme,
        rightHeader,
      },
      api: {
        ...optionsConfig.api,
        cancel: (data) => {
          console.log('cancel', data);
        },
      },
    };
  }, [optionsConfig]);



  return <div style={{ height: '100dvh' }}>
    <AgentScopeRuntimeWebUI
      ref={chatRef}
      // @ts-ignore
      options={options}
    />
  </div>;
}`},45411:function(h,n){n.Z=`import { FlowDirectionGraph, type FlowDirectionGraphOptions } from '@ant-design/graphs';
import { createGlobalStyle } from 'antd-style';
import { Card } from '@agentscope-ai/design'

const Style = createGlobalStyle\`
  .user-flow-node {
    width: calc(100% - 32px);
    height: calc(100% - 32px);
    background-color: var(--sps-color-fill);
    border-radius: 8px;
    padding: 10px 16px 24px;
    box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.12), 0 2px 4px 0 rgba(0, 0, 0, 0.1);
    box-sizing: content-box;
  }

  .user-flow-node-name {
    font-size: 16px;
    font-weight: bold;
    color: var(--sps-color-text);
    margin-bottom: 4px;
  }

  .user-flow-node-metric {
    font-size: 12px;
    color: var(--sps-color-text);
    display: flex;
    justify-content: space-between;
    line-height: 1.5;
  }

  .user-flow-node-metric--value {
    font-weight: bold;
  }
\`

const data = {
  nodes: [
    {
      id: 'node-0',
      name: '\u9875\u9762-0',
      layerName: '\u5C42\u7EA70',
      measure: {
        name: 'DAU',
        value: 17500000000,
        formattedValue: 175,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 1000000,
          formattedValue: 100,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#F5A623',
      },
    },
    {
      id: 'node-1',
      name: '\u9875\u97621',
      layerName: '\u5C42\u7EA70',
      measure: {
        name: 'DAU',
        value: 5500000000,
        formattedValue: 55,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 500000,
          formattedValue: 50,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#F5A623',
      },
    },
    {
      id: 'node-2',
      name: '\u9875\u97622',
      layerName: '\u5C42\u7EA70',
      measure: {
        name: 'DAU',
        value: 1000000000,
        formattedValue: 10,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 100000,
          formattedValue: 10,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#F5A623',
      },
    },
    {
      id: 'node-3',
      name: '\u9875\u97623',
      layerName: '\u5C42\u7EA70',
      measure: {
        name: 'DAU',
        value: 900000000,
        formattedValue: 9,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 50000,
          formattedValue: 5,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#F5A623',
      },
    },
    {
      id: 'node-4',
      name: '\u9875\u97624',
      layerName: '\u5C42\u7EA70',
      measure: {
        name: 'DAU',
        value: 5700000000,
        formattedValue: 57,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 60000,
          formattedValue: 6,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#F5A623',
      },
    },
    {
      id: 'node-5',
      name: '\u9875\u97625',
      layerName: '\u5C42\u7EA71',
      measure: {
        name: 'DAU',
        value: 24000000000,
        formattedValue: 240,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 1600000,
          formattedValue: 160,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#4A90E2',
      },
    },
    {
      id: 'node-6',
      name: '\u9875\u97626',
      layerName: '\u5C42\u7EA71',
      measure: {
        name: 'DAU',
        value: 6600000000,
        formattedValue: 66,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 110000,
          formattedValue: 11,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#4A90E2',
      },
    },
    {
      id: 'node-7',
      name: '\u9875\u97627',
      layerName: '\u5C42\u7EA72',
      measure: {
        name: 'DAU',
        value: 5000000000,
        formattedValue: 50,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 500000,
          formattedValue: 50,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#7ED321',
      },
    },
    {
      id: 'node-8',
      name: '\u9875\u97628',
      layerName: '\u5C42\u7EA72',
      measure: {
        name: 'DAU',
        value: 5000000000,
        formattedValue: 50,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 500000,
          formattedValue: 50,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#7ED321',
      },
    },
    {
      id: 'node-9',
      name: '\u9875\u97629',
      layerName: '\u5C42\u7EA72',
      measure: {
        name: 'DAU',
        value: 90000000000,
        formattedValue: 90,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 500000,
          formattedValue: 50,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#7ED321',
      },
    },
    {
      id: 'node-10',
      name: '\u9875\u976210',
      layerName: '\u5C42\u7EA72',
      measure: {
        name: 'DAU',
        value: 100000000000,
        formattedValue: 100,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 100000,
          formattedValue: 10,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#7ED321',
      },
    },
    {
      id: 'node-11',
      name: '\u9875\u976211',
      layerName: '\u5C42\u7EA72',
      measure: {
        name: 'DAU',
        value: 1000000000,
        formattedValue: 10,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 90000,
          formattedValue: 9,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#7ED321',
      },
    },
    {
      id: 'node-12',
      name: '\u9875\u976212',
      layerName: '\u5C42\u7EA72',
      measure: {
        name: 'DAU',
        value: 600000000,
        formattedValue: 6,
        formattedUnit: '\u4E07',
      },
      relatedMeasures: [
        {
          name: 'MAU',
          value: 10000,
          formattedValue: 1,
          formattedUnit: '\u4E07',
        },
      ],
      compareMeasures: [],
      style: {
        stroke: '#7ED321',
      },
    },
  ],
  edges: [
    {
      id: 'edge-0',
      source: 'node-0',
      target: 'node-5',
      measure: {
        name: 'DAU',
        value: 17500000000,
        formattedValue: 175,
        formattedUnit: '\u4E07',
      },
    },
    {
      id: 'edge-1',
      source: 'node-1',
      target: 'node-5',
      measure: {
        name: 'DAU',
        value: 5500000000,
        formattedValue: 55,
        formattedUnit: '\u4E07',
      },
    },
    {
      id: 'edge-2',
      source: 'node-2',
      target: 'node-5',
      measure: {
        name: 'DAU',
        value: 1000000000,
        formattedValue: 10,
        formattedUnit: '\u4E07',
      },
    },
    {
      id: 'edge-3',
      source: 'node-3',
      target: 'node-6',
      measure: {
        name: 'DAU',
        value: 900000000,
        formattedValue: 9,
        formattedUnit: '\u4E07',
      },
    },
    {
      id: 'edge-4',
      source: 'node-5',
      target: 'node-7',
      measure: {
        name: 'DAU',
        value: 5000000000,
        formattedValue: 50,
        formattedUnit: '\u4E07',
      },
    },
    {
      id: 'edge-5',
      source: 'node-5',
      target: 'node-8',
      measure: {
        name: 'DAU',
        value: 5000000000,
        formattedValue: 50,
        formattedUnit: '\u4E07',
      },
    },
    {
      id: 'edge-6',
      source: 'node-5',
      target: 'node-9',
      measure: {
        name: 'DAU',
        value: 9000000000,
        formattedValue: 90,
        formattedUnit: '\u4E07',
      },
    },
    {
      id: 'edge-7',
      source: 'node-5',
      target: 'node-10',
      measure: {
        name: 'DAU',
        value: 5000000000,
        formattedValue: 50,
        formattedUnit: '\u4E07',
      },
    },
    {
      id: 'edge-8',
      source: 'node-6',
      target: 'node-11',
      measure: {
        name: 'DAU',
        value: 1000000000,
        formattedValue: 10,
        formattedUnit: '\u4E07',
      },
    },
    {
      id: 'edge-9',
      source: 'node-4',
      target: 'node-6',
      measure: {
        name: 'DAU',
        value: 5700000000,
        formattedValue: 57,
        formattedUnit: '\u4E07',
      },
    },
    {
      id: 'edge-11',
      source: 'node-6',
      target: 'node-12',
      measure: {
        name: 'DAU',
        value: 600000000,
        formattedValue: 6,
        formattedUnit: '\u4E07',
      },
    },
    {
      id: 'edge-12',
      source: 'node-6',
      target: 'node-10',
      measure: {
        name: 'DAU',
        value: 5000000000,
        formattedValue: 50,
        formattedUnit: '\u4E07',
      },
    },
  ],
};


const UserFlowNode = ({ data }) => {
  const metrics = [
    { name: 'DAU', value: data.measure.formattedValue + data.measure.formattedUnit },
    { name: 'MAU', value: data.relatedMeasures[0].formattedValue + data.relatedMeasures[0].formattedUnit },
  ];

  return (
    <div className="user-flow-node">
      <div className="user-flow-node-name">{data.name}</div>
      {metrics.map((metric) => (
        <div className="user-flow-node-metric" key={metric.name}>
          <div className="user-flow-node-metric-name">{metric.name}</div>
          <div className="user-flow-node-metric-value">{metric.value}</div>
        </div>
      ))}
    </div>
  );
};

const transformData = (data) => {
  const REF_NODE_IDS = ['node-5', 'node-6'];
  const findNodeById = (id) => data.nodes.find((node) => node.id === id);
  data.edges.forEach((edge) => {
    edge.data ||= {};
    const isSplit = REF_NODE_IDS.includes(edge.source);
    edge.data.type = isSplit ? 'split' : 'proportion';
    edge.data.ratio = edge.measure.value / findNodeById(isSplit ? edge.source : edge.target).measure.value;
  });
  return data;
};

export default (props: { data: { page: string } }) => {
  const options: FlowDirectionGraphOptions = {
    autoFit: 'view',
    padding: 8,
    data: transformData(data),
    node: {
      style: {
        component: (data) => <UserFlowNode data={data} />,
        size: [160, 90],
      },
    },
    edge: {
      style: {
        stroke: (d) =>
          d.data.type === 'split' ? 'l(0) 0:#F04864 0.5:#7EC2F3 1:#1890FF' : 'l(0) 0:#1890FF 0.5:#7EC2F3 1:#F04864',
        labelText: (d) => {
          const { type, ratio } = d.data;
          const text = type === 'split' ? '\u5206\u6D41' : '\u5360\u6BD4';
          return \`\${text} \${(Number(ratio) * 100).toFixed(2)}%\`;
        },
        labelBackground: true,
      },
    },
    transforms: (prev) => [
      ...prev,
      {
        type: 'map-edge-line-width',
        key: 'map-edge-line-width',
        value: (d) => d.data.ratio,
        minValue: 0,
        maxValue: 1,
        minLineWidth: 1,
        maxLineWidth: 32,
      },
    ],
    layout: {
      type: 'dagre',
      nodesep: 40,
      ranksep: 200,
    },
  };

  return <>
    <Style />
    <Card title={props.data.page} style={{ maxWidth: '100%', minWidth: '450px' }}>
      <div style={{ height: '400px' }}>
        <FlowDirectionGraph {...options} />
      </div>
    </Card>
  </>
};`},57190:function(h,n){n.Z=`import React, { useState } from 'react';
import { Button, Card, Select } from '@agentscope-ai/design'
import { Flex } from 'antd';
import { useMessages, uuid } from '@agentscope-ai/chat';


interface ISelectPageCardContext {
  data: {
    pages: string[];
  }
  context: {
    onInput: {
      onSubmit: (data: { query: string }) => void;
    };
  }
}

function SelectPageCard(props: ISelectPageCardContext) {
  console.log(props);
  const { updateMessage } = useMessages() || {};
  const [page, setPage] = useState<string>(props.data.pages[0]);

  return <Card title='Please select a page'>
    <Flex gap={8}>
      <Select value={page} onChange={setPage} options={props.data.pages.map(page => ({ label: page, value: page }))} />
      <Button onClick={() => {
        updateMessage({
          id: uuid(),
          role: 'assistant',
          cards: [{
            code: 'PagePVChartCard',
            data: {
              page,
            }
          }]
        });
      }}>OK</Button>
      <Button onClick={() => {
        props.context.onInput?.onSubmit?.({
          query: page,
        });
      }}>Send</Button>
    </Flex>
  </Card>;
}


export default SelectPageCard;`},48796:function(h,n){n.Z=`import { ChatAnywhere, ChatAnywhereRef, DefaultCards, sleep, TMessage, uuid } from '@agentscope-ai/chat';
import SelectPageCard from "./SelectPageCard";
import PagePVChartCard from "./PagePVChartCard";
import { useCallback, useEffect, useRef } from 'react';
import { useAsyncEffect } from 'ahooks';

export default function () {
  const ref = useRef<ChatAnywhereRef>();

  useAsyncEffect(async () => {
    ref.current.updateMessage({
      id: uuid(),
      role: 'user',
      content: 'I want to View page PV data',
    });

    await sleep(100);

    ref.current.updateMessage({
      id: uuid(),
      role: 'assistant',
      cards: [{
        code: 'SelectPageCard',
        data: {
          pages: ['https://foo.com/page1', 'https://foo.com/page2', 'https://foo.com/page3'],
        }
      }]
    })

  }, []);


  const onSubmit = useCallback(async (data) => {
    ref.current.updateMessage({
      id: uuid(),
      role: 'user',
      content: data.query,
    });
    await sleep(100);

    ref.current.updateMessage({
      id: uuid(),
      role: 'assistant',
      cards: [{
        code: 'PagePVChartCard',
        data: {
          page: data.query,
        }
      }]
    });

  }, []);

  const onStop = useCallback(async () => {

  }, []);

  return <ChatAnywhere
    ref={ref}
    cardConfig={{
      SelectPageCard,
      PagePVChartCard,
    }}
    uiConfig={{}}
    onInput={{
      onSubmit,
    }}
    onStop={onStop}
  >

  </ChatAnywhere>
}`},87508:function(h,n){n.Z=`import { Bubble, CustomCardsProvider } from "@agentscope-ai/chat";
import SelectPageCard from "./SelectPageCard";
import PagePVChartCard from "./PagePVChartCard";

export default function () {
  return <CustomCardsProvider cardConfig={{
    SelectPageCard: SelectPageCard,
    PagePVChartCard: PagePVChartCard,
  }}>
    <Bubble.List
      items={[{
        id: '0',
        role: 'user',
        content: 'I want to View page PV data',
      }, {
        id: '1',
        role: 'assistant',
        cards: [{
          code: 'SelectPageCard',
          data: {
            pages: ['https://foo.com/page1', 'https://foo.com/page2', 'https://foo.com/page3'],
          }
        }],
      }, {
        id: '2',
        role: 'assistant',
        cards: [{
          code: 'PagePVChartCard',
          data: {
            page: 'https://foo.com/page1',
          }
        }]
      }]}
    >
    </Bubble.List>
  </CustomCardsProvider>
}`},96794:function(h,n){n.Z=`import React, { useCallback, useRef, useState } from 'react';
import ReactDOM from 'react-dom';
import { Card, Flex, Statistic, UploadFile, Input } from 'antd';
import { Button } from '@agentscope-ai/design';
import { useMount } from 'ahooks';
import compact from 'lodash/compact';

import {
  ChatAnywhere,
  ChatAnywhereRef,
  uuid,
  Stream,
  TMessage,
  createCard,
  DefaultCards,
  Welcome,
  useChatAnywhere
} from '@agentscope-ai/chat';
import { SparkAttachmentLine, SparkDeepThinkLine, SparkReplaceLine } from '@agentscope-ai/icons';

import Header from '../Header';
import sessionLocalStorage from '../sessionLocalStorage';
import { useDemoContext } from '../DemoContext';

import { createGlobalStyle } from 'antd-style';

const Style = createGlobalStyle\`
.sps-chat-anywhere-chat-welcome {
  height: auto !important;
}
\`

export default function () {
  const ref = useRef<ChatAnywhereRef>();
  // @ts-ignore
  window.ref = ref;

  const { demoContext, getDemoContext, setDemoContext } = useDemoContext();

  const currentQA = useRef<{
    query?: TMessage,
    answer?: TMessage,
    abortController?: AbortController;
  }>({});


  useMount(() => {
    ref.current.updateSession(
      sessionLocalStorage.get()
    );
  })


  const onFinish = useCallback((status: 'finished' | 'interrupted' = 'finished', usage?: any) => {
    currentQA.current.answer.msgStatus = status;
    currentQA.current.answer.cards = currentQA.current.answer.cards.map(item => {
      if (item.code === 'Text') {
        return createCard('Text', {
          ...item.data,
          msgStatus: status,
        });
      } else if (item.code === 'DeepThink') {
        return createCard('DeepThink', {
          ...item.data,
          loading: false,
        });
      } else {
        return item;
      }
    });

    if (status === 'interrupted') {
      currentQA.current.answer.cards.push(
        createCard('Interrupted', {})
      );
    }

    currentQA.current.answer.cards.push(
      createCard('Footer', {
        left: <DefaultCards.FooterActions data={[
          {
            icon: <SparkReplaceLine />, label: '\u91CD\u65B0\u751F\u6210', onClick: () => {
              onRegenerate(currentQA.current.answer);
            }
          },
        ]} />,
        right: usage ? <DefaultCards.FooterCount data={[['\u8F93\u5165tokens', usage.prompt_tokens], ['\u8F93\u51FAtokens', usage.completion_tokens]]} ></DefaultCards.FooterCount> : null

      })
    )
    ref.current.setLoading(false);
    ReactDOM.flushSync(() => {
      ref.current.updateMessage(currentQA.current.answer);
    });

    saveToLocalStorage();
  }, [demoContext]);

  const saveToLocalStorage = useCallback(async () => {
    // \u5C06\u5F53\u524D\u7684\u6D88\u606F\u5217\u8868\u540C\u6B65\u5230\u5F53\u524D session \u4E2D
    ReactDOM.flushSync(() => {
      ref.current.updateSessionMessages(ref.current.getMessages());
    });

    sessionLocalStorage.set(ref.current.getSession());
  }, [demoContext]);

  const chat = useCallback(async (messages: TMessage[]) => {

    currentQA.current.answer = {
      id: uuid(),
      cards: [],
      content: '',
      role: 'assistant',
      msgStatus: 'generating',
    }

    ref.current.updateMessage(currentQA.current.answer);

    currentQA.current.abortController = new AbortController();

    const requestMessages = messages.map(msg => {
      return {
        ...msg,
        content: msg.content || (msg.cards || []).reduce((p, c) => { return p + (c.code === 'Text' ? c.data.content : '') }, '')
      };
    });


    const response = await fetch('https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': \`Bearer \${getDemoContext().apikey}\`
      },
      body: JSON.stringify({
        model: getDemoContext().model,
        enable_thinking: getDemoContext().enable_thinking,
        messages: requestMessages,
        stream: true,
        stream_options: {
          include_usage: true,

        },
      }),
      signal: currentQA.current.abortController.signal
    });


    if (response.status !== 200) return;

    let content = '';
    let reasoning_content = '';
    for await (const chunk of Stream({
      readableStream: response.body,
    }, {
      openaiCompatible: true,
    })) {

      if (currentQA.current.answer.msgStatus === 'interrupted') {
        currentQA.current.abortController.abort();
        break;
      }

      if (chunk.data === '[DONE]') {
        onFinish('finished')
        break;
      }

      const data = JSON.parse(chunk.data);
      if (data.usage) {
        onFinish('finished', data.usage);
        break;
      }

      content += data.choices[0]?.delta.content || '';
      reasoning_content += data.choices[0]?.delta.reasoning_content || '';

      currentQA.current.answer.cards = compact([
        reasoning_content && createCard('DeepThink', {
          content: reasoning_content,
          loading: !content,
        }),
        content && createCard('Text', {
          content: content,
          animation: true,
          msgStatus: 'generating'
        })
      ])


      ref.current.updateMessage(currentQA.current.answer);
    }
  }, [demoContext]);

  const onInput = useCallback(async (data: { query: string; fileList?: UploadFile[][] }) => {
    if (!getDemoContext().apikey) {
      document.dispatchEvent(new CustomEvent('settingOpen'));
      return;
    }

    clearFooter();
    currentQA.current.answer = undefined;
    currentQA.current.query = {
      id: uuid(),
      cards: [{
        code: 'Text',
        data: {
          content: data.query,
          msgStatus: 'finished',
          // raw: true,
        }
      }],
      role: 'user',
      msgStatus: 'finished',
    };

    if (data.fileList?.length) {
      const images = [];
      const files = [];
      // @ts-ignore
      const allFileList = data.fileList.reduce((p, c) => [...p, ...c], [])
      allFileList.forEach(file => {
        if (file.type.startsWith('image')) {
          images.push(file)
        } else {
          files.push(file)
        }
      })

      if (images.length) {
        currentQA.current.query.cards.push(createCard('Images', images.map(item => {
          return item.response
        })))
      }

      if (files.length) {
        currentQA.current.query.cards.push(
          createCard('Files', files.map(item => {
            return {
              filename: item.name,
              bytes: item.size
            }
          }))
        )
      }
    }

    ref.current.setLoading(true);


    ReactDOM.flushSync(() => {
      ref.current.updateMessage(currentQA.current.query);
    });

    ref.current.scrollToBottom();
    chat(ref.current.getMessages());
  }, [demoContext]);

  const onStop = useCallback(() => {
    onFinish('interrupted');
  }, [demoContext]);

  const clearFooter = useCallback(() => {
    try {
      currentQA.current.answer.cards = currentQA.current.answer.cards.filter(item => item.code !== 'Footer');
      ref.current.updateMessage(currentQA.current.answer);
    } catch (error) {
    }
  }, [demoContext]);

  const onRegenerate = useCallback((msg: Partial<TMessage>) => {
    ReactDOM.flushSync(() => {
      ref.current.removeMessage(msg);
    });
    const messages = ref.current.getMessages();
    chat(messages);
  }, [demoContext]);

  const enable_thinking = demoContext.enable_thinking;

  const [onUpload, setOnUpload] = useState([]);

  return <div style={{ height: '100vh' }}>
    <Style />
    <ChatAnywhere
      cardConfig={{ MyCard }}
      uiConfig={{
        welcome: <Welcome
          style={{ marginTop: 'calc(50vh - 128px)' }}
          title="Nice to meet you!"
          desc="How can I help you today?"
        />,
        bubbleList: {
          pagination: true,
        },
        disclaimer: 'AI can also make mistakes, so please check and use it carefully',
        header: <Header />,
        logo: <div style={{ fontFamily: 'Montserrat', fontWeight: 'bold' }}>Spark Chat</div>,
        quickInput: <>
          <Button size="small" onClick={() => {
            ReactDOM.flushSync(() => {
              ref.current.updateMessage({
                id: uuid(),
                cards: [
                  {
                    code: 'MyCard',
                    data: [
                      ['MyCard', 10000]
                    ],
                  },
                ],
                role: 'assistant',
                msgStatus: 'finished',
              })

            })
            ref.current.scrollToBottom();
          }}>quickInput</Button>
          <Button size="small" onClick={() => {

          }}>test case A</Button>
          <Button size="small">test case B</Button>
        </>
      }}
      ref={ref}
      onInput={{
        morePrefixActions: <>
          {
            enable_thinking !== undefined ?
              <Button type="text" color={enable_thinking ? 'primary' : 'default'} variant={enable_thinking ? 'filled' : 'text'} icon={<SparkDeepThinkLine />} style={{ padding: '0 6px', gap: 6 }} onClick={() => setDemoContext({ enable_thinking: !enable_thinking })}>DeepThink</Button>
              : null
          }
        </>,
        onSubmit: onInput,
        maxLength: 100000,
        beforeSubmit: () => Promise.resolve(true),
        suggestions: [
          {
            label: 'Draw a picture',
            value: 'draw',
          },
        ],
      }}
      onStop={onStop}
      onRegenerate={onRegenerate}
      onSessionKeyChange={(currentSessionKey) => {
        currentQA.current.answer = undefined;
        currentQA.current.query = undefined;
        sessionLocalStorage.set({ currentSessionKey });
      }}
      onUpload={[{
        multiple: false,
        icon: <SparkAttachmentLine />,
        customRequest(options) {
          options.onSuccess({
            url: URL.createObjectURL(options.file as Blob)
          });
        }
      }, {
        multiple: false,
        icon: <SparkAttachmentLine />,
        customRequest(options) {
          options.onSuccess({
            url: URL.createObjectURL(options.file as Blob)
          });
        }
      }]}
    ></ChatAnywhere>
  </div>
}


function MyCard(props) {
  const onInput = useChatAnywhere(v => v.onInput);

  const [v, sv] = useState('');
  return <div><Card title="my card" size="small" >
    <Flex gap={32}>
      {props.data.map((item) => {
        return <Statistic title={item[0]} value={item[1]} key={item[0]} />
      })}
    </Flex>
    <br />
    <Flex gap={8}>
      <Input placeholder="\u8BF7\u8F93\u5165" onChange={e => sv(e.target.value)} /> <Button onClick={() => {
        onInput.onSubmit({ query: v });
      }}>\u63D0\u4EA4</Button>
    </Flex>
  </Card></div>;
}`},39250:function(h,n){n.Z=`import { useSetState } from 'ahooks';
import { SetState } from 'ahooks/lib/useSetState';
import React, { useState } from 'react';
import { useGetState } from 'ahooks';


type TContext = {
  model: string
  apikey: string
  theme: 'purple' | 'carbon' | string
  enable_thinking?: boolean;
}

export const DemoContext = React.createContext<{
  demoContext: TContext,
  setDemoContext: SetState<TContext>,
  getDemoContext: () => TContext
}>(undefined);

export const DemoContextProvider = function (props: { children: React.ReactNode }) {
  const [demoContext, setDemoContext, getDemoContext] = useGetState<TContext>({
    model: 'qwen-turbo',
    apikey: localStorage.getItem('apikey') || '',
    theme: localStorage.getItem('theme') || 'carbon',
    enable_thinking: undefined,
  });


  return <DemoContext.Provider value={{
    demoContext, getDemoContext, setDemoContext: data => {
      return setDemoContext((state) => {
        const nextState = {
          ...state,
          ...data,
        }

        if (nextState.model === 'qwen3-32b' && nextState.enable_thinking === undefined) {
          nextState.enable_thinking = true
        }

        if (nextState.model !== 'qwen3-32b') {
          nextState.enable_thinking = undefined;
        }

        localStorage.setItem('apikey', nextState.apikey);
        localStorage.setItem('theme', nextState.theme);
        return nextState
      });
    }
  }}>
    {props.children}
  </DemoContext.Provider>
};


export const useDemoContext = function () {
  return React.useContext(DemoContext);
};


`},35753:function(h,n){n.Z=`import React from "react";
import useStyle from './style';
import { Dropdown, Button } from 'antd';
import { useDemoContext } from '../DemoContext';
import { SparkDownLine } from "@agentscope-ai/icons";

const items = [
  {
    key: 'qwen-turbo',
    label: <Model name="Qwen-Turbo" />
  },
  {
    key: 'qwen-max',
    label: <Model name="Qwen-Max" />
  },
  {
    key: 'qwq-plus',
    label: <Model name="Qwen-QwQ-Plus" />
  },
  {
    key: 'qwen3-32b',
    label: <Model name="Qwen3-32B" />
  },
];

function Model(props: {
  name?: string;
  size?: number;
  arrow?: boolean
}) {
  const { name = '', size = 12, arrow = false } = props;
  const { styles } = useStyle();
  return <div className={styles.model}>
    <img src="https://img.alicdn.com/imgextra/i4/O1CN01SdyYTH1Kfz6skC36o_!!6000000001192-2-tps-96-96.png" className={styles.modelLogo} />
    <div className={styles.modelName} style={{ fontSize: size }} >{name}</div>
    {
      arrow && <SparkDownLine />
    }
  </div>
}

export default function () {
  const { demoContext, setDemoContext } = useDemoContext();

  const current = items.find(item => item.key === demoContext.model);

  return <Dropdown
    menu={{
      items,
      selectable: true,
      onSelect: ({ key }) => {
        setDemoContext({
          model: key,
        });
      }
    }}
  >
    <Button type="text" style={{ padding: '0 6px' }}>
      {current?.label ? React.cloneElement(current.label, { size: 16, arrow: true }) : null}
    </Button>
  </Dropdown>
}`},21367:function(h,n){n.Z=`import React, { useEffect, useState } from "react";
import { Drawer, Button, Form, Input, IconButton } from '@agentscope-ai/design';
import { ColorPicker, Flex, Radio } from "antd";
import { useDemoContext } from "../DemoContext";
import { SparkSettingLine } from "@agentscope-ai/icons";


export default function () {
  const [form] = Form.useForm();
  const [open, setOpen] = useState(false);
  const { getDemoContext, setDemoContext } = useDemoContext();

  useEffect(() => {
    const settingOpen = () => setOpen(true);
    document.addEventListener('settingOpen', settingOpen);
    return () => {
      document.removeEventListener('settingOpen', settingOpen);
    }
  }, [])


  return <>
    <IconButton icon={<SparkSettingLine />} onClick={() => setOpen(true)} />

    <Drawer title="Setting"
      afterOpenChange={() => {
        form.setFieldsValue({
          apikey: getDemoContext().apikey,
        })
      }}

      onClose={() => setOpen(false)} open={open} footer={<Button type="primary" onClick={async () => {
        const values = await form.validateFields();
        setDemoContext({
          apikey: values.apikey,
        })
        setOpen(false);
      }}>Save</Button>}>
      <Form
        form={form}
      >
        <Form.Item name="apikey" label="API Key" rules={[{ required: true }]}>
          <Input />
        </Form.Item>

        <a target="_blank" href="https://help.aliyun.com/zh/model-studio/developer-reference/get-api-key">How to get apikey? </a>
      </Form>
    </Drawer>
  </>
}


function ThemeInput({ value, onChange }) {
  return <Flex gap={8} align="center">
    <Radio.Group options={[
      { label: 'Carbon', value: 'carbon' },
      { label: 'Purple', value: 'purple' },
    ]} value={value} onChange={e => onChange(e.target.value)} />
    <ColorPicker value={value} onChange={e => {
      return onChange(e.toCssString());
    }} /></Flex>
}`},90073:function(h,n){n.Z=`import ModelSelect from "./ModelSelect";
import Setting from "./Setting";
import useStyle from "./style";

export default function () {
  const { styles } = useStyle();
  return <div className={styles.header}>
    <ModelSelect />
    <div style={{ flex: 1 }} />
    <Setting />
  </div>
}`},78630:function(h,n){n.Z=`import { createStyles } from 'antd-style';

export default createStyles(({ css, token }) => ({
  header: css\`
    height: 100%;
    display: flex;
    align-items: center;
    justify-content: space-between;
    padding: 0 12px 0 12px;
    width: 100%;
  \`,
  model: css\`
    height: 20px;
    display: flex;
    align-items: center;
    gap: 8px;
  \`,
  modelLogo: css\`
    width: 20px;
    height: 20px;
    background-color: \${token.colorBgBase};
    border-radius: 6px;
  \`,
  modelName: css\`
    font-weight: 500;
    line-height: 20px;
    color: \${token.colorText};
  \`,
}));
`},81895:function(h,n){n.Z=`import React from 'react';
import { DemoContextProvider } from './DemoContext';
import Chat from './Chat';
import { ConfigProvider, carbonTheme } from '@agentscope-ai/design';


export default function () {
  return <div style={{ height: '100vh' }} >
    <ConfigProvider {...carbonTheme}
      prefix='sps'
      prefixCls="sps">
      <DemoContextProvider>
        <Chat />
      </DemoContextProvider>
    </ConfigProvider>
  </div>
}

`},70011:function(h,n){n.Z=`const sessionLocalStorage = {
  get() {
    const localStorageDataString = localStorage.getItem(
      'chat-anywhere-session',
    );

    var data: {
      currentSessionKey: string;
      currentRegenerateIndex: number;
      sessionList: {
        label: string;
        key: string;
        messages: any[][];
      }[];
    };

    if (localStorageDataString) {
      data = JSON.parse(localStorageDataString);
    } else {
      data = {
        sessionList: [
          {
            label: Date.now().toString(),
            key: '9ed1150f-6046-4917-a124-6e85ba1ea933',
            messages: [[]],
          },
        ],
        currentSessionKey: '9ed1150f-6046-4917-a124-6e85ba1ea933',
        currentRegenerateIndex: 0,
      };
    }

    data.sessionList = (data.sessionList || []).map((session) => {
      return {
        ...session,
        messages: (session.messages || []).map((messageGroup) => {
          return (messageGroup || []).map((message) => {
            if (!message || typeof message !== 'object') return message;
            return {
              ...message,
              history: true,
            };
          });
        }),
      };
    });

    return data;
  },
  set(data) {
    const beforeData = sessionLocalStorage.get() || {};

    localStorage.setItem(
      'chat-anywhere-session',
      JSON.stringify({
        ...beforeData,
        ...data,
      }),
    );
  },
};

export default sessionLocalStorage;
`},95401:function(h,n){n.Z=`.container {
  height: 48px;
  background: var(--sps-color-bg-base);
  display: flex;
  align-items: center;
  padding: 0 16px;
  justify-content: space-between;
}

.leftContent {
  display: flex;
  align-items: center;
  gap: 8px;
}

.rightContent {
  display: flex;
  align-items: center;
  gap: 16px;
}

.icon {
  color: var(--sps-color-text);
  font-size: 20px !important;
}

.title {
  font-size: 14px;
  line-height: 24px;
  font-weight: 500;
  color: var(--sps-color-text);
  font-family: 'claude clone';
}

.duration {
  font-size: 12px;
  line-height: 20px;
  color: var(--sps-color-text-tertiary);
  font-family: 'claude clone';
}

.thumbnail {
  .fullscreenBtn {
    display: none;
  }
}`},80516:function(h,n){n.Z=`import React, { useEffect, useState } from 'react';
import { IconFont, IconButton } from '@agentscope-ai/design';
import styles from './Chat.module.less';
import { formatDuration } from '../lib/time';
import { IParseData } from '../type';
import classNames from 'classnames';
import { SparkPaperLine } from '@agentscope-ai/icons';

interface IProps {
  /**
   * \u5F00\u59CB\u65F6\u95F4
   */
  startTime: number;
  /**
   * \u6570\u636E
   */
  data: IParseData;
  /**
   * \u6807\u9898
   */
  title: string;
  /**
   * \u7F29\u7565\u6A21\u5F0F
   */
  thumbnail: boolean;
  /**
   * \u662F\u5426\u5B8C\u6210
   */
  finished: boolean;
  /**
   * \u5168\u5C4F
   */
  onFullscreen: () => void;
  /**
   * \u81EA\u5B9A\u4E49\u6837\u5F0F
   */
  className?: string;
}

const ChatHeader: React.FC<IProps> = (props) => {
  const { title, startTime, data, thumbnail, finished, onFullscreen, className } = props;
  const [endTime, setEndTime] = useState(finished ? data.result?.endTime : Date.now());

  useEffect(() => {
    let clock;

    if (!finished) {
      clock = setInterval(() => {
        setEndTime(Date.now());
      }, 1000);
    } else {
      setEndTime(data.result.endTime);
    }

    return () => {
      clearInterval(clock);
    };
  }, [finished]);

  const durationText = formatDuration(startTime, endTime);

  return (
    <div
      className={classNames(styles.container, className, {
        [styles.thumbnail]: thumbnail,
      })}
    >
      <div className={styles.leftContent}>
        <SparkPaperLine style={{ fontSize: 20 }} />
        <span className={styles.title}>{title}</span>
      </div>
      <div className={styles.rightContent}>
        <span className={styles.duration}>{durationText}</span>
        <IconButton
          className={styles.fullscreenBtn}
          iconType="spark-fullscreen-line"
          onClick={onFullscreen}
          bordered={false}
        />
      </div>
    </div>
  );
};

export default ChatHeader;
`},49166:function(h,n){n.Z=`

.tabsContainer {
  display: flex;
  justify-content: center;
  margin-top: 16px;
  
}
`},35414:function(h,n){n.Z=`import React, { useMemo, useState } from 'react';
import { Button, IconButton, Tabs } from '@agentscope-ai/design';
import styles from './Preview.module.less';
import { IParseData } from '../type';

interface ViewProps {
  data: IParseData;
  /** \u8FD4\u56DE\u6309\u94AE\u70B9\u51FB\u56DE\u8C03 */
  activeTab: string;
  onTabChange?: (key: string) => void;
}

const PreviewHeader: React.FC<ViewProps> = ({ data, activeTab, onTabChange }) => {
  const items = useMemo(() => {
    const list = [];

    if (data.result?.content) {
      list.push({
        key: 'report',
        label: 'Report',
      });
    }

    if (Array.isArray(data.steps) && data.steps.length > 0) {
      list.push({
        key: 'process',
        label: 'Process',
      });
    }

    if (Array.isArray(data.site?.list) && data.site.list.length > 0) {
      list.push({
        key: 'source',
        label: 'Source',
      });
    }

    return list;
  }, [data]);

  const handleTabChange = (key: string) => {
    onTabChange?.(key);
  };

  const handleExport = () => {
    if (!data.result?.fileKey) {
      return;
    }
  };

  return (
    <div className={styles.tabsContainer}>
      <Tabs type="segmented" items={items} activeKey={activeTab} onChange={handleTabChange} />
    </div>
  );
};

export default PreviewHeader;
`},63720:function(h,n){n.Z=`import React, { useState } from 'react';
import { IParseData } from '../type';
import PreviewHeader from '../Header/Preview';
import styles from './index.module.less';
import Result from '../Result';
import Process from '../Process';
import ResearchSource from '../ResearchSource';
import RightPanel, { RightPanelContent, RightPanelHeader } from '../../../components/RightPanel';

interface IProps {
  data: IParseData;
  viewIndex?: number;
}

const Preview: React.FC<IProps> = (props) => {
  const { data, viewIndex } = props;
  const [activeTab, setActiveTab] = useState<string>('process');

  return <RightPanel
    header={<RightPanelHeader title={data.title} actions={null} />}
    content={<RightPanelContent>
      <PreviewHeader
        data={data}
        activeTab={activeTab}
        onTabChange={setActiveTab}
      />

      <div className={styles.content}>
        {activeTab === 'report' && <Result data={data.result} />}
        {activeTab === 'process' && <Process list={data.steps} viewIndex={viewIndex} />}
        {activeTab === 'source' && <ResearchSource list={data.site?.list} />}
      </div>
    </RightPanelContent>}
  >
  </RightPanel>
};

export default Preview;
`},92930:function(h,n){n.Z=`import React, { useEffect, useRef } from 'react';
import { Markdown } from '@agentscope-ai/chat';
import { IStep } from '../type';
import styles from './index.module.less';
import SiteList from '../SiteList';
import classNames from 'classnames';

interface IProps {
  list: IStep[];
  viewIndex?: number;
}

const Process: React.FC<IProps> = (props) => {
  const { list } = props;
  const viewRefList = useRef<HTMLDivElement[]>([]);

  useEffect(() => {
    // if (props.viewIndex !== undefined) {
    //   viewRefList.current[props.viewIndex]?.scrollIntoView({ behavior: 'smooth' });
    // }
  }, [props.viewIndex]);

  return (
    <div className={styles.container}>
      {list.map((d, index) => (
        <div key={index} ref={(el) => (viewRefList.current[index] = el)}>
          {d.list?.map((item) => (
            <>
              <Markdown content={item.content} className={styles.content} />
              {Array.isArray(item.siteList) && item.siteList.length > 0 && (
                <div className={classNames(styles.siteList)}>
                  <div className={styles.siteListTitle}>Research websites</div>
                  <SiteList list={item.siteList} />
                </div>
              )}
            </>
          ))}
        </div>
      ))}
    </div>
  );
};

export default Process;
`},25780:function(h,n){n.Z=`import React from 'react';
import { IUrl } from '../type';
import styles from './index.module.less';
import SiteDetailList from '../SiteList/Detail';

interface IProps {
  list: IUrl[];
}

const ResearchSource: React.FC<IProps> = (props) => {
  const { list } = props;
  return (
    <div className={styles.container}>
      <div className={styles.title}>Searched {list.length} web pages</div>
      <div className={styles.content}>
        <SiteDetailList list={list} />
      </div>
    </div>
  );
};

export default ResearchSource;
`},38788:function(h,n){n.Z=`import React from 'react';
import { Markdown } from '@agentscope-ai/chat';
import { IResult } from '../type';
import styles from './index.module.less';

interface IProps {
  data: IResult;
}

const Result: React.FC<IProps> = (props) => {
  const { data } = props;
  return <Markdown content={data.content} className={styles.container} />
};

export default Result;
`},66322:function(h,n){n.Z=`import React from 'react';
import styles from './index.module.less';
import { SparkLinkLine, SparkRightLine } from '@agentscope-ai/icons';

interface IProps {
  count: number;
  onClick: () => void;
}

const More: React.FC<IProps> = ({ count, onClick }) => {
  return (
    <div className={styles.moreWrapper} onClick={onClick}>
      <div className={styles.more}>
        <SparkLinkLine className={styles.linkIcon} />
        <span className={styles.title}>Read More ({count})</span>
      </div>
      <SparkRightLine className={styles.rightIcon} />
    </div>
  );
};

export default More;
`},1330:function(h,n){n.Z=`.urlWrapper {
  cursor: pointer;
}

.urlWrapper,
.moreWrapper {
  position: relative;
  padding: 4px 8px;
  flex-grow: 0;
  align-self: inherit;
  background: var(--sps-color-fill-tertiary);
  border-radius: 99px;
}

.urlWrapper.simple {
  .description {
    display: none;
  }
}

.urlWrapper.detail {
  .title {
    font-weight: 500;
  }
}

.url,
.more {
  display: flex;
  align-items: center;
  gap: 8px;
}

.icon {
  flex: 0 0 auto;
  width: 16px;
  height: 16px;
  border: 1px solid var(--sps-color-border-secondary);
  border-radius: 99px;
  background: #FFFFFF;
  display: flex;
  align-items: center;
  justify-content: center;
}

.iconImage {
  width: 8px;
  height: 8px;
  object-fit: contain;
}

.title {
  font-size: 12px;
  line-height: 20px;
  color: var(--sps-color-text);
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
  font-family: 'claude clone';
}

.more {
  .title {
    font-weight: 500;
  }
}

.description {
  color: var(--sps-color-text-secondary);
  font-size: 12px;
  line-height: 20px;
}

.moreWrapper {
  display: flex;
  align-items: center;
  justify-content: space-between;
}

.linkIcon {
  color: var(--sps-color-text);
}

.rightIcon {
  color: var(--sps-color-text-secondary);
}`},3519:function(h,n){n.Z=`import React from 'react';
import { IUrl } from '../type';
import styles from './index.module.less';
import classNames from 'classnames';

interface ISiteItemProps {
  data: IUrl;
  type: 'detail' | 'simple';
}

const SiteItem: React.FC<ISiteItemProps> = ({ data, type = 'simple' }) => {
  return (
    <div
      className={classNames(styles.urlWrapper, styles[type])}
      onClick={() => {
        if (data.url) {
          window.open(data.url, '_blank');
        }
      }}
    >
      <div className={styles.url}>
        <div className={styles.icon}>
          {data.icon && <img src={data.icon} className={styles.iconImage} alt="" />}
        </div>
        <span className={styles.title}>{data.title}</span>
        <span className={styles.description}>{data.description}</span>
      </div>
    </div>
  );
};

export default SiteItem;
`},97867:function(h,n){n.Z=`.container {
  position: relative;
}

.content {
  display: flex;
  flex-direction: column;
  gap: 8px;
}`},4621:function(h,n){n.Z=`import React from 'react';
import { IUrl } from '../type';
import styles from './Detail.module.less';
import SiteItem from '../SiteItem';

interface IProps {
  list: IUrl[];
}

const SiteDetailList: React.FC<IProps> = ({ list }) => {
  return (
    <div className={styles.container}>
      <div className={styles.content}>
        {list.map((item) => (
          <SiteItem key={item.title} data={item} type="detail" />
        ))}
      </div>
    </div>
  );
};

export default SiteDetailList;
`},3109:function(h,n){n.Z=`import React, { useMemo, useState } from 'react';
import { IUrl } from '../type';
import styles from './index.module.less';
import SiteItem from '../SiteItem';
import More from '../SiteItem/More';
import { Button } from '@agentscope-ai/design';
import { SparkUpLine } from '@agentscope-ai/icons';

interface IProps {
  list: IUrl[];
}

const SiteList: React.FC<IProps> = ({ list }) => {
  const [expand, setExpand] = useState(false);

  const [currentList, restCount] = useMemo(() => {
    if (list.length <= 8 || expand) {
      return [list, 0];
    }

    return [list.slice(0, 7), list.length - 7];
  }, [list, expand]);

  return (
    <div className={styles.container}>
      <div className={styles.content}>
        {currentList.map((item) => (
          <SiteItem key={item.title} data={item} type="simple" />
        ))}
        {restCount > 0 && <More count={restCount} onClick={() => setExpand(true)} />}
      </div>
      {expand && (
        <Button
          size="small"
          type="textCompact"
          icon={<SparkUpLine />}
          onClick={() => setExpand(false)}
          className={styles.collapseButton}
        >
          Collapse
        </Button>
      )}
    </div>
  );
};

export default SiteList;
`},36144:function(h,n){n.Z=`import React from 'react';
import { Steps } from 'antd';
import { IStep } from '../type';
import styles from './index.module.less';
import { Tooltip } from '@agentscope-ai/design';
import { SparkCheckCircleLine, SparkLoadingLine } from '@agentscope-ai/icons';

interface StepsProps {
  // \u6B65\u9AA4\u5217\u8868
  list?: IStep[];
  onStepClick?: (index: number) => void;
}

const StepsList: React.FC<StepsProps> = ({ list = [], onStepClick }) => {
  return (
    <div className={styles.stepsContainer}>
      <Steps
        direction="vertical"
        size="small"
        items={list.map((step, index) => ({
          title: <Tooltip title={step.title}>{step.title}</Tooltip>,
          status:
            step.status === 'RUNNING' ? 'process' : step.status === 'FINISHED' ? 'finish' : 'wait',
          icon: step.status === 'RUNNING' ? <SparkLoadingLine className={styles.loading} /> : <SparkCheckCircleLine />,
          onClick: () => {
            // onStepClick?.(index);
          },
        }))}
      />
    </div>
  );
};

export default StepsList;
`},17812:function(h,n){n.Z=`import React, { useMemo, useState } from 'react';
import { IData, IParseData } from './type';
import styles from './index.module.less';
import ChatHeader from './Header/Chat';
import StepsList from './Steps';
import Process from './Process';
import classNames from 'classnames';
import Preview from './Preview';
import ReactDOM from 'react-dom';
import useFullscreen from '../../hooks/useFullscreen';

interface IProps {
  data: IData;
}

const DeepResearch: React.FC<IProps> = (props) => {
  const [preview, setPreview] = useFullscreen();
  const [viewIndex, setViewIndex] = useState(0);

  const cardCreateTime = useMemo(() => {
    return Date.now();
  }, []);

  const data: IParseData = useMemo(() => {
    const { steps = [], result, ...rest } = props.data?.data || {};

    return {
      steps: steps.map((step) => {
        return {
          ...step,
          startTime: parseInt(step.startTime || \`0\`),
          endTime: parseInt(step.endTime || \`0\`),
        };
      }),
      result: {
        ...result,
        startTime: parseInt(result?.startTime || \`0\`),
        endTime: parseInt(result?.endTime || \`0\`),
      },
      ...rest,
    };
  }, [props.data?.data]);

  const startTime = useMemo(() => {
    const time = Math.min(
      data.steps.reduce((acc, step) => {
        return Math.min(acc, step.startTime);
      }, Number.MAX_SAFE_INTEGER),
      data.result.startTime || Number.MAX_SAFE_INTEGER,
    );

    return time === Number.MAX_SAFE_INTEGER ? cardCreateTime : time;
  }, [data.steps, data.result, cardCreateTime]);

  const finished = useMemo(() => {
    return props.data.msgStatus === 'finished';
  }, [props.data.msgStatus]);

  return (
    <div
      className={classNames(styles.container, {
        [styles.thumbnail]: preview,
      })}
    >
      <ChatHeader
        className={styles.header}
        title={data.title}
        startTime={startTime}
        data={data}
        finished={finished}
        thumbnail={preview}
        onFullscreen={() => setPreview(true)}
      />
      <div className={styles.content}>
        <div className={styles.list}>
          <StepsList list={data.steps} onStepClick={setViewIndex} />
        </div>
        <div className={styles.view}>
          <Process list={data.steps} viewIndex={viewIndex} />
        </div>
      </div>
      {
        preview && ReactDOM.createPortal(<Preview data={data} />, document.getElementById('right-panel'))
      }
    </div>
  );
};

export default DeepResearch;
`},42153:function(h,n){n.Z=`export const formatDuration = (start: number, end: number) => {
  const durationInSeconds = Math.floor((end - start) / 1000);
  const hours = Math.floor(durationInSeconds / 3600);
  const minutes = Math.floor((durationInSeconds % 3600) / 60);
  const seconds = durationInSeconds % 60;

  const paddedMinutes = minutes.toString().padStart(2, '0');
  const paddedSeconds = seconds.toString().padStart(2, '0');

  if (hours > 0) {
    return \`\${paddedMinutes} minutes \${paddedSeconds} seconds\`;
  } else if (minutes > 0) {
    return \`\${paddedMinutes} minutes \${paddedSeconds} seconds\`;
  } else {
    return \`\${paddedSeconds} seconds\`;
  }
};
`},49756:function(h,n){n.Z=`import { SparkCopyLine, SparkReplaceLine, SparkThumbsDownLine, SparkThumbsUpLine } from "@agentscope-ai/icons";
import { DefaultCards } from "@agentscope-ai/chat";

export default function () {
  return <DefaultCards.FooterActions data={[
    {
      icon: <SparkReplaceLine />, label: '', onClick: () => { }
    },
    {
      icon: <SparkCopyLine />, label: '', onClick: () => { }
    },
    {
      icon: <SparkThumbsUpLine />, label: '', onClick: () => { }
    },
    {
      icon: <SparkThumbsDownLine />, label: '', onClick: () => { }
    }
  ]} />
}`},43167:function(h,n){n.Z=`import { SparkCode01Line, SparkExitFullscreenLine, SparkFullscreenLine, SparkPlayCircleFill } from '@agentscope-ai/icons';
import useStyle from './style';
import { IconButton, CodeBlock, Button } from '@agentscope-ai/design';
import { ReactNode, useContext, useEffect, useState } from 'react';
import ReactDOM from 'react-dom';
import RightPanel, { RightPanelContent, RightPanelHeader } from '../../components/RightPanel';
import useFullscreen from '../../hooks/useFullscreen';


export default function (props: { data: { title: string, content: string, loading?: boolean } }) {
  const [fullscreen, setFullscreen] = useFullscreen();
  const data = props.data;
  const { styles } = useStyle();

  const code = <CodeBlock value={data.content} language="html" readOnly />
  return <div className={styles.webDev}>
    <div className={styles.header}>
      <div className={styles.left}>
        <SparkCode01Line style={{ fontSize: 20 }} />
        <span className={styles.title}>
          {data.title}
        </span>
      </div>
      {
        fullscreen ?
          null :
          <IconButton onClick={() => setFullscreen(!fullscreen)} bordered={false} icon={<SparkFullscreenLine />} />
      }
    </div>

    {
      fullscreen ?
        ReactDOM.createPortal(<RightPanelWebDev code={code} title={data.title} loading={data.loading} content={data.content} />, document.getElementById('right-panel')) :
        <div className={styles.content}>
          {code}
        </div>
    }
  </div>
}

function RightPanelWebDev({ code, title, loading, content }: { code: ReactNode, title: string, loading?: boolean, content: string }) {
  const [preview, setPreview] = useState(false);
  const { styles } = useStyle();

  const contentNode = preview ? <iframe className={styles.iframe} srcDoc={content} /> : code;

  return <RightPanel
    header={<RightPanelHeader title={title} actions={
      <Button
        icon={<SparkPlayCircleFill />}
        type="primaryLess" loading={loading} onClick={() => {
          if (loading) return;
          setPreview(!preview);
        }}>{
          preview ? 'Exit' : 'Preview'
        }</Button>
    } />}
    content={<RightPanelContent>{contentNode}</RightPanelContent>}>
  </RightPanel>
}`},35625:function(h,n){n.Z=`import { createStyles } from 'antd-style';

export default createStyles(({ css, token }) => ({
  webDev: {
    width: '100%',
    backgroundColor: token.colorBgBase,
    border: \`1px solid \${token.colorBorderSecondary}\`,
    borderRadius: token.borderRadius,
    overflow: 'hidden',
  },

  header: {
    display: 'flex',
    justifyContent: 'space-between',
    alignItems: 'center',
    height: 48,
    padding: '0 16px',
  },
  left: {
    display: 'flex',
    alignItems: 'center',
  },

  title: {
    fontFamily: 'claude clone',
    fontSize: 14,
    marginLeft: 8,
    fontWeight: 600,
  },

  content: {},

  iframe: {
    border: 0,
    display: 'block',
    width: '100%',
    height: '100%',
  },
}));
`},69573:function(h,n){n.Z="export default `data:application/octet-stream;base64,`;\n"},8980:function(h,n){n.Z=`import { ChatAnywhere, ChatAnywhereRef, DefaultCards, TMessage, uuid } from '@agentscope-ai/chat';
import { useCallback, useContext, useEffect, useRef } from 'react';
import { SparkAttachmentLine, SparkCopyLine, SparkReplaceLine, SparkThumbsDownLine, SparkThumbsUpLine } from '@agentscope-ai/icons';
import ReactDOM from 'react-dom';
import Header from '../Header';
import Welcome from '../Welcome';
import ModeSelect from '../ModeSelect';
import OptionsSelect from '../OptionsSelect';
import PresetSelect from '../PresetSelect';
import { DemoContext } from '../../context/DemoProvider';
import { produce } from 'immer';
import mock from '../../mock';
import WebDev from '../../cards/WebDev';
import DeepResearch from '../../cards/DeepResearch';
import useStyle from './style';
import { RightPanelPortal } from '../RightPanel';
import Footer from '../../cards/Footer';

export default function Claude() {
  const { styles } = useStyle()
  const { demoContext, setDemoContext, getDemoContext } = useContext(DemoContext)
  const ref = useRef<ChatAnywhereRef>();
  const currentQA = useRef<{
    query?: Partial<TMessage> & { id: string },
    answer?: Partial<TMessage> & { id: string },
  }>({});


  const onSubmit = useCallback(async (data) => {
    ref.current.setLoading(true);

    setDemoContext({ initial: false });
    currentQA.current.answer = undefined;
    currentQA.current.query = {
      id: uuid(),
      cards: [{
        code: 'Text',
        data: {
          content: data.query,
          msgStatus: 'finished',
        }
      }],
      role: 'user',
      msgStatus: 'finished',
    };
    ReactDOM.flushSync(() => {
      ref.current.updateMessage(currentQA.current.query);
    });


    currentQA.current.answer = {
      id: uuid(),
      role: 'assistant',
      content: '',
      cards: [],
      msgStatus: 'generating',
    }

    ref.current.updateMessage(currentQA.current.answer);

    await mock({ getDemoContext, currentQA, ref });
    await sleep(100);

    onFinish('finished');
    ref.current.setLoading(false);
  }, []);

  const onFinish = useCallback((status: 'finished' | 'interrupted' = 'finished') => {
    const prevMessage = ref.current
      .getMessages()
      .find((item) => item.id === currentQA.current.answer.id);

    const nextMessage = produce(prevMessage, (draft) => {
      draft.msgStatus = status;
      (draft.cards || []).forEach(item => {
        item.data.msgStatus = status;
        item.data.loading = false;
      });


      draft.cards.push({
        code: 'Footer',
        data: {
          left: <Footer />
        }
      })
    });

    ref.current.updateMessage(nextMessage);

  }, []);

  const onStop = useCallback(() => { }, []);

  return <div className={styles.container}>
    <div className={styles.chat} style={{ width: demoContext.canvas ? 480 : '100%' }}>
      <ChatAnywhere
        cardConfig={{ WebDev, DeepResearch }}
        uiConfig={{
          disclaimer: 'AI can also make mistakes, so please check carefully and use it with caution',
          header: <Header chat={ref} />,
          welcome: <Welcome />,
        }}
        ref={ref}
        onInput={{
          zoomable: !demoContext.initial,
          morePrefixActions: <OptionsSelect />,
          beforeUI: <ModeSelect />,
          afterUI: <PresetSelect onSubmit={onSubmit} />,
          placeholder: 'please type here',
          onSubmit: onSubmit
        }}
        onStop={onStop}
        onUpload={[{
          multiple: false,
          icon: <SparkAttachmentLine />,
          customRequest(options) {
            options.onSuccess({
              url: URL.createObjectURL(options.file as Blob)
            });
          }
        }]}
      ></ChatAnywhere>
    </div>
    <RightPanelPortal />
  </div>
}



function sleep(ms: number) {
  return new Promise(resolve => setTimeout(resolve, ms));
}`},80885:function(h,n){n.Z=`import useStyle from './style';
import ModelSelect from '../ModelSelect';
import { IconButton } from '@agentscope-ai/design';
import { useContext } from 'react';
import { SparkClearLine, SparkMoonLine, SparkReplaceLine, SparkSunLine } from '@agentscope-ai/icons';
import { ConfigProviderContext } from '../../context/ConfigProvider';
import { ChatAnywhereRef } from '@agentscope-ai/chat';
import { Flex } from 'antd';

export default function (props: { chat: { current: ChatAnywhereRef } }) {
  const { styles } = useStyle();

  const { dark, setDark } = useContext(ConfigProviderContext);

  return <div className={styles.header}>
    <ModelSelect size='small' />
    <Flex gap={8}>
      <IconButton
        bordered={false}
        onClick={() => props.chat.current.removeAllMessages()}
        icon={<SparkClearLine />}
      />
      {/* <IconButton
        bordered={false}
        onClick={() => setDark(!dark)}
        icon={!dark ? <SparkSunLine /> : <SparkMoonLine />}
      /> */}
    </Flex>
  </div>
}`},18103:function(h,n){n.Z=`import { createStyles } from 'antd-style';

export default createStyles(({ css, token }) => ({
  header: css\`
    width: 100%;
    height: 64px;
    padding: 0 24px 0 24px;
    display: flex;
    align-items: center;
    justify-content: space-between;
  \`,
}));`},47820:function(h,n){n.Z=`import { SparkCode01Fill, SparkCode01Line, SparkPaperLine } from "@agentscope-ai/icons";
import { ChatInput } from "@agentscope-ai/chat";
import { useContext } from "react";
import { DemoContext } from "../../context/DemoProvider";

export default function () {
  const { demoContext, setDemoContext } = useContext(DemoContext);
  if (demoContext.initial && !demoContext.mode) return null;

  return <ChatInput.ModeSelect options={[
    {
      icon: <SparkCode01Line />,
      label: 'Web Development',
      value: 'WebDev',
    },
    {
      icon: <SparkPaperLine />,
      label: 'Deep Research',
      value: 'DeepResearch',
    },
  ]}
    value={demoContext.mode}
    onChange={mode => setDemoContext({ mode })}
  />
}`},41845:function(h,n){n.Z=`import { Dropdown } from '@agentscope-ai/design';
import React, { useContext } from 'react';
import useStyle from './styles';
import { SparkDownLine, SparkTrueLine } from '@agentscope-ai/icons';
import { DemoContext } from '../../context/DemoProvider';


const defaultImage =
  'https://gw.alicdn.com/imgextra/i1/O1CN01qYTgPn1Oaj6j7oc14_!!6000000001722-55-tps-24-25.svg';


const MODEL_OPTIONS = [
  {
    value: 'claude-3.5-sonnet',
    label: 'Claude 3.5 Sonnet',

  },
  {
    value: 'claude-4.1-Opus',
    label: 'Claude 4.1 Opus',

  },
];

export default function SimpleModelSelect(props: { size: 'small' | 'large' }) {
  const { size = 'large' } = props;
  const { styles, cx } = useStyle();
  const { demoContext, setDemoContext } = useContext(DemoContext);
  const selectedModel = MODEL_OPTIONS.find((item) => item.value === demoContext.model);

  return (
    <div>
      <Dropdown
        menu={{
          items: MODEL_OPTIONS.map(item => ({
            key: item.value,
            label: <Item {...item} onClick={() => {
              setDemoContext({ model: item.value })
            }} selected={demoContext.model === item.value} />,
          }))
        }}
      >
        <div className={cx(styles.display, styles[size])}>
          <img src={defaultImage} />
          {selectedModel?.label}
          <SparkDownLine />
        </div>
      </Dropdown>
    </div>
  );
}

interface ItemProps {
  value: string;
  label: string;
  selected: boolean;
  onClick: () => void;
}


function Item(props: ItemProps) {
  const { styles } = useStyle();

  return (
    <div onClick={() => props.onClick()} className={styles.item}>
      <div className={styles.itemIcon}>
        <img src={defaultImage} width={14} height={14} />
      </div>


      <div className={styles.itemTitle}>{props.label}</div>
      {props.selected && <SparkTrueLine />}
    </div>
  );
}

`},41129:function(h,n){n.Z=`import { createStyles } from 'antd-style';

export default createStyles(({ css, token }) => ({
  display: {
    fontFamily: 'claude clone',
    display: 'flex',
    alignItems: 'center',
    gap: 8,
    cursor: 'pointer',
    color: token.colorText,
  },

  small: {
    fontSize: 14,
    height: 20,

    img: {
      width: 20,
      height: 20,
      padding: 2.5,
      background: token.colorPrimaryBg,
      borderRadius: 4,
    },
  },
  large: {
    fontSize: 20,
    height: 40,

    img: {
      width: 24,
      height: 24,
    },
  },

  item: {
    display: 'flex',
    alignItems: 'center',
    gap: 8,
  },
  itemIcon: {
    width: 20,
    height: 20,
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 4,
  },

  itemTitle: {
    fontFamily: 'claude clone',
  },
}));
`},60062:function(h,n){n.Z=`import { Button, ButtonProps } from '@agentscope-ai/design'
import { DemoContext } from '../../context/DemoProvider'
import { useContext } from 'react'
import { SparkDeepSearchLine, SparkDeepThinkLine } from '@agentscope-ai/icons'
import { createStyles } from 'antd-style'

export default function () {
  const { demoContext, setDemoContext } = useContext(DemoContext)

  return <>
    <OptionButton
      icon={<SparkDeepThinkLine />}
      selected={demoContext.enableThinking}
      onClick={() => setDemoContext({ enableThinking: !demoContext.enableThinking })}>Deep Thinking</OptionButton>
    <OptionButton
      selected={demoContext.enableSearch}
      icon={<SparkDeepSearchLine />}
      onClick={() => setDemoContext({ enableSearch: !demoContext.enableSearch })}>Web Search</OptionButton>
  </>
}

function OptionButton(props: ButtonProps & { selected: boolean }) {
  const { styles, cx } = useStyle();

  return <Button {...props}
    className={
      cx(styles.btn, {
        [styles.selected]: props.selected,
      })
    } />
}

const useStyle = createStyles(({ token }) => {
  return {
    btn: {
      gap: 6,

    },
    selected: {
      color: \`\${token.colorPrimary} !important\`,
      backgroundColor: \`\${token.colorPrimaryBg} !important\`,
      borderColor: \`\${token.colorPrimaryBg} !important\`,
    }
  }
});`},42038:function(h,n){n.Z=`import { SparkUpperrightArrowLine } from "@agentscope-ai/icons"
import { Skeleton } from "antd"
import { createStyles } from "antd-style"


const OPTIONS = [
  {
    prompt: 'deep research on the topic of city pop',
  },
  {
    prompt: 'deep research on the topic of city pop',
  },
  {
    prompt: 'deep research on the topic of city pop',
  },
]

export default function (props: { onSubmit: (data: { query: string }) => void }) {
  const { styles } = useStyle()

  return <div className={styles.preset}>
    {OPTIONS.map(item => (
      <div key={item.prompt} className={styles.presetItem} onClick={() => props.onSubmit({ query: item.prompt })}>
        <div className={styles.children}>
          <img width={24} height={24} src="https://gw.alicdn.com/imgextra/i1/O1CN01K7jgEj1sywWTkPSGY_!!6000000005836-55-tps-40-40.svg" />
          <div className={styles.prompt}>{item.prompt}</div>
          <div className={styles.skeletonContainer}>
            <div className={styles.skeletonLine} />
            <div className={styles.skeletonLine} />
            <div className={styles.skeletonShortLine} />
          </div>
        </div>
        <div className={styles.content}>
          <div className={styles.title}>{item.prompt}</div>
          <SparkUpperrightArrowLine />
        </div>
      </div>
    ))}
  </div>
}

const useStyle = createStyles(({ token, css, cx }) => {
  const content = cx(css\`
    position: absolute;
    bottom: 0;
    left: 0;
    right: 0;
    background-color: \${token.colorBgBase};
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 12px;
    height: 48px;
    padding: 0 12px;
    border-top: 1px solid \${token.colorBorderSecondary};
    opacity: 0;
    transition: all 0.3s ease;
  \`);


  return {
    preset: {
      marginTop: 32,
      display: 'flex',
      justifyContent: 'center',
      gap: 12,
    },
    presetItem: css\`
      position: relative;
      display: flex;
      border: 1px solid \${token.colorBorderSecondary};
      border-radius: 8px;
      height: 200px;
      flex: 1;
      cursor: pointer;
      overflow: hidden;
      padding: 24px;
      background-color: \${token.colorFillTertiary};
      &:hover {
        border-color: \${token.colorPrimary};

        .\${content} {
          opacity: 1;
        }
      }
    \`,
    children: {
      display: 'flex',
      flexDirection: 'column',
      gap: 10,
      height: '100%',
      width: '100%',
      backgroundColor: token.colorBgBase,
      borderRadius: 8,
      padding: '20px 12px',
    },
    prompt: css\`
      font-size: 12px;
      line-height: 20px;
      font-weight: 500;
      color: \${token.colorText};
      text-overflow: ellipsis;
      overflow: hidden;
      display: -webkit-box;
      -webkit-line-clamp: 1;
      -webkit-box-orient: vertical;
    \`,
    content,
    title: css\`
      font-size: 14px;
      color: \${token.colorText};
      text-overflow: ellipsis;
      overflow: hidden;
      white-space: nowrap;
    \`,
    skeletonContainer: css\`
      display: flex;
      flex-direction: column;
      gap: 8px;
    \`,
    skeletonLine: css\`
      width: 100%;
      height: 8px;
      background-color: \${token.colorFillTertiary};
      border-radius: 99px;
    \`,
    skeletonShortLine: css\`
      width: 50%;
      height: 8px;
      background-color: \${token.colorFillTertiary};
      border-radius: 99px;
    \`,
  }
})`},43659:function(h,n){n.Z=`import { SparkUpperrightArrowLine } from "@agentscope-ai/icons"
import { createStyles } from "antd-style"


const WEB_DEV_PRESET_OPTIONS = [
  {
    prompt: 'write a landing page for a fruit shop',
    image: 'https://gw.alicdn.com/imgextra/i1/O1CN01ObcQyY1vyQveMjl76_!!6000000006241-55-tps-390-200.svg',

  },
  {
    prompt: 'write a portfolio website',
    image: 'https://gw.alicdn.com/imgextra/i2/O1CN01NciUQw1tzgmhPLmfF_!!6000000005973-55-tps-390-200.svg',
  }
]

export default function (props: { onSubmit: (data: { query: string }) => void }) {
  const { styles } = useStyle()

  return <div className={styles.preset}>
    {WEB_DEV_PRESET_OPTIONS.map(item => (
      <div key={item.prompt} className={styles.presetItem} onClick={() => props.onSubmit({ query: item.prompt })}>
        <img src={item.image} alt={item.prompt} />
        <div className={styles.content}>
          <div className={styles.title}>{item.prompt}</div>
          <SparkUpperrightArrowLine />
        </div>
      </div>
    ))}
  </div>
}

const useStyle = createStyles(({ token, css, cx }) => {
  const content = cx(css\`
    position: absolute;
    bottom: 0;
    left: 0;
    right: 0;
    background-color: \${token.colorBgBase};
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 12px;
    height: 48px;
    border-top: 1px solid \${token.colorBorderSecondary};
    opacity: 0;
    transition: all 0.3s ease;
  \`);


  return {
    preset: {
      marginTop: 32,
      display: 'flex',
      justifyContent: 'center',
      gap: 12,
    },
    presetItem: css\`
      position: relative;
      display: flex;
      border: 1px solid \${token.colorBorderSecondary};
      border-radius: 8px;
      cursor: pointer;
      overflow: hidden;
      &:hover {
        border-color: \${token.colorPrimary};

        .\${content} {
          opacity: 1;
        }
      }
    \`,
    content,
    title: css\`
      font-size: 14px;
      color: \${token.colorText};
    \`,
  }
})`},13434:function(h,n){n.Z=`import { useContext } from "react";
import { DemoContext } from "../../context/DemoProvider";
import { SparkAiEditLine, SparkCode01Line, SparkEffciencyLine, SparkPaperLine } from "@agentscope-ai/icons";
import { Button } from "@agentscope-ai/design";
import { createStyles } from "antd-style";
import { ChatAnywhereRef } from "@agentscope-ai/chat";
import WebDevPreset from "./WebDevPreset";
import DeepResearchPreset from "./DeepResearchPreset";

const PRESET_OPTIONS = [
  {
    icon: <SparkCode01Line />,
    label: 'Web Development',
    value: 'WebDev',
    type: 'mode',
  },
  {
    icon: <SparkPaperLine />,
    label: 'Deep Research',
    value: 'DeepResearch',
    type: 'mode',
  },
  {
    icon: <SparkAiEditLine />,
    label: 'Write Article',
    value: 'Write Article',
    type: 'quick',
  },

  {
    icon: <SparkEffciencyLine />,
    label: 'Brainstorm',
    value: 'Brainstorm',
    type: 'quick',
  },
]

export default function (props: { onSubmit: (data: { query: string }) => void }) {
  const { demoContext, setDemoContext } = useContext(DemoContext);
  const { styles } = useStyle()

  if (!demoContext.initial) return null;

  const mode = demoContext.mode;

  if (mode === 'WebDev') return <WebDevPreset onSubmit={props.onSubmit} />;
  if (mode === 'DeepResearch') return <DeepResearchPreset onSubmit={props.onSubmit} />;


  return <div className={styles.preset}>
    {PRESET_OPTIONS.map(item => (
      <div key={item.value}>
        <Button icon={item.icon} onClick={() => {
          if (item.type === 'mode') {
            setDemoContext({ mode: item.value })
          } else {
            props.onSubmit({ query: item.value })
          }
        }}>{item.label}</Button>
      </div>
    ))}
  </div>
}


const useStyle = createStyles(({ token }) => {
  return {
    preset: {
      marginTop: 32,
      display: 'flex',
      justifyContent: 'center',
      gap: 12,
    }
  }
})`},72595:function(h,n){n.Z=`import { ReactNode, useContext, useEffect } from "react"
import useStyle from "./style"
import { DemoContext } from "../../context/DemoProvider";
import { IconButton } from "@agentscope-ai/design";
import { SparkFalseLine } from "@agentscope-ai/icons";

export default function RightPanel(props: { header: ReactNode, content: ReactNode }) {
  const { styles } = useStyle();
  const { demoContext, setDemoContext, getDemoContext } = useContext(DemoContext)
  const { header, content } = props;

  useEffect(() => {
    setDemoContext({ canvas: true });
    return () => {
      setDemoContext({ canvas: false });
    }
  }, []);

  return <div className={styles.rightPanel}>
    {header}
    {content}
  </div>
}

export const RightPanelContent = (props: { children: ReactNode }) => {
  const { styles } = useStyle();
  return <div className={styles.content}>
    {props.children}
  </div>
}

export const RightPanelHeader = (props: { title: string, actions: ReactNode }) => {
  const { demoContext, setDemoContext, getDemoContext } = useContext(DemoContext)
  const { styles } = useStyle();
  return <div className={styles.header}>
    <div className={styles.headerLeft}>
      <IconButton bordered={false} icon={<SparkFalseLine />} onClick={() => {
        setDemoContext({ canvas: false });
      }} />
      <span className={styles.title}>{props.title}</span>
    </div>
    <div className={styles.headerRight}>
      {props.actions}

    </div>

  </div>
}

export const useRightPanel = () => { }

export const RightPanelPortal = () => {
  return <div id="right-panel" style={{ flex: 1, width: 0 }}>

  </div>
} `},51966:function(h,n){n.Z=`import { createGlobalStyle, createStyles } from 'antd-style';
import ModelSelect from '../ModelSelect';

export default function () {
  const { styles } = useStyle();

  return <>
    <Style />
    <div className={styles.welcome}>
      <div className={styles.h1}>Welcome to LLMs Experience</div>
      <div className={styles.h2}>Start with <ModelSelect size="large" /></div>
    </div>
  </>
}

const useStyle = createStyles(({ css, token }) => ({
  welcome: {
    display: 'flex',
    flexDirection: 'column',
    alignItems: 'center',
    gap: 4,
    marginTop: 'calc(10vh + 80px + 64px)',
    marginBottom: 20,

  },
  h1: {
    fontFamily: 'claude clone',
    fontSize: 32,
    lineHeight: '48px',
    fontWeight: '500',
  },
  h2: {
    fontFamily: 'claude clone',
    display: 'flex',
    justifyContent: 'center',
    alignItems: 'center',
    gap: 16,
    fontSize: 20,
    lineHeight: '32px',
    color: token.colorTextSecondary,
  },
}));



const Style = createGlobalStyle\`
.sps-chat-anywhere-chat-welcome {
  height: auto !important;  
}\`



`},40047:function(h,n){n.Z=`import { ConfigProvider } from '@agentscope-ai/design';
import { darkTheme, lightTheme } from './theme';
import { useState } from 'react';
import React from 'react';


export const ConfigProviderContext = React.createContext<{
  dark?: boolean;
  setDark: (dark: boolean) => void;
}>(undefined);

export default function (props) {
  const [dark, setDark] = useState(false);
  const currentTheme = dark ? darkTheme : lightTheme;

  return <>
    <ConfigProviderContext.Provider value={{
      dark,
      setDark,
    }}>
      <ConfigProvider
        {...currentTheme}
        style={{ height: '100%' }}
        iconfont='https://at.alicdn.com/t/a/font_4807885_6wjcpzayws6.js'
        prefix='sps'
        prefixCls="sps"
      >
        {props.children}
      </ConfigProvider>
    </ConfigProviderContext.Provider>
  </>;
}`},81677:function(h,n){n.Z=`import { useGetState } from 'ahooks';
import React, { useState } from 'react';


type TContext = {
  model: string;
  enableSearch: boolean;
  enableThinking: boolean;
  mode?: string;
  initial?: boolean;
  canvas: boolean;
}

export const DemoContext = React.createContext<{
  demoContext: TContext,
  setDemoContext: (data: Partial<TContext>) => void,
  getDemoContext: () => TContext
}>(undefined);




export default function (props: { children: React.ReactNode }) {
  const [demoContext, setDemoContext, getDemoContext] = useGetState<TContext>({
    model: 'claude-3.5-sonnet',
    enableSearch: false,
    enableThinking: false,
    initial: true,
    mode: undefined,
    canvas: false,
  });


  return <DemoContext.Provider value={{
    demoContext, setDemoContext(data) {
      setDemoContext(v => {
        return { ...v, ...data }
      })
    }, getDemoContext
  }}>
    {props.children}
  </DemoContext.Provider>
}`},27528:function(h,n){n.Z=`import { generateThemeByToken } from '@agentscope-ai/design';

const lightTokens = {
  borderRadiusXS: 4,
  borderRadiusSM: 6,
  borderRadius: 8,
  borderRadiusLG: 12,
  borderRadiusXL: 16,
  borderRadiusFull: 999,
  wireframe: false,
  colorPrimaryBg: 'rgba(239, 237, 229, 1)',
  colorPrimaryBgHover: 'rgba(239, 237, 229, 1)',
  colorPrimaryBorder: 'rgba(186, 90, 59, 1)',
  colorPrimaryBorderHover: 'rgba(194, 98, 66, 1)',
  colorPrimaryHover: 'rgba(194, 98, 66, 1)',
  colorPrimary: 'rgba(208, 110, 78, 1)',
  colorPrimaryActive: 'rgba(194, 98, 66, 1)',
  colorPrimaryTextHover: 'rgba(186, 90, 59, 1)',
  colorPrimaryText: 'rgba(186, 90, 59, 1)',
  colorPrimaryTextActive: 'rgba(186, 90, 59, 1)',
  colorTextBase: 'rgba(26, 26, 29, 1)',
  colorBgBase: 'rgba(255, 255, 255, 1)',
  colorText: 'rgba(20, 20, 19, 0.88)',
  colorTextSecondary: 'rgba(20, 20, 19, 0.65)',
  colorTextTertiary: 'rgba(20, 20, 19, 0.45)',
  colorTextQuaternary: 'rgba(20, 20, 19, 0.25)',
  colorBorder: 'rgba(193, 192, 187, 1)',
  colorBorderSecondary: 'rgba(234, 233, 231, 1)',
  colorFill: 'rgba(43, 27, 0, 0.12)',
  colorFillSecondary: 'rgba(43, 27, 0, 0.08)',
  colorFillTertiary: 'rgba(43, 27, 0, 0.03)',
  colorFillQuaternary: 'rgba(43, 27, 0, 0.02)',
  colorBgContainer: 'rgba(255, 255, 255, 1)',
  colorBgElevated: 'rgba(255, 255, 255, 1)',
  colorBgLayout: 'rgba(249, 248, 244, 1)',
  colorBgSpotlight: 'rgba(20, 20, 19, 0.85)',
  colorBgMask: 'rgba(0, 0, 0, 0.45)',
  colorInfo: 'rgba(11, 131, 241, 1)',
  colorInfoHover: 'rgba(94, 188, 255, 1)',
  colorInfoBorder: 'rgba(135, 209, 255, 1)',
  colorInfoText: 'rgba(11, 131, 241, 1)',
  colorInfoBg: 'rgba(230, 247, 255, 1)',
  colorInfoBgHover: 'rgba(176, 227, 255, 1)',
  colorInfoBorderHover: 'rgba(94, 188, 255, 1)',
  colorSuccess: 'rgba(91, 185, 139, 1)',
  colorSuccessHover: 'rgba(171, 212, 190, 1)',
  colorSuccessBg: 'rgba(235, 250, 240, 1)',
  colorSuccessBgHover: 'rgba(223, 237, 228, 1)',
  colorSuccessBorder: 'rgba(211, 224, 217, 1)',
  colorSuccessBorderHover: 'rgba(171, 212, 190, 1)',
  colorWarning: 'rgba(250, 173, 20, 1)',
  colorWarningHover: 'rgba(255, 214, 102, 1)',
  colorWarningBg: 'rgba(255, 251, 230, 1)',
  colorWarningBgHover: 'rgba(255, 241, 184, 1)',
  colorWarningBorder: 'rgba(255, 229, 143, 1)',
  colorWarningBorderHover: 'rgba(255, 214, 102, 1)',
  colorError: 'rgba(255, 77, 79, 1)',
  colorErrorHover: 'rgba(255, 120, 117, 1)',
  colorErrorBg: 'rgba(255, 242, 240, 1)',
  colorErrorBgHover: 'rgba(255, 241, 240, 1)',
  colorErrorBorder: 'rgba(255, 204, 199, 1)',
  colorErrorBorderHover: 'rgba(255, 163, 158, 1)',
  colorLink: 'rgba(0, 77, 255, 1)',
  boxShadow: '0px 4px 6px 0px rgba(0, 0, 0, 0.08)',
  boxShadowSecondary:
    '0px 12px 24px -16px rgba(0, 0, 0, 0.06), 0px 8px 40px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)',
  colorTextWhite: 'rgba(255, 255, 255, 1)',
  colorFillDisable: 'rgba(235, 235, 232, 1)',
  colorPurple: 'rgba(97, 92, 237, 1)',
  colorPurpleHover: 'rgba(97, 92, 237, 1)',
  colorPurpleBg: 'rgba(226, 222, 255, 0.8)',
  colorPink: 'rgba(236, 72, 153, 1)',
  colorPinkHover: 'rgba(236, 72, 153, 1)',
  colorPinkBg: 'rgba(251, 220, 239, 0.8)',
  colorYellow: 'rgba(234, 179, 8, 1)',
  colorYellowHover: 'rgba(234, 179, 8, 1)',
  colorYellowBg: 'rgba(255, 250, 184, 0.8)',
  colorOrange: 'rgba(250, 129, 37, 1)',
  colorOrangeHover: 'rgba(250, 129, 37, 1)',
  colorOrangeBg: 'rgba(255, 232, 201, 0.8)',
  colorTeal: 'rgba(20, 184, 166, 1)',
  colorTealHover: 'rgba(20, 184, 166, 1)',
  colorTealBg: 'rgba(200, 244, 233, 0.8)',
  colorBlue: 'rgba(0, 144, 255, 1)',
  colorBlueHover: 'rgba(0, 144, 255, 1)',
  colorBlueBg: 'rgba(194, 229, 255, 0.8)',
  colorMauve: 'rgba(142, 140, 153, 1)',
  colorMauveHover: 'rgba(142, 140, 153, 1)',
  colorMauveBg: 'rgba(239, 240, 243, 0.8)',
  boxShadowTertiary:
    '0px 12px 32px -16px rgba(0, 0, 0, 0.24), 0px 12px 60px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)',
  boxShadowTertiaryLeft:
    '-12px 0px 32px -16px rgba(0, 0, 0, 0.24), -12px 0px 60px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)',
  boxShadowInput:
    '0px 12px 24px -16px rgba(54, 54, 73, 0.04), 0px 12px 40px 0px rgba(51, 51, 71, 0.08), 0px 0px 1px 0px rgba(44, 44, 54, 0.02)',
};
const darkTokens = {
  borderRadiusXS: 4,
  borderRadiusSM: 6,
  borderRadius: 8,
  borderRadiusLG: 12,
  borderRadiusXL: 16,
  borderRadiusFull: 999,
  wireframe: false,
  colorPrimaryBg: 'rgba(41, 35, 29, 1)',
  colorPrimaryBgHover: 'rgba(70, 60, 50, 1)',
  colorPrimaryBorder: 'rgba(133, 80, 40, 1)',
  colorPrimaryBorderHover: 'rgba(146, 102, 52, 1)',
  colorPrimaryHover: 'rgba(156, 88, 57, 1)',
  colorPrimary: 'rgba(156, 88, 57, 1)',
  colorPrimaryActive: 'rgba(146, 102, 52, 1)',
  colorPrimaryTextHover: 'rgba(133, 80, 40, 1)',
  colorPrimaryText: 'rgba(160, 96, 63, 1)',
  colorPrimaryTextActive: 'rgba(133, 80, 40, 1)',
  colorTextBase: 'rgba(230, 230, 230, 1)',
  colorBgBase: 'rgba(15, 13, 12, 1)',
  colorText: 'rgba(230, 230, 230, 0.9)',
  colorTextSecondary: 'rgba(180, 180, 180, 0.8)',
  colorTextTertiary: 'rgba(255, 255, 255, 0.6)',
  colorTextQuaternary: 'rgba(255, 255, 255, 0.3)',
  colorBorder: 'rgba(82, 76, 73, 1)',
  colorBorderSecondary: 'rgba(50, 46, 43, 1)',
  colorFill: 'rgba(82, 79, 74, 0.32)',
  colorFillSecondary: 'rgba(83, 79, 72, 0.24)',
  colorFillTertiary: 'rgba(50, 50, 50, 0.26)',
  colorFillQuaternary: 'rgba(67, 64, 59, 0.12)',
  colorBgContainer: 'rgba(52, 50, 47, 1)',
  colorBgElevated: 'rgba(47, 42, 39, 1)',
  colorBgLayout: 'rgba(26, 26, 24, 1)',
  colorBgSpotlight: 'rgba(10, 10, 10, 0.85)',
  colorBgMask: 'rgba(0, 0, 0, 0.6)',
  colorInfo: 'rgba(11, 131, 241, 1)',
  colorInfoHover: 'rgba(94, 188, 255, 1)',
  colorInfoBorder: 'rgba(57, 144, 201, 1)',
  colorInfoText: 'rgba(11, 131, 241, 1)',
  colorInfoBg: 'rgba(17, 34, 51, 1)',
  colorInfoBgHover: 'rgba(38, 72, 102, 1)',
  colorInfoBorderHover: 'rgba(94, 188, 255, 1)',
  colorSuccess: 'rgba(91, 185, 139, 1)',
  colorSuccessHover: 'rgba(116, 164, 124, 1)',
  colorSuccessBg: 'rgba(41, 54, 49, 1)',
  colorSuccessBgHover: 'rgba(63, 81, 71, 1)',
  colorSuccessBorder: 'rgba(115, 163, 119, 1)',
  colorSuccessBorderHover: 'rgba(171, 212, 190, 1)',
  colorWarning: 'rgba(250, 173, 20, 1)',
  colorWarningHover: 'rgba(255, 140, 40, 1)',
  colorWarningBg: 'rgba(45, 35, 26, 1)',
  colorWarningBgHover: 'rgba(77, 59, 35, 1)',
  colorWarningBorder: 'rgba(255, 166, 40, 1)',
  colorWarningBorderHover: 'rgba(255, 214, 102, 1)',
  colorError: 'rgba(255, 77, 79, 1)',
  colorErrorHover: 'rgba(238, 101, 78, 1)',
  colorErrorBg: 'rgba(43, 11, 8, 1)',
  colorErrorBgHover: 'rgba(74, 42, 40, 1)',
  colorErrorBorder: 'rgba(238, 91, 78, 1)',
  colorErrorBorderHover: 'rgba(255, 163, 158, 1)',
  colorLink: 'rgba(77, 125, 255, 1)',
  colorTextWhite: 'rgba(255, 255, 255, 1)',
  colorFillDisable: 'rgba(90, 90, 90, 1)',
  colorPurple: 'rgba(97, 92, 237, 1)',
  colorPurpleHover: 'rgba(97, 92, 237, 1)',
  colorPurpleBg: 'rgba(60, 53, 85, 1)',
  colorPink: 'rgba(236, 72, 153, 1)',
  colorPinkHover: 'rgba(236, 72, 153, 1)',
  colorPinkBg: 'rgba(74, 42, 58, 1)',
  colorYellow: 'rgba(234, 179, 8, 1)',
  colorYellowHover: 'rgba(234, 179, 8, 1)',
  colorYellowBg: 'rgba(77, 71, 32, 1)',
  colorOrange: 'rgba(250, 129, 37, 1)',
  colorOrangeHover: 'rgba(250, 129, 37, 1)',
  colorOrangeBg: 'rgba(255, 232, 201, 0.8)',
  colorTeal: 'rgba(20, 184, 166, 1)',
  colorTealHover: 'rgba(20, 184, 166, 1)',
  colorTealBg: 'rgba(42, 70, 64, 1)',
  colorBlue: 'rgba(0, 144, 255, 1)',
  colorBlueHover: 'rgba(0, 144, 255, 1)',
  colorBlueBg: 'rgba(42, 59, 78, 0.8)',
  colorMauve: 'rgba(142, 140, 153, 1)',
  colorMauveHover: 'rgba(142, 140, 153, 1)',
  colorMauveBg: 'rgba(46, 46, 50, 0.8)',
  boxShadow: '0px 4px 6px 0px rgba(255, 255, 255, 0.08)',
  boxShadowSecondary:
    '0px 12px 24px -16px rgba(255, 255, 255, 0.04), 0px 8px 40px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.03)',
  boxShadowTertiary:
    '0px 12px 32px -16px rgba(255, 255, 255, 0.16), 0px 12px 60px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.04)',
  boxShadowTertiaryLeft:
    '-12px 0px 32px -16px rgba(255, 255, 255, 0.16), -12px 0px 60px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.04)',
  boxShadowInput:
    '0px 12px 24px -16px rgba(254, 254, 255, 0.04), 0px 12px 40px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.02)',
};

// @ts-ignore
export const darkTheme = generateThemeByToken(darkTokens, true);
// @ts-ignore
export const lightTheme = generateThemeByToken(lightTokens, false);
`},61553:function(h,n){n.Z=`import { useContext, useEffect, useRef, useState } from "react";
import { DemoContext } from "../context/DemoProvider";
import { useUnmount } from 'ahooks';

const fullScreenMap = {};
function useFullscreen(id?: string) {

  const idRef = useRef(id || Math.random().toString());
  const { demoContext } = useContext(DemoContext)
  const [fullscreen, setFullscreen] = useState(fullScreenMap[idRef.current]);


  useEffect(() => {
    fullScreenMap[idRef.current] = [fullscreen, setFullscreen];
  }, [fullscreen, setFullscreen])


  useUnmount(() => {
    delete fullScreenMap[idRef.current];
  })


  useEffect(() => {
    if (!demoContext.canvas) {
      for (const key in fullScreenMap) {
        fullScreenMap[key][0] = false;
        fullScreenMap[key][1](false);
      }
    }
  }, [demoContext.canvas])

  return [fullscreen, (value) => {
    for (const key in fullScreenMap) {
      fullScreenMap[key][0] = false;
      fullScreenMap[key][1](false);
    }

    fullScreenMap[idRef.current] = value;
    setFullscreen(value);
  }]
}


export default useFullscreen;`},3491:function(h,n){n.Z=`import Chat from './components/Chat';
import ConfigProvider from './context/ConfigProvider';
import DemoProvider from './context/DemoProvider';
import Style from './style';

export default function () {

  return <ConfigProvider>
    <Style />
    <DemoProvider>
      <Chat />
    </DemoProvider>
  </ConfigProvider>
}`},55397:function(h,n){n.Z=`import { produce } from 'immer';
import deepResearchContent from './deepResearchContent';
import sleep from './sleep';

export default async function (currentQA, chatRef) {
  await sleep(1000);

  const prevMessage = chatRef.current
    .getMessages()
    .find((item) => item.id === currentQA.current.answer.id);

  const nextMessage = produce(prevMessage, (draft) => {
    draft.cards.push({
      code: 'Text',
      data: {
        content: 'I am conducting in-depth research on the topic you provided.',
        msgStatus: 'finished',
        className: 'claude-clone-font',
      },
    });

    draft.cards.push({
      code: 'DeepResearch',
      data: {
        data: {
          title: 'loading...',
          steps: [],
        },
      },
    });
  });

  chatRef.current.updateMessage(nextMessage);
  await sleep(100);

  for (const chunk of mock()) {
    await sleep(1000);

    const prevMessage = chatRef.current
      .getMessages()
      .find((item) => item.id === currentQA.current.answer.id);

    const nextMessage = produce(prevMessage, (draft) => {
      draft.cards[1].data.data = chunk;
    });

    chatRef.current.updateMessage(nextMessage);
  }
}

function mock() {
  const steps = [
    {
      title: deepResearchContent.title,
      steps: [],
    },
    {
      title: deepResearchContent.title,
      steps: deepResearchContent.steps.map((item) => ({
        ...item,
        status: 'RUNNING',
      })),
    },
    {
      title: deepResearchContent.title,
      steps: deepResearchContent.steps.map((item, index) => {
        if (index > 0) return { ...item, status: 'RUNNING' };
        return { ...item, status: 'FINISHED' };
      }),
    },
    {
      title: deepResearchContent.title,
      steps: deepResearchContent.steps.map((item, index) => {
        if (index > 1) return { ...item, status: 'RUNNING' };
        return { ...item, status: 'FINISHED' };
      }),
    },
    {
      title: deepResearchContent.title,
      steps: deepResearchContent.steps.map((item, index) => {
        return { ...item, status: 'FINISHED' };
      }),
    },
    deepResearchContent,
  ];

  return steps;
}
`},90108:function(h,n){n.Z=`export default {
  title: 'Free AI tool research',
  steps: [
    {
      title: 'Exploring free AI tools',
      status: 'FINISHED',
      startTime: '2024-01-15T09:00:00Z',
      endTime: '2024-01-15T10:30:00Z',
      list: [
        {
          content: \`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.\`,
          siteList: [
            {
              title: 'City Pop - Wikipedia',
            },
            {
              title: 'City Pop - Wikipedia',
            },
            {
              title: 'City Pop - Wikipedia',
            },
            {
              title: 'City Pop - Wikipedia',
            },
            {
              title: 'City Pop - Wikipedia',
            },
            {
              title: 'City Pop - Wikipedia',
            },
            {
              title: 'City Pop - Wikipedia',
            },
            {
              title: 'City Pop - Wikipedia',
            },
            {
              title: 'City Pop - Wikipedia',
            },
            {
              title: 'City Pop - Wikipedia',
            },
          ],
        },
      ],
    },
    {
      title: 'Considering search strategies',
      status: 'FINISHED',
      startTime: '2024-01-15T10:30:00Z',
      endTime: '2024-01-15T12:00:00Z',
      list: [
        {
          content: \`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics.\`,
          siteList: [
            {
              title: 'City Pop - Wikipedia',
            },
          ],
        },
      ],
    },
    {
      title: 'Optimizing discovery methods',
      status: 'FINISHED',
      startTime: '2024-01-15T12:00:00Z',
      endTime: '2024-01-15T13:30:00Z',
      list: [
        {
          content: \`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics.\`,
        },
      ],
    },
  ],
  site: {
    list: [
      {
        title: 'City Pop - Wikipedia',
        description: 'City Pop - Wikipedia',
      },
      {
        title: 'City Pop - Wikipedia',
        description: 'City Pop - Wikipedia',
      },
      {
        title: 'City Pop - Wikipedia',
        description: 'City Pop - Wikipedia',
      },
    ],
  },
  result: {
    fileKey: 'deep_research_ai_healthcare_20240115',
    startTime: '2024-01-15T09:00:00Z',
    endTime: '2024-01-15T15:00:00Z',
    status: 'FINISHED',
    content: \`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.\`,
  },
};
`},63390:function(h,n){n.Z=`import thinkingAnswer from "./thinkingAnswer";
import normalAnswer from "./normalAnswer";
import webDevAnswer from "./webDevAnswer";
import deepResearchAnswer from "./deepResearchAnswer";

export default async function ({ getDemoContext, currentQA, ref }) {
  const context = getDemoContext();
  if (context.mode === 'WebDev') return await webDevAnswer(currentQA, ref);
  if (context.mode === 'DeepResearch') return await deepResearchAnswer(currentQA, ref);
  if (context.enableThinking) return await thinkingAnswer(currentQA, ref);
  return await normalAnswer(currentQA, ref);
}`},78063:function(h,n){n.Z=`import { produce } from 'immer';
import sleep from './sleep';

const mock = () => {
  let text = \`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.

City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.
\`;

  const res = [];

  while (text.length > 0) {
    const step = Math.floor(Math.random() * 100) + 1;
    const chunk = text.slice(0, step);
    text = text.slice(step);
    res.push(chunk);
  }

  return res;
};

export default async function (currentQA, chatRef) {
  for (const chunk of mock()) {
    await sleep(chunk.length * 5);

    const prevMessage = chatRef.current
      .getMessages()
      .find((item) => item.id === currentQA.current.answer.id);

    const nextMessage = produce(prevMessage, (draft) => {
      if (draft.cards.length === 0) {
        draft.cards.push({
          code: 'Text',
          data: {
            content: chunk,
            msgStatus: 'generating',
            className: 'claude-clone-font',
          },
        });
      } else {
        draft.cards[0].data.content = draft.cards[0].data.content + chunk;
      }
    });

    chatRef.current.updateMessage(nextMessage);
  }
}
`},17885:function(h,n){n.Z=`export default function sleep(ms: number) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}
`},52765:function(h,n){n.Z=`import { produce } from 'immer';
import sleep from './sleep';

const mock = () => {
  let text = \`City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.
  
City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period.
City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s.
Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist.
Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.\`;

  const res = [];

  while (text.length > 0) {
    const step = Math.floor(Math.random() * 100) + 1;
    const chunk = text.slice(0, step);
    text = text.slice(step);
    res.push(chunk);
  }

  return res;
};

export default async function (currentQA, chatRef) {
  for (const chunk of mock()) {
    await sleep(chunk.length * 5);

    const prevMessage = chatRef.current
      .getMessages()
      .find((item) => item.id === currentQA.current.answer.id);

    const nextMessage = produce(prevMessage, (draft) => {
      if (draft.cards.length === 0) {
        draft.cards.push({
          code: 'DeepThink',
          data: {
            content: chunk,
            loading: true,
            className: 'claude-clone-font',
          },
        });
      } else {
        draft.cards[0].data.content = draft.cards[0].data.content + chunk;
      }
    });

    chatRef.current.updateMessage(nextMessage);
  }

  for (const chunk of mock()) {
    await sleep(chunk.length * 5);

    const prevMessage = chatRef.current
      .getMessages()
      .find((item) => item.id === currentQA.current.answer.id);

    const nextMessage = produce(prevMessage, (draft) => {
      draft.cards[0].data.loading = false;
      if (!draft.cards[1]) {
        draft.cards.push({
          code: 'Text',
          data: {
            content: chunk,
            msgStatus: 'generating',
            className: 'claude-clone-font',
          },
        });
      } else {
        draft.cards[1].data.content = draft.cards[1].data.content + chunk;
      }
    });

    chatRef.current.updateMessage(nextMessage);
  }
}
`},66293:function(h,n){n.Z=`import { produce } from 'immer';
import sleep from './sleep';
import webDevContent from './webDevContent';

export default async function (currentQA, chatRef) {
  await sleep(2000);

  const prevMessage = chatRef.current
    .getMessages()
    .find((item) => item.id === currentQA.current.answer.id);

  const nextMessage = produce(prevMessage, (draft) => {
    draft.cards.push({
      code: 'Text',
      data: {
        content: 'I am creating a document for your idea.',
        msgStatus: 'finished',
        className: 'claude-clone-font',
      },
    });
    draft.cards.push({
      code: 'WebDev',
      data: {
        loading: true,
        content: '',
        title: currentQA.current.query.cards[0].data.content,
      },
    });
  });

  chatRef.current.updateMessage(nextMessage);

  for (const chunk of mock()) {
    await sleep(chunk.length);

    const prevMessage = chatRef.current
      .getMessages()
      .find((item) => item.id === currentQA.current.answer.id);

    const nextMessage = produce(prevMessage, (draft) => {
      draft.cards[1].data.content = draft.cards[1].data.content + chunk;
    });

    chatRef.current.updateMessage(nextMessage);
  }
}

function mock() {
  let text = webDevContent();
  const res = [];

  while (text.length > 0) {
    const step = Math.floor(Math.random() * 100) + 1;
    const chunk = text.slice(0, step);
    text = text.slice(step);
    res.push(chunk);
  }

  return res;
}
`},13993:function(h,n){n.Z=`export default () => \`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>My Portfolio</title>
    <style>
        * {
            margin: 0;
            padding: 0;
            box-sizing: border-box;
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
        }
        
        body {
            background-color: #f8f9fa;
            color: #333;
            line-height: 1.6;
        }
        
        header {
            background-color: #2c3e50;
            color: white;
            padding: 2rem 0;
            text-align: center;
        }
        
        nav {
            background-color: #34495e;
            position: sticky;
            top: 0;
            z-index: 100;
        }
        
        nav ul {
            display: flex;
            justify-content: center;
            list-style: none;
        }
        
        nav ul li a {
            color: white;
            display: block;
            padding: 1rem 2rem;
            text-decoration: none;
            transition: background-color 0.3s;
        }
        
        nav ul li a:hover {
            background-color: #2c3e50;
        }
        
        section {
            max-width: 1200px;
            margin: 0 auto;
            padding: 4rem 2rem;
        }
        
        .hero {
            text-align: center;
            padding: 6rem 2rem;
        }
        
        .hero h1 {
            font-size: 3.5rem;
            margin-bottom: 1rem;
            color: #3498db;
        }
        
        .hero p {
            font-size: 1.5rem;
            max-width: 800px;
            margin: 0 auto 2rem;
        }
        
        .btn {
            display: inline-block;
            background-color: #3498db;
            color: white;
            padding: 0.8rem 1.8rem;
            border-radius: 50px;
            text-decoration: none;
            font-weight: bold;
            transition: all 0.3s;
            margin-top: 1rem;
        }
        
        .btn:hover {
            background-color: #2980b9;
            transform: translateY(-3px);
            box-shadow: 0 5px 15px rgba(0, 0, 0, 0.1);
        }
        
        h2 {
            text-align: center;
            font-size: 2.5rem;
            margin-bottom: 3rem;
            color: #2c3e50;
            position: relative;
        }
        
        h2:after {
            content: '';
            display: block;
            width: 80px;
            height: 4px;
            background-color: #3498db;
            margin: 1rem auto;
        }
        
        .about-content {
            display: flex;
            align-items: center;
            gap: 4rem;
        }
        
        .about-img {
            flex: 1;
            border-radius: 10px;
            overflow: hidden;
            box-shadow: 0 10px 30px rgba(0, 0, 0, 0.1);
        }
        
        .about-img img {
            width: 100%;
            height: auto;
            display: block;
        }
        
        .about-text {
            flex: 1;
        }
        
        .about-text h3 {
            font-size: 1.8rem;
            margin-bottom: 1rem;
            color: #2c3e50;
        }
        
        .skills {
            display: flex;
            flex-wrap: wrap;
            gap: 1rem;
            margin-top: 2rem;
        }
        
        .skill {
            background-color: #ecf0f1;
            padding: 0.5rem 1.5rem;
            border-radius: 50px;
            font-weight: 500;
            color: #2c3e50;
        }
        
        .projects {
            display: grid;
            grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));
            gap: 2rem;
        }
        
        .project {
            background-color: white;
            border-radius: 10px;
            overflow: hidden;
            box-shadow: 0 5px 15px rgba(0, 0, 0, 0.1);
            transition: transform 0.3s;
        }
        
        .project:hover {
            transform: translateY(-10px);
        }
        
        .project img {
            width: 100%;
            height: 200px;
            object-fit: cover;
        }
        
        .project-content {
            padding: 1.5rem;
        }
        
        .project-content h3 {
            font-size: 1.5rem;
            margin-bottom: 0.5rem;
            color: #2c3e50;
        }
        
        .project-content p {
            margin-bottom: 1rem;
            color: #7f8c8d;
        }
        
        .project-links {
            display: flex;
            gap: 1rem;
        }
        
        .project-links a {
            color: #3498db;
            text-decoration: none;
            font-weight: 500;
        }
        
        .project-links a:hover {
            text-decoration: underline;
        }
        
        .contact-content {
            display: flex;
            gap: 4rem;
        }
        
        .contact-info {
            flex: 1;
        }
        
        .contact-info p {
            margin-bottom: 1.5rem;
            display: flex;
            align-items: center;
        }
        
        .contact-info i {
            margin-right: 1rem;
            color: #3498db;
            font-size: 1.2rem;
        }
        
        .contact-form {
            flex: 1;
        }
        
        .form-group {
            margin-bottom: 1.5rem;
        }
        
        .form-group label {
            display: block;
            margin-bottom: 0.5rem;
            color: #2c3e50;
            font-weight: 500;
        }
        
        .form-group input,
        .form-group textarea {
            width: 100%;
            padding: 0.8rem;
            border: 1px solid #ddd;
            border-radius: 5px;
            font-size: 1rem;
        }
        
        .form-group textarea {
            height: 150px;
            resize: vertical;
        }
        
        footer {
            background-color: #2c3e50;
            color: white;
            text-align: center;
            padding: 2rem;
            margin-top: 4rem;
        }
        
        .social-links {
            margin: 1rem 0;
        }
        
        .social-links a {
            color: white;
            margin: 0 0.8rem;
            font-size: 1.5rem;
            transition: color 0.3s;
        }
        
        .social-links a:hover {
            color: #3498db;
        }
        
        @media (max-width: 768px) {
            .about-content,
            .contact-content {
                flex-direction: column;
            }
            
            .hero h1 {
                font-size: 2.5rem;
            }
            
            .hero p {
                font-size: 1.2rem;
            }
            
            .about-img,
            .about-text,
            .contact-info,
            .contact-form {
                flex: none;
                width: 100%;
            }
            
            nav ul {
                flex-direction: column;
            }
            
            nav ul li a {
                text-align: center;
            }
        }
    </style>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
</head>
<body>
    <header>
        <h1>John Doe</h1>
        <p>Full-Stack Developer & Designer</p>
    </header>
    
    <nav>
        <ul>
            <li><a href="#home">Home</a></li>
            <li><a href="#about">About</a></li>
            <li><a href="#projects">Projects</a></li>
            <li><a href="#contact">Contact</a></li>
        </ul>
    </nav>
    
    <section id="home" class="hero">
        <h1>Hi, I'm John Doe</h1>
        <p>A passionate full-stack developer creating amazing web experiences</p>
        <a href="#projects" class="btn">View My Work</a>
    </section>
    
    <section id="about">
        <h2>About Me</h2>
        <div class="about-content">
            <div class="about-img">
                <img src="https://images.unsplash.com/photo-1507003211169-0a1d58bc3589?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80" alt="John Doe">
            </div>
            <div class="about-text">
                <h3>Who I Am</h3>
                <p>I'm a full-stack developer with over 5 years of experience building web applications and digital experiences. I specialize in JavaScript frameworks, responsive design, and creating intuitive user interfaces.</p>
                <p>My journey in web development started when I built my first website at the age of 16. Since then, I've worked with startups and established companies to bring their digital visions to life.</p>
                <p>When I'm not coding, you can find me hiking, reading tech blogs, or experimenting with new technologies.</p>
                <div class="skills">
                    <span class="skill">HTML5</span>
                    <span class="skill">CSS3</span>
                    <span class="skill">JavaScript</span>
                    <span class="skill">React</span>
                    <span class="skill">Node.js</span>
                    <span class="skill">Python</span>
                    <span class="skill">MongoDB</span>
                    <span class="skill">UI/UX Design</span>
                </div>
            </div>
        </div>
    </section>
    
    <section id="projects">
        <h2>My Projects</h2>
        <div class="projects">
            <div class="project">
                <img src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80" alt="E-Commerce Platform">
                <div class="project-content">
                    <h3>E-Commerce Platform</h3>
                    <p>A full-featured online shopping platform with payment integration, user authentication, and admin dashboard.</p>
                    <div class="project-links">
                        <a href="#"><i class="fas fa-external-link-alt"></i> Live Demo</a>
                        <a href="#"><i class="fab fa-github"></i> Source Code</a>
                    </div>
                </div>
            </div>
            <div class="project">
                <img src="https://images.unsplash.com/photo-1551650975-87deedd944c3?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80" alt="Task Management App">
                <div class="project-content">
                    <h3>Task Management App</h3>
                    <p>A collaborative task management application with real-time updates, team features, and progress tracking.</p>
                    <div class="project-links">
                        <a href="#"><i class="fas fa-external-link-alt"></i> Live Demo</a>
                        <a href="#"><i class="fab fa-github"></i> Source Code</a>
                    </div>
                </div>
            </div>
            <div class="project">
                <img src="https://images.unsplash.com/photo-1542831371-29b0f74f6b5b?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80" alt="Weather Dashboard">
                <div class="project-content">
                    <h3>Weather Dashboard</h3>
                    <p>A responsive weather application that displays current conditions, forecasts, and interactive maps.</p>
                    <div class="project-links">
                        <a href="#"><i class="fas fa-external-link-alt"></i> Live Demo</a>
                        <a href="#"><i class="fab fa-github"></i> Source Code</a>
                    </div>
                </div>
            </div>
        </div>
    </section>
    
    <section id="contact">
        <h2>Contact Me</h2>
        <div class="contact-content">
            <div class="contact-info">
                <p><i class="fas fa-envelope"></i> john.doe@example.com</p>
                <p><i class="fas fa-phone"></i> +1 (555) 123-4567</p>
                <p><i class="fas fa-map-marker-alt"></i> San Francisco, CA</p>
                <p>I'm currently available for freelance work or full-time positions. If you have a project in mind or just want to connect, feel free to reach out!</p>
            </div>
            <div class="contact-form">
                <form>
                    <div class="form-group">
                        <label for="name">Name</label>
                        <input type="text" id="name" required>
                    </div>
                    <div class="form-group">
                        <label for="email">Email</label>
                        <input type="email" id="email" required>
                    </div>
                    <div class="form-group">
                        <label for="message">Message</label>
                        <textarea id="message" required></textarea>
                    </div>
                    <button type="submit" class="btn">Send Message</button>
                </form>
            </div>
        </div>
    </section>
    
    <footer>
        <div class="social-links">
            <a href="#"><i class="fab fa-github"></i></a>
            <a href="#"><i class="fab fa-linkedin"></i></a>
            <a href="#"><i class="fab fa-twitter"></i></a>
            <a href="#"><i class="fab fa-dribbble"></i></a>
        </div>
        <p>&copy; 2023 John Doe. All rights reserved.</p>
    </footer>

    <script>
        // Smooth scrolling for navigation links
        document.querySelectorAll('nav a, .btn').forEach(link => {
            link.addEventListener('click', function(e) {
                e.preventDefault();
                const targetId = this.getAttribute('href');
                const targetSection = document.querySelector(targetId);
                window.scrollTo({
                    top: targetSection.offsetTop,
                    behavior: 'smooth'
                });
            });
        });

        // Form submission
        const form = document.querySelector('form');
        form.addEventListener('submit', function(e) {
            e.preventDefault();
            alert('Thank you for your message! I will get back to you soon.');
            form.reset();
        });
    <\/script>
</body>
</html>\`;
`},38136:function(h,n){n.Z=`import { createStyles } from "antd-style";

export default function Background({ children }: { children: React.ReactNode }) {
  const { styles } = useStyles();
  return <div className={styles.container}>
    <video src="https://cloud.video.taobao.com/vod/F7ULxg_UaBXJe9XRA2IVhiyjjYRZ10ythvs6iv-yoJg.mp4" autoPlay loop muted playsInline className={styles.video} />
    {children}
  </div>;
}


const useStyles = createStyles(({ token }) => {
  return {
    container: {
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
      position: 'relative',
      width: '100%',
      height: '100%',
      padding: 70,
    },
    video: {
      position: 'absolute',
      inset: 0,
      width: '100%',
      height: '100%',
      objectFit: 'cover',
      zIndex: -1,
    },
  };
});`},24235:function(h,n){n.Z=`import React from 'react';
import { ChatInput } from '@agentscope-ai/chat';
import { createStyles } from 'antd-style';

export default function (props: { onSubmit: (content: string) => void, loading: boolean }) {
  const { styles } = useStyle()
  const [content, setContent] = React.useState('');


  return <ChatInput
    loading={props.loading}
    className={styles.container}
    value={content}
    onChange={setContent}
    onSubmit={async () => {
      props.onSubmit(content);
      setContent('');
    }}
    placeholder='Search what you want to know'
  ></ChatInput>
}


const useStyle = createStyles(({ css, token }) => ({
  container: {
  },
}));`},66878:function(h,n){n.Z=`import { Bubble, CustomCardsProvider } from "@agentscope-ai/chat";
import { createStyles } from "antd-style";

export default function ({ messages }) {
  const { styles } = useStyle();

  return <CustomCardsProvider
    cardConfig={{
    }}>
    <Bubble.List
      classNames={{
        wrapper: styles.wrapper,
        list: styles.list
      }}
      // @ts-ignore
      items={messages}
    />
  </CustomCardsProvider>
}

const useStyle = createStyles(({ css, token }) => ({
  wrapper: css\`
    height: 0;
    flex: 1;
    
  \`,

  list: css\`
  \`
}));`},59469:function(h,n){n.Z=`import { createStyles, createGlobalStyle } from "antd-style";
import { Bubble, ChatAnywhere, ChatAnywhereRef, CustomCardsProvider, DefaultCards, Disclaimer, TMessage, uuid } from '@agentscope-ai/chat';
import { Vaso } from 'vaso'
import MessageList from "./MessageList";
import Input from "./Input";
import { useState } from "react";
import { SparkCopyLine, SparkReplaceLine, SparkThumbsDownLine, SparkThumbsUpLine } from "@agentscope-ai/icons";

export default function () {
  const { styles } = useStyles();
  const [messages, setMessages] = useState<TMessage[]>([
    {
      role: 'user',
      content: 'what is citypop',
      id: '1',
    },
    {
      role: 'assistant',
      cards: [
        {
          code: 'DeepThink',
          data: {
            content: 'City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period. City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s. Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist. Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.'
          }
        },
        {
          code: 'Text',
          data: {
            content: 'City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period. City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s. Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist. Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.'
          },
        },
        {
          code: 'Footer',
          data: {
            left: <DefaultCards.FooterActions data={[
              {
                icon: <SparkCopyLine />, onClick: () => { }
              },
              {
                icon: <SparkReplaceLine />, onClick: () => { }
              },
              {
                icon: <SparkThumbsUpLine />, onClick: () => { }
              },
              {
                icon: <SparkThumbsDownLine />, onClick: () => { }
              },
            ]} />,
          }
        }
      ],
      id: '2',
    },
  ]);
  return <>
    <Style />
    <Vaso className={styles.container} radius={20} depth={0.5} dispersion={0} blur={10}>
      <div className={styles.chat}>
        <MessageList messages={messages} />
        <Input onSubmit={() => {

          setMessages(v => [...v, {
            role: 'user',
            content: 'what is citypop',
            id: uuid(),
          }])

          setTimeout(() => {
            setMessages(v => [...v, {
              role: 'assistant',
              id: uuid(),
              content: 'City Pop, short for City Pop, is a genre of music that originated in Japan during the late 1970s and early 1980s. It blends elements of jazz, folk, easy listening, and AOR (Adult Oriented Rock) with a distinctly urban feel. The term "City Pop" often evokes images of the sophisticated, stylish, and sometimes nostalgic lifestyle of Tokyo and other major Japanese cities during this period. City Pop is characterized by its smooth melodies, mellow rhythms, and often dreamy or introspective lyrics. Common themes include city life, summer vibes, nature, and romantic encounters. The music is frequently associated with a particular aesthetic that reflects the fashion, design, and cultural trends of urban Japan in the 1980s. Some notable City Pop artists include Mariya Takeuchi, known for her iconic song " Plastic Love," and Joe Hisaishi, a composer famous for his work on Studio Ghibli films, though he is not strictly considered a City Pop artist. Today, City Pop has experienced a resurgence in popularity, particularly among younger generations who appreciate its unique sound and the nostalgic value it holds.'
            }])
          }, 1000);

        }} loading={false} />
      </div>
    </Vaso>
  </>
}

const useStyles = createStyles(({ token, css, prefixCls }) => {

  return {
    container: css\`
      width: 100%;
      height: 100%;
      border: 1px solid rgba(200, 196, 191, .5);
      border-radius: 20px;
      overflow: hidden;
    \`,

    chat: {
      display: 'flex',
      flexDirection: 'column',
      maxWidth: 800,
      margin: '0 auto',
      padding: '20px 0',
      height: '100%',
    },
  };
});

const Style = createGlobalStyle\`
  .ant-bubble-content-filled {
    border: 1px solid rgba(200, 196, 191, .5);
    background-color: rgba(200, 196, 191, .1) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;
  }

  .ant-accordion-deep-thinking {
    border-color:  rgba(200, 196, 191, .5) !important;
  }

  .ant-accordion-group-header-close {
    border: 1px solid rgba(200, 196, 191, .5) !important;
    background-color: rgba(200, 196, 191, .1) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;


  }

  .ant-accordion-group-icon-success {
    color: #fff !important;
  }

  .ant-sender {
    background-color: rgba(54, 53, 52, 0.3) !important;
    border-color: rgba(200, 196, 191, .5) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;
    
  }

  .ant-sender textarea::placeholder,
  .ant-sender textarea {
   color: rgba(231, 231, 237, .88) !important;
  }

  .ant-sender-actions-list .ant-btn {
    background-color: rgba(200, 196, 191, .3) !important;
    border-color: rgba(200, 196, 191, .5) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;


  }

  .ant-bubble-list-scroll-to-bottom .ant-btn {
    background-color: rgba(200, 196, 191, .3) !important;
    border-color: rgba(200, 196, 191, .5) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;



    &:hover {
      color: #fff !important;
      background-color: rgba(200, 196, 191, .5) !important;
      border-color: rgba(200, 196, 191, .5) !important;
    }
  }

  .ant-bubble-footer-left .ant-btn {
    border-color: rgba(200, 196, 191, .5) !important;
    box-shadow: rgba(0, 0, 0, 0.1) 0px 1px 20px !important;
  }

\`;`},37917:function(h,n){n.Z=`import { createStyles } from "antd-style";
import Background from "./Background";
import Chat from "./Chat";
import { ConfigProvider, carbonDarkTheme } from '@agentscope-ai/design';

carbonDarkTheme.theme.token.borderRadius = 20;
carbonDarkTheme.theme.token.borderRadiusLG = 20;
carbonDarkTheme.theme.token.borderRadiusSM = 20;

export default function LiquidGlass() {
  const { styles } = useStyles();
  return <ConfigProvider
    {...carbonDarkTheme}
  >
    <div className={styles.container}>
      <Background>
        <Chat />
      </Background>
    </div>
  </ConfigProvider>;
}


const useStyles = createStyles(({ token }) => {
  return {
    container: {
      width: '100vw',
      height: '100vh',
    },
  };
});`},1269:function(h,n){n.Z=`import { createStyles } from 'antd-style';
import { useEffect, useState } from 'react';

export default function (props: { icon: React.ReactNode, title: string, children: React.ReactNode }) {
  const { styles } = useStyle();
  const [opacity, setOpacity] = useState(0);

  useEffect(() => {
    setOpacity(1);
  }, []);

  
  return <div className={styles.block} style={{ opacity }}>
    <div className={styles.header} data-anchor={props.title}>
      <div className={styles.icon}>{props.icon}</div>
      <div className={styles.title}>{props.title}</div>
    </div>

    <div className={styles.content}>
      {props.children}
    </div>
  </div>
}

const useStyle = createStyles(({ css, token }) => ({
  block: {
    marginTop: 24,
    marginBottom: 48,
    transition: 'opacity 1s ease-in-out',
  },
  header: {
    display: 'flex',
    alignItems: 'center',
    gap: 8,
    color: token.colorText,
    marginBottom: 12,
  },
  icon: {
    fontSize: 24,
  },
  title: {
    fontSize: 18,
    textTransform: 'capitalize',
  },
  content: {

  },
}));`},57879:function(h,n){n.Z=`import { Markdown } from "@agentscope-ai/chat";
import Block from "./Block";
import { SparkMessageLine } from "@agentscope-ai/icons";

export default function (props: { content: string, loading: boolean }) {
  console.log(props)
  return <Block
    title="answer"
    icon={<SparkMessageLine />}
  >
    <Markdown cursor={props.loading} content={props.content} baseLineHeight={1.8571428571428572}>
    </Markdown>
  </Block>
}`},90165:function(h,n){n.Z=`import { DeepThinking } from "@agentscope-ai/chat";

export default function ({ loading, content }) {
  return <DeepThinking
    defaultOpen
    content={content}
    loading={loading}
    title={loading ? 'Deep thinking' : 'Completed deep thinking'}
    className="perplexcity-deep-thinking"
  />

}`},74811:function(h,n){n.Z=`import { SparkPictureLine } from "@agentscope-ai/icons";
import { createStyles } from "antd-style";
import Block from "./Block";
import { Image } from "antd";

export default function ({ images }: { images: string[] }) {
  const { styles } = useStyle();

  return <Block
    title="images"
    icon={<SparkPictureLine />}
  >
    <div className={styles.images}>

      <Image.PreviewGroup>
        {images.map((image, index) => (
          <Image src={image} key={index} style={{ width: 180.55172413793102, height: 136, borderRadius: 12, overflow: 'hidden' }} />
        ))}
      </Image.PreviewGroup>

    </div>
  </Block>
}


const useStyle = createStyles(({ css, token }) => ({
  images: {
    display: 'flex',
    gap: 12,
    padding: 12,
    overflow: 'auto',
    backgroundColor: token.colorFillSecondary,
    borderRadius: token.borderRadius,

    '&::-webkit-scrollbar': {
      display: 'none',
    },

    '.sps-image-mask': {
      borderRadius: 12
    }
  },

}));`},8905:function(h,n){n.Z=`import { SparkDeepSearchLine, SparkRightArrowLine } from "@agentscope-ai/icons";
import Block from "./Block";
import { createStyles } from "antd-style";

export default function (props: { related: string[] }) {
  const { styles } = useStyle();
  
  return <Block
    title="related"
    icon={<SparkDeepSearchLine />}
  >
    <div className={styles.related}>
      {props.related.map((item, index) => (
        <div className={styles.item} key={index}>
          <SparkRightArrowLine className={styles.icon} />
          {item}
        </div>
      ))}
    </div>
  </Block>
}


const useStyle = createStyles(({ css, token }) => ({
  related: {
    display: 'flex',
    flexDirection: 'column',
    gap: 8,
  },
  item: {
    height: 24,
    gap: 8,
    display: 'flex',
    alignItems: 'center',
    cursor: 'pointer',
    color: token.colorTextTertiary,
    '&:hover': {
      color: token.colorText,
    }
  },
  icon: {
    fontSize: 20,
    color: token.colorPrimary,
  },
}));`},47413:function(h,n){n.Z=`import { Markdown } from "@agentscope-ai/chat";
import Block from "./Block";
import { SparkPageSearchLine } from "@agentscope-ai/icons";
import { createStyles } from "antd-style";

interface IItem {
  title: string;
  url: string;
  logo: string,
  content: string,
}

function Item(props: IItem) {
  const { styles } = useStyle();

  return <div className={styles.item}>
    <img className={styles.logo} src={props.logo} />

    <div className={styles.content}>
      <div className={styles.title}>{props.title}</div>
      <a className={styles.url} href={props.url} target="_blank" >{props.url}</a>
    </div>
  </div>
}

export default function (props: { sources: IItem[] }) {
  const { styles } = useStyle();

  return <Block
    title="sources"
    icon={<SparkPageSearchLine />}
  >

    <div className={styles.sources}>
      {
        props.sources.map((item, index) => (
          <div className={styles.source} key={index}>
            <Item  {...item} />
            <Markdown content={item.content} baseLineHeight={1.8571428571428572}>
            </Markdown>
          </div>
        ))
      }
    </div>
  </Block>
}


const useStyle = createStyles(({ css, token }) => ({
  item: {
    display: 'flex',
    alignItems: 'center',
    gap: 8,
    marginBottom: 8,
  },
  logo: {
    width: 32,
    height: 32,
    borderRadius: 32,
  },
  content: {

  },
  source: {},
  sources: {
    display: 'flex',
    flexDirection: 'column',
    gap: 24,
  },
  title: {
    fontSize: 12,
    lineHeight: '20px',
    color: token.colorText,
  },
  url: {
    fontSize: 12,
    lineHeight: '20px',
    color: token.colorTextTertiary,
    textDecoration: 'none',
    '&:hover': {
      color: token.colorText,
    }
  },
}));`},75634:function(h,n){n.Z=`import { RefObject, useEffect, useRef, useState } from "react";
import useStyle from './style';
import { Scrollspy } from '@makotot/ghostui';


export default function (props) {
  const [sectionRefs, setSectionRefs] = useState<RefObject<HTMLElement>[]>([]);
  const { styles } = useStyle();
  const contentRef = useRef<HTMLDivElement>(null);


  useEffect(() => {
    const refs = props.contentRef?.current?.querySelectorAll('[data-anchor]');

    if (refs?.length) {
      const refsArray = Array.from(refs);
      setSectionRefs(refsArray as any);
    }

  }, [props.data]);


  return <>
    <div className={styles.content} ref={contentRef}>
      {props.children}
    </div>

    {
      sectionRefs.length ? <Scrollspy sectionRefs={sectionRefs} rootSelector="#perplexcity-bubble-list>div" >
        {
          ({ currentElementIndexInViewport }) => {
            console.log(currentElementIndexInViewport);
            return <div>...</div>
          }
        }
      </Scrollspy> : null
    }
  </>

}`},80564:function(h,n){n.Z=`import useStyle from './style';
import Images from './Images';
import Content from './Content';
import Related from './Related';
import Sources from './Sources';
// @ts-ignore
import { ReactComponent as Logo } from './Logo.svg';
import DeepThinking from './DeepThinking';
import TOC from './TOC';


export default function (props) {
  const { deepThinking, images, content, sources, related } = props.data;
  const { styles } = useStyle();


  return <div className={styles.container}>
    <div className={styles.avatar}>
      <Logo />
    </div>

    <TOC data={props.data}>
      {
        deepThinking && <DeepThinking {...deepThinking} />
      }
      {
        images && <Images images={images} />
      }
      {
        content && <Content {...content} />
      }
      {
        sources && <Sources sources={sources} />
      }
      {
        related && <Related related={related} />
      }
    </TOC>
  </div>
}`},59004:function(h,n){n.Z=`import { createStyles } from 'antd-style';

export default createStyles(({ css, token }) => ({
  container: css\`
    width: 100%;
    display: flex;
    gap: 12px;
  \`,

  content: {
    width: 0,
    flex: 1,
  },
  avatar: {
    width: 32,
    height: 32,
    borderRadius: 8,
    overflow: 'hidden'
  },
}));
`},96092:function(h,n){n.Z=`import { Avatar } from '@agentscope-ai/design';
import useStyle from './style';
import { SparkUserLine } from '@agentscope-ai/icons';

export default function (props) {


  const { styles } = useStyle();
  return <div className={styles.container}>
    <Avatar className={styles.avatar} size={32} icon={<SparkUserLine style={{ fontSize: 20 }} />} />

    <div className={styles.content}>
      {props.data.content}
      <div className={styles.hr} />
    </div>
  </div>
}`},31497:function(h,n){n.Z=`import { Disclaimer } from '@agentscope-ai/chat';
import { useState } from 'react';
import Welcome from '../Welcome';
import useStyle from './style';
import Input from '../Input';
import MessagesList from '../MessagesList';
import mock from './mock';

export default function Claude() {
  const { styles } = useStyle()
  const [messages, setMessages] = useState([]);
  const [loading, setLoading] = useState(false);


  return <div className={styles.container}>
    <div className={styles.chat}>
      {
        messages.length ? <MessagesList messages={messages} /> : <Welcome />
      }
      <Input loading={loading} onSubmit={async (content) => {


        setLoading(true);

        setMessages([{
          role: 'user',
          cards: [
            { code: 'Query', data: { content: 'What is CityPop?' } }
          ]
        }, {
          role: 'assistant',
          msgStatus: 'generating',
          cards: [{
            code: 'Answer',
            data: {
              deepThinking: {
                loading: true,
                content: '...'
              }
            },
          }],
        }]);

        await mock({ messages, setMessages });

        setLoading(false);
      }} />
      <div className={styles.disclaimer}>
        <Disclaimer />
      </div>
    </div>
  </div>
}

`},69172:function(h,n){n.Z=`import { produce } from 'immer';

const data = {
  deepThinking: {
    loading: false,
    content: \`Okay, the user is asking about "citypop." Let me start by recalling what I know about that term. I remember that City Pop is a genre of music, but I need to make sure I'm accurate.

First, when did City Pop originate? I think it was in Japan during the late 1970s and 1980s. That makes sense because Japan's economy was booming then, leading to a vibrant urban culture. The genre probably blends Western styles like funk, jazz, and disco with Japanese pop elements. Artists like Tatsuro Yamashita and Mariya Takeuchi come to mind. Oh, and there's that song "Plastic Love" by Mariya Takeuchi that went viral online recently, which might have sparked renewed interest.

I should also mention the themes of City Pop songs\u2014they often reflect city life, romance, and leisure, fitting the upbeat and cosmopolitan vibe of the time. The visual style associated with it includes retro anime and vibrant cityscapes, which adds to its aesthetic appeal. It's interesting how the internet, especially platforms like YouTube and Spotify, helped revive this genre internationally. Maybe I should note that connection between modern digital culture and the resurgence of City Pop.

Wait, are there any subgenres or key characteristics I should highlight? The use of synthesizers, smooth vocals, and lush production are typical. Also, City Pop was influenced by yacht rock and soft rock from the West. It's important to link it to Japan's economic bubble era, as that context explains the music's optimistic and luxurious feel.

I need to ensure I don't confuse it with other Japanese genres like J-pop or Enka. City Pop is distinct in its fusion with Western sounds. Including notable albums, like Tatsuro Yamashita's "For You" or Anri's "Timely!!", could be helpful. Oh, and the connection to future funk and vaporwave genres that sample City Pop tracks is a good point for its modern influence.\`,
  },
  images: [
    'https://gw.alicdn.com/imgextra/i3/O1CN01zgK3yS1FAm7uKIHWc_!!6000000000447-0-tps-1232-928.jpg',
    'https://gw.alicdn.com/imgextra/i2/O1CN01Vasr7w1PzHxyioPPm_!!6000000001911-0-tps-1232-928.jpg',
    'https://gw.alicdn.com/imgextra/i4/O1CN01M98Xnc26huPmyBon2_!!6000000007694-0-tps-1232-928.jpg',
    'https://gw.alicdn.com/imgextra/i1/O1CN01EJ1oqo1J01Hx71bgB_!!6000000000965-0-tps-1232-928.jpg',
    'https://gw.alicdn.com/imgextra/i3/O1CN01zgK3yS1FAm7uKIHWc_!!6000000000447-0-tps-1232-928.jpg',
    'https://gw.alicdn.com/imgextra/i2/O1CN01Vasr7w1PzHxyioPPm_!!6000000001911-0-tps-1232-928.jpg',
    'https://gw.alicdn.com/imgextra/i4/O1CN01M98Xnc26huPmyBon2_!!6000000007694-0-tps-1232-928.jpg',
    'https://gw.alicdn.com/imgextra/i1/O1CN01EJ1oqo1J01Hx71bgB_!!6000000000965-0-tps-1232-928.jpg',
  ],
  content: {
    content: \`City Pop (\u30B7\u30C6\u30A3\u30FC\u30DD\u30C3\u30D7) is a subgenre of Japanese pop music that emerged in the late 1970s and peaked in popularity during the 1980s. It is characterized by its fusion of various musical styles, including funk, disco, R&B, soft rock, and boogie, often reflecting the urban and sophisticated lifestyle of city dwellers. The genre is known for its polished production, catchy melodies, and nostalgic themes, often evoking a sense of escapism and leisure associated with the vibrant city life.
## Key Features of City Pop

- Musical Influences: City Pop draws heavily from Western music trends of the time, particularly adult-oriented genres like funk and disco. It incorporates elements of jazz, lounge music, and even tropical sounds.
- Cultural Context: The genre developed during a period of rapid economic growth in Japan, which influenced its themes of urban sophistication and leisure. It served as a soundtrack for the cosmopolitan lifestyle of the era.
- Aesthetic: City Pop is often associated with vibrant and colorful artwork, featuring themes of summer, beaches, and urban nightlife. The cover art typically reflects a retro-futuristic vision, with bright colors and nostalgic imagery.
\`,
    loading: false,
  },
  sources: [
    {
      title: 'City Pop',
      logo: 'https://gw.alicdn.com/imgextra/i3/O1CN0134H6gj1MP9sJ1T17L_!!6000000001426-55-tps-32-32.svg',
      url: 'https://en.wikipedia.org/wiki/City_Pop',
      content: \`## City pop

City Pop (\u30B7\u30C6\u30A3\u30FC\u30DD\u30C3\u30D7) is a subgenre of Japanese pop music that emerged in the late 1970s and peaked in popularity during the 1980s. It is characterized by its fusion of various musical styles, including funk, disco, R&B, soft rock, and boogie, often reflecting the urban and sophisticated lifestyle of city dwellers. The genre is known for its polished production, catchy melodies, and nostalgic themes, often evoking a sense of escapism and leisure associated with the vibrant city life.\`,
    },
    {
      title: 'City Pop',
      logo: 'https://gw.alicdn.com/imgextra/i3/O1CN0134H6gj1MP9sJ1T17L_!!6000000001426-55-tps-32-32.svg',
      url: 'https://en.wikipedia.org/wiki/City_Pop',
      content: \`## City pop

City Pop (\u30B7\u30C6\u30A3\u30FC\u30DD\u30C3\u30D7) is a subgenre of Japanese pop music that emerged in the late 1970s and peaked in popularity during the 1980s. It is characterized by its fusion of various musical styles, including funk, disco, R&B, soft rock, and boogie, often reflecting the urban and sophisticated lifestyle of city dwellers. The genre is known for its polished production, catchy melodies, and nostalgic themes, often evoking a sense of escapism and leisure associated with the vibrant city life.\`,
    },
  ],
  related: [
    \`How did Japan's economic boom influence the development of City Pop?\`,
    \`What are the key characteristics of City Pop music?\`,
    \`How did Japan's economic boom influence the development of City Pop?\`,
    \`What are the key characteristics of City Pop music?\`,
  ],
};

const mock = [
  {
    deepThinking: {
      loading: true,
      content: data.deepThinking.content.slice(0, 20),
    },
  },

  {
    deepThinking: {
      loading: true,
      content: data.deepThinking.content.slice(0, 40),
    },
  },
  {
    deepThinking: {
      loading: true,
      content: data.deepThinking.content.slice(0, 60),
    },
  },
  {
    deepThinking: {
      loading: true,
      content: data.deepThinking.content.slice(0, 80),
    },
  },
  {
    deepThinking: {
      loading: true,
      content: data.deepThinking.content.slice(0, 100),
    },
  },
  {
    deepThinking: {
      loading: true,
      content: data.deepThinking.content.slice(0, 120),
    },
  },
  {
    deepThinking: data.deepThinking,
  },

  {
    deepThinking: data.deepThinking,
    images: data.images.slice(0, 1),
  },
  {
    deepThinking: data.deepThinking,
    images: data.images.slice(0, 2),
  },
  {
    deepThinking: data.deepThinking,
    images: data.images.slice(0, 3),
  },

  {
    deepThinking: data.deepThinking,
    images: data.images.slice(0, 4),
  },

  {
    deepThinking: data.deepThinking,
    images: data.images,
  },
  {
    deepThinking: data.deepThinking,
    images: data.images,
    content: {
      loading: true,
      content: data.content.content.slice(0, 20),
    },
  },
  {
    deepThinking: data.deepThinking,
    images: data.images,
    content: {
      loading: true,
      content: data.content.content.slice(0, 40),
    },
  },
  {
    deepThinking: data.deepThinking,
    images: data.images,
    content: {
      loading: true,
      content: data.content.content.slice(0, 60),
    },
  },
  {
    deepThinking: data.deepThinking,
    images: data.images,
    content: {
      loading: true,
      content: data.content.content.slice(0, 80),
    },
  },
  {
    deepThinking: data.deepThinking,
    images: data.images,
    content: data.content,
  },
  {
    deepThinking: data.deepThinking,
    images: data.images,
    content: data.content,
    sources: data.sources.slice(0, 1),
  },
  {
    deepThinking: data.deepThinking,
    images: data.images,
    content: data.content,
    sources: data.sources,
  },
  {
    deepThinking: data.deepThinking,
    images: data.images,
    content: data.content,
    sources: data.sources,
    related: data.related,
  },
];

function sleep(ms: number) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

export default async function ({
  messages,
  setMessages,
}: {
  messages: any[];
  setMessages: (messages: any[]) => void;
}) {
  await sleep(500);

  for (const chunk of mock) {
    await sleep(1000);
    setMessages((list) => {
      const query = list[0];
      const answer = list[1];

      return [
        query,
        produce(answer, (draft) => {
          draft.cards[0].data = chunk;
        }),
      ];
    });
  }

  await sleep(100);
}
`},50767:function(h,n){n.Z=`import React from 'react';
import { ChatInput } from '@agentscope-ai/chat';
import useStyle from './style';

export default function (props: { onSubmit: (content: string) => void, loading: boolean }) {
  const { styles } = useStyle()
  const [content, setContent] = React.useState('');


  return <ChatInput
    loading={props.loading}
    className={styles.container}
    value={content}
    onChange={setContent}
    onSubmit={async () => {
      props.onSubmit(content);
      setContent('');
    }}
    placeholder='Search what you want to know'
  ></ChatInput>
}


`},87543:function(h,n){n.Z=`import { Bubble, CustomCardsProvider } from '@agentscope-ai/chat';
import useStyle from './style';
import Query from '../../cards/Query';
import Answer from "../../cards/Answer";

export default function ({ messages }) {
  const { styles } = useStyle();

  return <CustomCardsProvider
    cardConfig={{
      Query,
      Answer
    }}>
    <Bubble.List
      id="perplexcity-bubble-list"
      classNames={{
        wrapper: styles.wrapper,
        list: styles.list
      }}
      // @ts-ignore
      items={messages}
    />
  </CustomCardsProvider>
}`},69858:function(h,n){n.Z=`import EyeFollower from '@agentscope-ai/chat/Welcome/demo/EyeFollower';
import { createGlobalStyle, createStyles } from 'antd-style';

export default function () {
  const { styles } = useStyle();

  return <>
    <Style />
    <div className={styles.welcome}>
      <EyeFollower />
    </div>
  </>
}

const useStyle = createStyles(({ css, token }) => ({
  welcome: {
    display: 'flex',
    flexDirection: 'column',
    alignItems: 'center',
    gap: 4,
    marginTop: 'calc(10vh + 80px + 64px)',
    marginBottom: 32,

  },
}));



const Style = createGlobalStyle\`
.sps-chat-anywhere-chat-welcome {
  height: auto !important;  
}\`



`},80818:function(h,n){n.Z=`import { ConfigProvider } from '@agentscope-ai/design';
import { darkTheme, lightTheme } from './theme';
import { useState } from 'react';
import React from 'react';


export const ConfigProviderContext = React.createContext<{
  dark?: boolean;
  setDark: (dark: boolean) => void;
}>(undefined);

export default function (props) {
  const [dark, setDark] = useState(true);
  const currentTheme = dark ? darkTheme : lightTheme;

  return <>
    <ConfigProviderContext.Provider value={{
      dark,
      setDark,
    }}>
      <ConfigProvider
        {...currentTheme}
        style={{ height: '100%' }}
        iconfont='https://at.alicdn.com/t/a/font_4807885_6wjcpzayws6.js'
        prefix='sps'
        prefixCls="sps"
      >
        {props.children}
      </ConfigProvider>
    </ConfigProviderContext.Provider>
  </>;
}`},41355:function(h,n){n.Z=`import { useGetState } from 'ahooks';
import React, { useState } from 'react';


type TContext = {
  model: string;
  enableSearch: boolean;
  enableThinking: boolean;
  mode?: string;
  initial?: boolean;
  canvas: boolean;
}

export const DemoContext = React.createContext<{
  demoContext: TContext,
  setDemoContext: (data: Partial<TContext>) => void,
  getDemoContext: () => TContext
}>(undefined);




export default function (props: { children: React.ReactNode }) {
  const [demoContext, setDemoContext, getDemoContext] = useGetState<TContext>({
    model: 'claude-3.5-sonnet',
    enableSearch: false,
    enableThinking: false,
    initial: true,
    mode: undefined,
    canvas: false,
  });


  return <DemoContext.Provider value={{
    demoContext, setDemoContext(data) {
      setDemoContext(v => {
        return { ...v, ...data }
      })
    }, getDemoContext
  }}>
    {props.children}
  </DemoContext.Provider>
}`},34313:function(h,n){n.Z=`import { generateThemeByToken } from '@agentscope-ai/design';

const lightTokens = {
  borderRadiusXS: 4,
  borderRadiusSM: 6,
  borderRadius: 8,
  borderRadiusLG: 12,
  borderRadiusXL: 16,
  borderRadiusFull: 999,
  wireframe: false,
  colorPrimaryBg: 'rgba(239, 237, 229, 1)',
  colorPrimaryBgHover: 'rgba(239, 237, 229, 1)',
  colorPrimaryBorder: 'rgba(186, 90, 59, 1)',
  colorPrimaryBorderHover: 'rgba(194, 98, 66, 1)',
  colorPrimaryHover: 'rgba(194, 98, 66, 1)',
  colorPrimary: 'rgba(208, 110, 78, 1)',
  colorPrimaryActive: 'rgba(194, 98, 66, 1)',
  colorPrimaryTextHover: 'rgba(186, 90, 59, 1)',
  colorPrimaryText: 'rgba(186, 90, 59, 1)',
  colorPrimaryTextActive: 'rgba(186, 90, 59, 1)',
  colorTextBase: 'rgba(26, 26, 29, 1)',
  colorBgBase: 'rgba(255, 255, 255, 1)',
  colorText: 'rgba(20, 20, 19, 0.88)',
  colorTextSecondary: 'rgba(20, 20, 19, 0.65)',
  colorTextTertiary: 'rgba(20, 20, 19, 0.45)',
  colorTextQuaternary: 'rgba(20, 20, 19, 0.25)',
  colorBorder: 'rgba(193, 192, 187, 1)',
  colorBorderSecondary: 'rgba(234, 233, 231, 1)',
  colorFill: 'rgba(43, 27, 0, 0.12)',
  colorFillSecondary: 'rgba(43, 27, 0, 0.08)',
  colorFillTertiary: 'rgba(43, 27, 0, 0.03)',
  colorFillQuaternary: 'rgba(43, 27, 0, 0.02)',
  colorBgContainer: 'rgba(255, 255, 255, 1)',
  colorBgElevated: 'rgba(255, 255, 255, 1)',
  colorBgLayout: 'rgba(249, 248, 244, 1)',
  colorBgSpotlight: 'rgba(20, 20, 19, 0.85)',
  colorBgMask: 'rgba(0, 0, 0, 0.45)',
  colorInfo: 'rgba(11, 131, 241, 1)',
  colorInfoHover: 'rgba(94, 188, 255, 1)',
  colorInfoBorder: 'rgba(135, 209, 255, 1)',
  colorInfoText: 'rgba(11, 131, 241, 1)',
  colorInfoBg: 'rgba(230, 247, 255, 1)',
  colorInfoBgHover: 'rgba(176, 227, 255, 1)',
  colorInfoBorderHover: 'rgba(94, 188, 255, 1)',
  colorSuccess: 'rgba(91, 185, 139, 1)',
  colorSuccessHover: 'rgba(171, 212, 190, 1)',
  colorSuccessBg: 'rgba(235, 250, 240, 1)',
  colorSuccessBgHover: 'rgba(223, 237, 228, 1)',
  colorSuccessBorder: 'rgba(211, 224, 217, 1)',
  colorSuccessBorderHover: 'rgba(171, 212, 190, 1)',
  colorWarning: 'rgba(250, 173, 20, 1)',
  colorWarningHover: 'rgba(255, 214, 102, 1)',
  colorWarningBg: 'rgba(255, 251, 230, 1)',
  colorWarningBgHover: 'rgba(255, 241, 184, 1)',
  colorWarningBorder: 'rgba(255, 229, 143, 1)',
  colorWarningBorderHover: 'rgba(255, 214, 102, 1)',
  colorError: 'rgba(255, 77, 79, 1)',
  colorErrorHover: 'rgba(255, 120, 117, 1)',
  colorErrorBg: 'rgba(255, 242, 240, 1)',
  colorErrorBgHover: 'rgba(255, 241, 240, 1)',
  colorErrorBorder: 'rgba(255, 204, 199, 1)',
  colorErrorBorderHover: 'rgba(255, 163, 158, 1)',
  colorLink: 'rgba(0, 77, 255, 1)',
  boxShadow: '0px 4px 6px 0px rgba(0, 0, 0, 0.08)',
  boxShadowSecondary:
    '0px 12px 24px -16px rgba(0, 0, 0, 0.06), 0px 8px 40px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)',
  colorTextWhite: 'rgba(255, 255, 255, 1)',
  colorFillDisable: 'rgba(235, 235, 232, 1)',
  colorPurple: 'rgba(97, 92, 237, 1)',
  colorPurpleHover: 'rgba(97, 92, 237, 1)',
  colorPurpleBg: 'rgba(226, 222, 255, 0.8)',
  colorPink: 'rgba(236, 72, 153, 1)',
  colorPinkHover: 'rgba(236, 72, 153, 1)',
  colorPinkBg: 'rgba(251, 220, 239, 0.8)',
  colorYellow: 'rgba(234, 179, 8, 1)',
  colorYellowHover: 'rgba(234, 179, 8, 1)',
  colorYellowBg: 'rgba(255, 250, 184, 0.8)',
  colorOrange: 'rgba(250, 129, 37, 1)',
  colorOrangeHover: 'rgba(250, 129, 37, 1)',
  colorOrangeBg: 'rgba(255, 232, 201, 0.8)',
  colorTeal: 'rgba(20, 184, 166, 1)',
  colorTealHover: 'rgba(20, 184, 166, 1)',
  colorTealBg: 'rgba(200, 244, 233, 0.8)',
  colorBlue: 'rgba(0, 144, 255, 1)',
  colorBlueHover: 'rgba(0, 144, 255, 1)',
  colorBlueBg: 'rgba(194, 229, 255, 0.8)',
  colorMauve: 'rgba(142, 140, 153, 1)',
  colorMauveHover: 'rgba(142, 140, 153, 1)',
  colorMauveBg: 'rgba(239, 240, 243, 0.8)',
  boxShadowTertiary:
    '0px 12px 32px -16px rgba(0, 0, 0, 0.24), 0px 12px 60px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)',
  boxShadowTertiaryLeft:
    '-12px 0px 32px -16px rgba(0, 0, 0, 0.24), -12px 0px 60px 0px rgba(0, 0, 0, 0.12), 0px 0px 1px 0px rgba(0, 0, 0, 0.04)',
  boxShadowInput:
    '0px 12px 24px -16px rgba(54, 54, 73, 0.04), 0px 12px 40px 0px rgba(51, 51, 71, 0.08), 0px 0px 1px 0px rgba(44, 44, 54, 0.02)',
};
const darkTokens = {
  borderRadiusXS: 4,
  borderRadiusSM: 6,
  borderRadius: 8,
  borderRadiusLG: 12,
  borderRadiusXL: 16,
  borderRadiusFull: 999,
  wireframe: false,
  colorPrimaryBg: 'rgba(41, 35, 29, 1)',
  colorPrimaryBgHover: 'rgba(70, 60, 50, 1)',
  colorPrimaryBorder: 'rgba(133, 80, 40, 1)',
  colorPrimaryBorderHover: 'rgba(146, 102, 52, 1)',
  colorPrimaryHover: 'rgba(156, 88, 57, 1)',
  colorPrimary: 'rgba(156, 88, 57, 1)',
  colorPrimaryActive: 'rgba(146, 102, 52, 1)',
  colorPrimaryTextHover: 'rgba(133, 80, 40, 1)',
  colorPrimaryText: 'rgba(160, 96, 63, 1)',
  colorPrimaryTextActive: 'rgba(133, 80, 40, 1)',
  colorTextBase: 'rgba(230, 230, 230, 1)',
  colorBgBase: 'rgba(15, 13, 12, 1)',
  colorText: 'rgba(230, 230, 230, 0.9)',
  colorTextSecondary: 'rgba(180, 180, 180, 0.8)',
  colorTextTertiary: 'rgba(255, 255, 255, 0.6)',
  colorTextQuaternary: 'rgba(255, 255, 255, 0.3)',
  colorBorder: 'rgba(82, 76, 73, 1)',
  colorBorderSecondary: 'rgba(50, 46, 43, 1)',
  colorFill: 'rgba(82, 79, 74, 0.32)',
  colorFillSecondary: 'rgba(83, 79, 72, 0.24)',
  colorFillTertiary: 'rgba(50, 50, 50, 0.26)',
  colorFillQuaternary: 'rgba(67, 64, 59, 0.12)',
  colorBgContainer: 'rgba(52, 50, 47, 1)',
  colorBgElevated: 'rgba(47, 42, 39, 1)',
  colorBgLayout: 'rgba(26, 26, 24, 1)',
  colorBgSpotlight: 'rgba(10, 10, 10, 0.85)',
  colorBgMask: 'rgba(0, 0, 0, 0.6)',
  colorInfo: 'rgba(11, 131, 241, 1)',
  colorInfoHover: 'rgba(94, 188, 255, 1)',
  colorInfoBorder: 'rgba(57, 144, 201, 1)',
  colorInfoText: 'rgba(11, 131, 241, 1)',
  colorInfoBg: 'rgba(17, 34, 51, 1)',
  colorInfoBgHover: 'rgba(38, 72, 102, 1)',
  colorInfoBorderHover: 'rgba(94, 188, 255, 1)',
  colorSuccess: 'rgba(91, 185, 139, 1)',
  colorSuccessHover: 'rgba(116, 164, 124, 1)',
  colorSuccessBg: 'rgba(41, 54, 49, 1)',
  colorSuccessBgHover: 'rgba(63, 81, 71, 1)',
  colorSuccessBorder: 'rgba(115, 163, 119, 1)',
  colorSuccessBorderHover: 'rgba(171, 212, 190, 1)',
  colorWarning: 'rgba(250, 173, 20, 1)',
  colorWarningHover: 'rgba(255, 140, 40, 1)',
  colorWarningBg: 'rgba(45, 35, 26, 1)',
  colorWarningBgHover: 'rgba(77, 59, 35, 1)',
  colorWarningBorder: 'rgba(255, 166, 40, 1)',
  colorWarningBorderHover: 'rgba(255, 214, 102, 1)',
  colorError: 'rgba(255, 77, 79, 1)',
  colorErrorHover: 'rgba(238, 101, 78, 1)',
  colorErrorBg: 'rgba(43, 11, 8, 1)',
  colorErrorBgHover: 'rgba(74, 42, 40, 1)',
  colorErrorBorder: 'rgba(238, 91, 78, 1)',
  colorErrorBorderHover: 'rgba(255, 163, 158, 1)',
  colorLink: 'rgba(77, 125, 255, 1)',
  colorTextWhite: 'rgba(255, 255, 255, 1)',
  colorFillDisable: 'rgba(90, 90, 90, 1)',
  colorPurple: 'rgba(97, 92, 237, 1)',
  colorPurpleHover: 'rgba(97, 92, 237, 1)',
  colorPurpleBg: 'rgba(60, 53, 85, 1)',
  colorPink: 'rgba(236, 72, 153, 1)',
  colorPinkHover: 'rgba(236, 72, 153, 1)',
  colorPinkBg: 'rgba(74, 42, 58, 1)',
  colorYellow: 'rgba(234, 179, 8, 1)',
  colorYellowHover: 'rgba(234, 179, 8, 1)',
  colorYellowBg: 'rgba(77, 71, 32, 1)',
  colorOrange: 'rgba(250, 129, 37, 1)',
  colorOrangeHover: 'rgba(250, 129, 37, 1)',
  colorOrangeBg: 'rgba(255, 232, 201, 0.8)',
  colorTeal: 'rgba(20, 184, 166, 1)',
  colorTealHover: 'rgba(20, 184, 166, 1)',
  colorTealBg: 'rgba(42, 70, 64, 1)',
  colorBlue: 'rgba(0, 144, 255, 1)',
  colorBlueHover: 'rgba(0, 144, 255, 1)',
  colorBlueBg: 'rgba(42, 59, 78, 0.8)',
  colorMauve: 'rgba(142, 140, 153, 1)',
  colorMauveHover: 'rgba(142, 140, 153, 1)',
  colorMauveBg: 'rgba(46, 46, 50, 0.8)',
  boxShadow: '0px 4px 6px 0px rgba(255, 255, 255, 0.08)',
  boxShadowSecondary:
    '0px 12px 24px -16px rgba(255, 255, 255, 0.04), 0px 8px 40px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.03)',
  boxShadowTertiary:
    '0px 12px 32px -16px rgba(255, 255, 255, 0.16), 0px 12px 60px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.04)',
  boxShadowTertiaryLeft:
    '-12px 0px 32px -16px rgba(255, 255, 255, 0.16), -12px 0px 60px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.04)',
  boxShadowInput:
    '0px 12px 24px -16px rgba(254, 254, 255, 0.04), 0px 12px 40px 0px rgba(255, 255, 255, 0.08), 0px 0px 1px 0px rgba(255, 255, 255, 0.02)',
};

// @ts-ignore
export const darkTheme = generateThemeByToken(darkTokens, true);
// @ts-ignore
export const lightTheme = generateThemeByToken(lightTokens, false);
`},54804:function(h,n){n.Z=`import Chat from './components/Chat';
import ConfigProvider from './context/ConfigProvider';
import DemoProvider from './context/DemoProvider';
import Style from './style';

export default function () {

  return <ConfigProvider>
    <Style />
    <DemoProvider>
      <Chat />
    </DemoProvider>
  </ConfigProvider>
}`}}]);
